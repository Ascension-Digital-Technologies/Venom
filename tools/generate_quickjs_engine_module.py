...
def main() -> int:
    ap=argparse.ArgumentParser(description='Generate the C++ QuickJS browser-engine wrapper from authored JS modules.')
    ...
    if not modules: raise SystemExit('no QuickJS engine modules found')
    js=''.join(p.read_text(encoding='utf-8') for p in modules)
    if ')QJSENGINE"' in js: raise SystemExit('QuickJS engine source contains reserved raw-string delimiter')
