#!/usr/bin/env python3
"""Preflight the upstream QuickJS WASM cutover toolchain and ABI inputs.
...
def manifest(info: dict) -> str:
    ...
    lines = [
        ...
        f'quickjs_scaffold_sha256={info.get("quickjs_scaffold_sha256", "")}',
...
def main() -> int:
    ...
    scaffold = root / 'src/runtime/quickjs_runtime_scaffold.c'
    ...
    info = {
        ...
        'quickjs_scaffold_sha256': sha256_file(scaffold) if scaffold.exists() else '',
    ...
    print(f'[venom] QuickJS WASM preflight status: {status}')
