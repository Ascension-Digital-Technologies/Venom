#!/usr/bin/env python3
"""Control the verified upstream TurboJS WASM cutover workflow.

This tool intentionally separates three states:

1. repository/toolchain readiness (`--preflight-only`),
2. verified artifact embedding (`--artifact turbojs-runtime.wasm`), and
...
claims a verified upstream TurboJS WASM artifact with all ABI12 exports present
...
REQUIRED_REAL_PROVENANCE = {
    ...
    'artifact_kind': 'upstream-turbojs-wasm',
    'runtime_implementation': 'turbojs-wasm-upstream-turbojs',
    'runtime_claim': 'full-upstream-turbojs-wasm',
    'contract_only': 'false',
    'scaffold_runtime': 'false',
    'real_engine_candidate': 'true',
    'full_upstream_turbojs': 'true',
...
def embedded_provenance(header: Path) -> tuple[str, dict[str, str]]:
    ...
    if not m:
        raise SystemExit(f'could not find kTurboJsRuntimeWasmBlobProvenance in {header}')
...
def write_status(out_dir: Path, name: str, data: dict[str, object]) -> None:
    ...
    lines = [f'VENOM_TJS_WASM_CUTOVER_{name.upper()}_V1']
...
def preflight(repo_root: Path, out_dir: Path, emcc: str | None, allow_missing: bool) -> int:
    manifest = out_dir / 'turbojs-wasm-preflight.txt'
    report = out_dir / 'turbojs-wasm-preflight.json'
    cmd = [
        sys.executable,
        str(repo_root / 'tools' / 'turbojs_wasm_preflight.py'),
...
def verify_and_embed_artifact(repo_root: Path, artifact: Path, out_dir: Path, embed_out: Path, skip_embed: bool) -> int:
    if not artifact.is_file():
        ...
        return 2
    manifest = out_dir / 'turbojs-runtime.manifest.txt'
    exports_json = out_dir / 'turbojs-runtime.exports.json'
    release_abi_json = out_dir / 'turbojs-runtime.release-abi.json'
    abi_cmd = [sys.executable, str(repo_root / 'tools' / 'turbojs_release_abi.py'), str(release_abi_json)]
    ...
    if b'VTJSBC03' not in artifact_bytes or b'VTJSBC02' in artifact_bytes:
        print('[venom] artifact is not the native VTJSBC03 runtime or still contains the legacy VTJSBC02 decoder', file=sys.stderr)
        return 3
    with manifest.open('a', encoding='utf-8') as fp:
        fp.write('bytecode_format=VTJSBC03\n')
        fp.write('module_bundle_contract=VTJSMB04\n')
        ...
        fp.write('venom_tjs_wasm_native_stack_capacity=4194304\n')
    kv = parse_kv(manifest.read_text(encoding='utf-8'))
    if kv.get('artifact_kind') != 'upstream-turbojs-wasm' or kv.get('required_exports_satisfied') != 'true':
        print('[venom] artifact did not satisfy strict upstream TurboJS WASM manifest requirements', file=sys.stderr)
        return 3
    if not skip_embed:
        embed_cmd = [
            sys.executable,
...
def main() -> int:
    ...
    ap.add_argument('--artifact', type=Path, help='verified Emscripten-generated turbojs-runtime.wasm to inspect/embed')
    ap.add_argument('--embed-out', type=Path, default=None)
    ap.add_argument('--skip-embed', action='store_true')
    ap.add_argument('--verify-embedded', action='store_true')
    ap.add_argument('--require-real', action='store_true', help='fail unless embedded provenance proves real upstream TurboJS WASM')
    args = ap.parse_args()

    repo_root = args.repo_root.resolve()
    out_dir = (args.out_dir or (repo_root / 'build' / 'turbojs-wasm-cutover')).resolve()
    embed_out = (args.embed_out or (repo_root / 'src' / 'generated' / 'runtime' / 'turbojs_runtime_wasm_blob.hpp')).resolve()
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if args.verify_embedded or args.require_real:
        ...
        status = {
            ...
            'full_upstream_turbojs': info.get('full_upstream_turbojs', ''),
        ...
        print('[venom] embedded TurboJS WASM provenance:')
        for key in ('package_version', 'artifact_kind', 'runtime_implementation', 'real_engine_candidate', 'full_upstream_turbojs', 'required_exports_satisfied', 'finish_blocker'):
