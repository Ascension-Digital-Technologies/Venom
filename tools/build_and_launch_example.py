...
EXAMPLES = {
    '1': ('protected-chess', 8080),
    '2': ('nova-trade', 8081),
    '3': ('bot-detection', 8082),
    '4': ('typescript-showcase', 8083),
    '5': ('tsx-showcase', 8084),
    '6': ('aegis-operations', 8085),
    '7': ('javascript-playground', 8086),
}
...
def run(cmd, cwd, label, visible=False):
    info(f'{label}...')
    if visible:
        proc = subprocess.run([str(x) for x in cmd], cwd=cwd)
    else:
        proc = subprocess.run(
            [str(x) for x in cmd], cwd=cwd,
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    if proc.returncode:
        if not visible and proc.stdout:
            print(proc.stdout.rstrip(), file=sys.stderr)
        raise LauncherStepError(label, proc.returncode)
    passed(label)


def main():
    ap=argparse.ArgumentParser(description='Build Venom and launch one of the production examples.')
    ap.add_argument('example', choices=EXAMPLES)
    ap.add_argument('--profile', choices=['prod'])
    ap.add_argument('--port', type=int)
    ap.add_argument('--no-open', action='store_true')
    ap.add_argument('--build-dir', default='build')
    ns=ap.parse_args()
    root=Path(__file__).resolve().parents[1]
    name, default_port=EXAMPLES[ns.example]
    profile='prod'
    ...
    QuietHandler.playground_enabled = ns.example == '7'
    QuietHandler.playground_token = secrets.token_urlsafe(32) if ns.example == '7' else ""
