...
# The browser runtime uses QuickJS core only. quickjs-libc.c is intentionally excluded:
...
if O="$(python3 "$ROOT/tools/quickjs_runtime_lifecycle.py" resolve-wasm-opt --repo-root "$ROOT" --format path 2>/dev/null)"; then
