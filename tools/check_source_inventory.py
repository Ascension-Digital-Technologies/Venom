...
and tests/fuzz/targets/ must be named in CMakeLists.txt.
"""
...
def main() -> int:
    ...
    missing: list[str] = []
    for base in (root / "src", root / "tests" / "fuzz" / "targets"):
        for path in base.rglob("*"):
            if not path.is_file() or path.suffix.lower() not in SOURCE_SUFFIXES:
                continue
            rel = path.relative_to(root).as_posix()
            if rel in EXTERNAL_WASM_SOURCES or rel in CHECKED_IN_GENERATED_SNAPSHOTS or rel.startswith(IGNORED_PREFIXES):
                continue
            if rel not in cmake:
                missing.append(rel)
