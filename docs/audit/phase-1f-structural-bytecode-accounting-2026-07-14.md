# Phase 1F: Structural TurboJS Bytecode Accounting
...
The bot-detection production build emitted one TurboJS registry envelope containing two protected function intents. The release checker reported two TurboJS records even though `protection_closure.expected_turbojs_records` correctly reported one.

The prior checker counted occurrences of the ASCII envelope magic `VTJSE006` across decoded package section bytes. This remained vulnerable to false positives from non-executable data and embedded metadata.
...
3. begin with `VTJSE006` at payload offset zero.

Multiple protected functions compiled into one registry envelope therefore count as one executable TurboJS record, matching the compiler closure model.
...
- Old broad `count_bytes(section.data, "VTJSE006")` release accounting removed: PASS
...
- expected TurboJS records: 1
- emitted TurboJS records: 1
- release status: PASS
