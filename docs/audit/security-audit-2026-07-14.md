...
- TurboJS bytecode creation and protected export bridge
...
- Undisclosed browser and TurboJS zero-days
...
1. Production TurboJS records are required for protected functions, and the build fails when protected exports produce zero records.
...
5. TurboJS/WASM runtime provenance and required exports are verified.
...
Fail-closed TurboJS/WASM execution is a strong control. The runtime still executes on an attacker-controlled client, so bytecode, memory, and bridge behavior remain observable. WASM memory should be assumed readable by a sufficiently motivated attacker.

Recommended improvement: add instruction, wall-clock, heap, recursion, and output quotas at the TurboJS context level—not only at the browser request layer—and test forced termination during loops, allocation storms, deep recursion, and promise queues.
...
- TurboJS termination tests for infinite loops, microtask starvation, and allocation limits
