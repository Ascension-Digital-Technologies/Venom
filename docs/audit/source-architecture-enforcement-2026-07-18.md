...
- Extracted shared CLI/build option models into `src/core/options.hpp`.
- Extracted `JsChunk` and module-edge models into `src/graph/module_types.hpp`, removing the graph-to-pipeline dependency.
...
- Native runtime and TurboJS runtime probes built successfully.
...
- Added CTest coverage for native package execution and TurboJS native parity.
- Version output: `venom 2.0.0`.
- Final focused build, production-runtime, architecture, and ownership gate set passed 12/12.
- TurboJS native parity passed 6/6 checks.
