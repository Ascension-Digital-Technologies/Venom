...
The browser created `capturedAtMs` using the browser's live `Date.now()`. The protected QuickJS/WASM context independently evaluated `Date.now()` and could expose a non-advancing or differently based clock. The protected engine compared those clocks with a fixed 30-second tolerance, eventually rejecting fresh requests as `stale-telemetry`.
...
The protected engine no longer assumes browser and QuickJS wall clocks are synchronized. The browser also renegotiates once on `stale-telemetry` as a defensive recovery path.
