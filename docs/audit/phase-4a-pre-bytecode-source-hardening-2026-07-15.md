...
Reduce information recoverable from protected QuickJS bytecode without replacing the QuickJS execution engine or weakening JavaScript compatibility.

## Changes

Production builds now run protected JavaScript through the embedded Terser + javascript-obfuscator pipeline **before** QuickJS bytecode compilation. Development builds remain unchanged for diagnostics.
...
- application-specific string constants are encoded before QuickJS serializes constants;
- protected bytecode differs between independently salted builds;
- a generic QuickJS bytecode extractor receives already-obfuscated program logic.
...
- `quickjs_module_bundle_runtime` passed.

## Residual risks

This pass does not remove:

- recognizable QuickJS runtime machinery;
- fixed `VQJSE006` / `VJSB0006` envelope markers;
- the publicly understood QuickJS bytecode instruction set after runtime decoding;
