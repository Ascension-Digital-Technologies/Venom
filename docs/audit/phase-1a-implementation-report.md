...
Production builds now fail with `VNM-PROT-1001` whenever a function-level protected bridge candidate is not successfully extracted into the protected TurboJS registry. This closes the verified path where an explicitly protected arrow function could remain in browser JavaScript while the production build succeeded.
