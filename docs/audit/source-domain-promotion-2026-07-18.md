...
- `src/engine/quickjs/` became `src/quickjs/`.
- `src/engine/vm/` became `src/vm/`.
- Native files from `src/runtime/native/` moved directly into `src/runtime/`.
- Compiler and runtime JavaScript inputs were consolidated into `src/templates/{browser,quickjs-engine,typescript}/`.
...
The source tree now has ten clear domains: `cli`, `compiler`, `frontends`, `generated`, `package`, `pipeline`, `quickjs`, `runtime`, `templates`, and `vm`.
...
- Source inventory, source layout, CMake completeness, repository consistency, launcher structure, template ownership, browser runtime bundling, QuickJS module bundling, and TypeScript frontend integration checks passed.
