...
Extraction uses token-aware balanced range scanning for function and arrow bodies instead of the previous declaration-only line regex. The protected implementation is moved into the QuickJS bridge registry and the browser receives an asynchronous opaque-ID bridge stub.
...
- one resolved arrow intent maps to the protected registry and one QuickJS envelope;
