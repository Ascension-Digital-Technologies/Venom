...
The browser runtime now applies `VENOM_HOST_CAPABILITIES_V3` at TurboJS binding creation. Each script chunk receives only its declared host globals. The injected `__venomRuntime` value is a frozen capability-scoped facade rather than the complete bridge object.
