...
- The closure summary records requested, resolved, whole-file, and expected TurboJS record counts.
- `verify` independently reads `build.json` and compares requested versus resolved intents.
- `verify` compares expected protected TurboJS records against records found across all decoded package sections, including lazy script sections.
...
- Emitted TurboJS bytecode record count mismatch.
...
- Whole-file package inspection finds one TurboJS envelope across lazy sections: PASS.
