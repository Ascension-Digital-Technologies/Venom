...
Venom uses one persistent, isolated QuickJS runtime for the native JavaScript hardener during a compiler process. The runtime loads the embedded Terser and JavaScript Obfuscator bundles once, then reuses them for protected registries, runtime assets, the QuickJS engine adapter, workers, and the bootstrap loader.

## Safety boundaries

- Calls are serialized because a QuickJS runtime is not entered concurrently.
...
Protected source that is immediately compiled into authenticated QuickJS bytecode uses a bytecode-oriented policy. Expensive browser-source obfuscation is avoided because the source is not shipped as executable browser JavaScript; protection is provided by native QuickJS bytecode, VBC section encryption, integrity binding, and fail-closed runtime verification.
