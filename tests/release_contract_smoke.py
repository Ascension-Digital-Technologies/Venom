...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run([sys.executable,str(ROOT/'tools/generate_contract_manifest.py'),'--header',str(ROOT/'src/generated/contracts/product_contracts.hpp'),'--version','1.44.0','--out',str(old)],check=True)
    cmake=(ROOT/'CMakeLists.txt').read_text(); version=re.search(r'VERSION\s+(\d+\.\d+\.\d+)',cmake).group(1)
    subprocess.run([sys.executable,str(ROOT/'tools/generate_contract_manifest.py'),'--header',str(ROOT/'src/generated/contracts/product_contracts.hpp'),'--version',version,'--out',str(cur)],check=True)
