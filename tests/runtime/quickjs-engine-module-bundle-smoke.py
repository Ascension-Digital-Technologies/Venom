...
mods=sorted((root/'src/templates/quickjs-engine').glob('*.js'))
assert len(mods)>=5
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run([sys.executable,str(root/'tools/generators/runtime/generate_quickjs_engine_module.py'),
      '--input-dir',str(root/'src/templates/quickjs-engine'),
      '--prefix',str(root/'tools/generators/runtime/templates/quickjs_engine_module.prefix.cpp'),
      '--suffix',str(root/'tools/generators/runtime/templates/quickjs_engine_module.suffix.cpp'),
