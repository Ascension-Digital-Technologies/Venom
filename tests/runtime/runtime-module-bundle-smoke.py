...
modules=sorted((root/'src/runtime/js/browser').glob('*.js'))
assert len(modules)>=7
expected=''.join(p.read_text(encoding='utf-8') for p in modules)
checked=(root/'src/runtime/templates/runtime.js').read_text(encoding='utf-8')
assert checked==expected, 'checked-in browser runtime bundle drifted from authored modules'
with tempfile.TemporaryDirectory() as td:
    out=Path(td)/'runtime.js'
    subprocess.run([sys.executable,str(root/'tools/bundle_js_modules.py'),'--input-dir',str(root/'src/runtime/js/browser'),'--output',str(out)],check=True)
