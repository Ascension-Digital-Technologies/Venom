...
[[noreturn]] void fail(const std::string& message) {
  std::cerr << "QuickJS stack-overflow safety test failed: " << message << '\n';
  std::exit(1);
}
...
int main() {
  ...
  JS_SetMaxStackSize(runtime, venom::quickjs::kDefaultStackLimitBytes);
  ...
  std::cout << "recursive fingerprint overflow is a catchable QuickJS RangeError: PASS\n";
  return 0;
}
