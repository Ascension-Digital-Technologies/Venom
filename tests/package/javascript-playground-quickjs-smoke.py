...
with tempfile.TemporaryDirectory(prefix='venom-playground-') as td:
    ...
    required=['executeQuickJsChunk','quickJsWasm','clearQuickJsConsoleEvents']
    ...
    if 'await eval(${JSON.stringify(source)})' not in source: raise SystemExit('QuickJS completion-value wrapper is missing')
    ...
    if 'release-diversification.vrd3' in inspection or 'quickjs-abi-fingerprint.vqaf' in inspection:
