...
for token in ('VENOM_COMPILER_CACHE','VENOM_ENABLE_PCH','VENOM_ENABLE_MSVC_MP','VENOM_ENABLE_TURBOJS_IPO','CMAKE_CXX_COMPILER_LAUNCHER'):
    if token not in cm: fail.append(f'missing build acceleration contract: {token}')
for token in ('venom_configure_compiler_cache()','venom_apply_build_acceleration(tjs)'):
