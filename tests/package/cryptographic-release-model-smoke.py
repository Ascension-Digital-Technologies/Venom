...
def main():
    ...
    root=Path(sys.argv[1]).resolve(); venom=Path(sys.argv[2]).resolve(); out=Path(sys.argv[3]).resolve()
    ...
    run([sys.executable,str(verifier),str(out),'--strict-signature','--public-key',str(pub),'--min-release-sequence','90','--expect-channel','stable','--expect-version','1.0.1'])
