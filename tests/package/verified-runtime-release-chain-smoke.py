...
with tempfile.TemporaryDirectory() as td:
    d=Path(td)
    pkg=d/'venom-v1.16.0-linux.zip'; pkg.write_bytes(b'package')
    ...
    out=d/'set'
    cmd=[sys.executable,str(root/'tools/package_release_set.py'),'--version','1.16.0','--source-revision','test','--packages',str(pkg),'--compatibility-evidence',str(evidence),'--verified-runtime',str(wasm),'--out',str(out),'--source-date-epoch','1704067200']
    subprocess.run(cmd,check=True,capture_output=True,text=True)
    obj=json.loads((out/'RELEASE_SET.json').read_text())
    assert obj['verified_runtime']['name']=='quickjs-runtime.wasm'
    verify=subprocess.run([sys.executable,str(root/'tools/verify_release_set.py'),str(out),'--expect-version','1.16.0','--require-evidence','--require-runtime'],capture_output=True,text=True)
