...
for key in ('contract_only', 'scaffold_runtime', 'real_engine_candidate', 'full_upstream_turbojs', 'required_exports_satisfied'):
...
assert '__VENOM_TURBOJS_FALLBACK_BLOCK__' in engine
assert 'erase_once("      const fn = new Function' not in runtime
assert 'protected TurboJS module fallback block marker missing' not in engine
...
with tempfile.TemporaryDirectory() as td:
    d=Path(td)
    pkg=d/'venom-v3.0.0-linux.zip'; pkg.write_bytes(b'package')
    evidence=d/'evidence.zip'; evidence.write_bytes(b'evidence')
    wasm=d/'turbojs-runtime.wasm'; wasm.write_bytes(b'\0asmcanonical-runtime')
    out=d/'set'
    cmd=[sys.executable,str(root/'tools/package_release_set.py'),'--version','3.0.0','--source-revision','test','--packages',str(pkg),'--compatibility-evidence',str(evidence),'--verified-runtime',str(wasm),'--out',str(out),'--source-date-epoch','1704067200']
    subprocess.run(cmd,check=True,capture_output=True,text=True)
    obj=json.loads((out/'RELEASE_SET.json').read_text())
    assert obj['verified_runtime']['name']=='turbojs-runtime.wasm'
    verify=subprocess.run([sys.executable,str(root/'tools/verify_release_set.py'),str(out),'--expect-version','3.0.0','--require-evidence','--require-runtime'],capture_output=True,text=True)
