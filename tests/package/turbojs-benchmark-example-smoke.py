...
example = repo / "examples" / "turbojs-benchmark"
required = [
    "index.html",
    "browser/app.js",
...
for relative in required:
    if not (example / relative).is_file():
        raise SystemExit(f"TurboJS benchmark example is missing {relative}")

html = (example / "index.html").read_text(encoding="utf-8")
for marker in ('id="start-benchmark"', 'id="results-body"', 'src="./browser/app.js"'):
    if marker not in html:
        raise SystemExit(f"TurboJS benchmark HTML is missing {marker}")
...
for marker in ("runBenchmarkCase", "benchmarkIdentity", "typedArrays", "mapsSets", "allocation"):
    if marker not in benchmark:
        raise SystemExit(f"TurboJS benchmark protected suite is missing {marker}")
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    if not ir_path.is_file():
        raise SystemExit("TurboJS benchmark project IR was not emitted")
    ...
    if emitted_text:
        for exported in ("runBenchmarkCase", "benchmarkIdentity"):
            if exported not in emitted_text:
                raise SystemExit(f"TurboJS benchmark package metadata is missing {exported}")
...
print("TurboJS benchmark example smoke: PASS")
