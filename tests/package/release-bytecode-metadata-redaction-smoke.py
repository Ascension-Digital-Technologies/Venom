...
bytecode = (root / 'src/turbojs/bytecode.cpp').read_text(encoding='utf-8')
turbojs_h = (root / 'third_party/turbojs/turbojs.h').read_text(encoding='utf-8')
turbojs_c = (root / 'third_party/turbojs/turbojs.c').read_text(encoding='utf-8')
...
for text, token in ((turbojs_h, 'JS_RewriteModuleRequestNames'),
                    (turbojs_c, 'int JS_RewriteModuleRequestNames(')):
    if token not in text:
        raise SystemExit(f'missing TurboJS module request rewrite API: {token}')
