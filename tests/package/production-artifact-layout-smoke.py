...
with tempfile.TemporaryDirectory(prefix='venom-v091-') as td:
    out=Path(td)/'dist'
    cmd=[str(venom),'build',str(root/'tests'/'fixtures'/'production-site'),'--out',str(out),'--profile','browser-protect','--runtime','wasm','--hashed','--strict-release','--vendor-cache',str(root/'tests'/'fixtures'/'remote-cache'),'--offline']
