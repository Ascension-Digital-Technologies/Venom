...
def main() -> int:
    if len(sys.argv) != 3:
        print('usage: quickjs-wasm-cutover-workflow-smoke.py <repo-root> <out-dir>', file=sys.stderr)
        return 2
    ...
    cutover_tool = root / 'tools' / 'quickjs_wasm_cutover.py'
    ...
    preflight_text = (cutover_out / 'quickjs-wasm-preflight.txt').read_text(encoding='utf-8')
    for marker in ('VENOM_QJS_WASM_PREFLIGHT_V1', 'package_version=83', 'missing_required_export_count=0'):
    ...
    if passed.returncode != 0:
        print(passed.stdout)
        print('strict real-engine verification should pass for the checked-in upstream QuickJS WASM artifact', file=sys.stderr)
        return 1
    ...
    for marker in ('VENOM_QJS_WASM_CUTOVER_EMBEDDED_STATUS_V1', 'artifact_kind=upstream-quickjs-wasm', 'strict_real=true'):
        if marker not in status_text:
            print(passed.stdout)
            print(f'missing embedded status marker {marker!r}', file=sys.stderr)
            return 1

    print('quickjs wasm cutover workflow smoke: PASS')
    return 0
