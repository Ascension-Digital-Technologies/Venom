...
bootstrap = js + '\n' + worker
tjs = (root/'src/generated/runtime/turbojs_engine_module.cpp').read_text()
...
required = [
    ...
    ('TurboJS digest attestation', 'TurboJS WASM digest attestation mismatch' in bootstrap),
    ...
    ('worker wasm instantiate probe', 'probeTurboJsModule' in bootstrap and 'WebAssembly.instantiate(module' in bootstrap),
    ('required TurboJS export presence gate', 'missingRequired' in worker and 'requiredKinds' in worker),
    ...
    ('unapproved TurboJS export rejection', 'unapprovedExtras' in worker),
    ('TurboJS export kind validation', 'kindMismatches' in worker),
    ('no brittle exact export-count gate', 'actual.length !== required.length' not in worker),
    ('worker runtime readiness attestation', 'turboJsRuntimeReady: true' in bootstrap and 'readiness attestation missing' in bootstrap),
    ('compiled module reuse', '__venomWorkerTurboJsModule' in tjs),
