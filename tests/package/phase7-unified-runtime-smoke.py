...
build=(root/'src/pipeline/build.cpp').read_text() + (root/'src/pipeline/build_package_metadata.cpp').read_text() + (root/'src/pipeline/build_runtime_metadata.cpp').read_text() + (root/'src/pipeline/build_runtime_audit_metadata.cpp').read_text() + (root/'src/pipeline/build_runtime_module_metadata.cpp').read_text() + (root/'src/pipeline/planning/section_plan.cpp').read_text()
...
checks={
 ...
 'fingerprint section':'turbojs-abi-fingerprint.vqaf' in build,
 'fingerprint parser':'parseTurboJsAbiFingerprint' in runtime,
 ...
 '23 function ABI':abi.count('\"venom_tjs_') == 23,
}
