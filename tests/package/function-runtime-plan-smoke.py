...
for marker in [
    ...
    if marker not in js:
        raise SystemExit(f'missing function runtime planner marker: {marker}')
for marker in ['function-plan.txt', 'function-plan.json']:
    if marker not in build:
        raise SystemExit(f'missing function runtime report emission: {marker}')
print('function runtime plan smoke passed')
