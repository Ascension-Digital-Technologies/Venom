...
def run(js):
    ...
    p=subprocess.run([str(venom),'build',str(site),'--out',str(base/'out'),'--profile','dev'],text=True,capture_output=True)
    return td,p
...
for source in (
    ...
    td,p=run(source)
    if p.returncode == 0 or 'malformed file-scope Venom realm directive' not in p.stdout+p.stderr:
        raise SystemExit('malformed file directive did not fail closed\n'+p.stdout+p.stderr)
    td.cleanup()

td,p=run('// @venom: browser\nconst x=1;\n// @venom: protected extra\nexport function y(){return x;}\n')
if p.returncode == 0 or 'malformed function-level Venom realm directive' not in p.stdout+p.stderr:
    raise SystemExit('malformed function directive did not fail closed\n'+p.stdout+p.stderr)
td.cleanup()
print('[PASS] exact realm directive grammar rejects typos and illegal modifiers')
