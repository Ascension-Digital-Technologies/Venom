...
ps=(root/'tools'/'windows-scripts'/'release-closure.ps1').read_text(encoding='utf-8')
sh=(root/'scripts'/'linux'/'release-closure.sh').read_text(encoding='utf-8')
bat=(root/'scripts'/'windows'/'release-closure.bat').read_text(encoding='utf-8')
required=(
 'final_release_gate.py','release_closure.py','cmake','ctest',
 'doctor-production','protected-chess','nova-trade','bot-detection','analyze','check_production_leaks.py',
...
if 'run_release_closure.py' not in ps or 'run_release_closure.py' not in sh or 'release-closure.ps1' not in bat:
