...
if len(sys.argv) != 4:
    print('usage: quickjs-upstream-runtime-bridge-smoke.py <venom> <site> <out>')
    raise SystemExit(2)
...
for marker in ['upstreamQuickJsReady', 'upstreamParityRecord', 'upstreamObjectModelScore', 'upstreamBytecodeSemanticsScore', 'upstreamBytecodeSemanticsRecord', 'quickjs-wasm-abi12-upstream-global-host-api-shims']:
    if marker not in engine:
        raise SystemExit(f'quickjs engine module missing upstream marker: {marker}')

print('QuickJS upstream bytecode semantics smoke passed')
