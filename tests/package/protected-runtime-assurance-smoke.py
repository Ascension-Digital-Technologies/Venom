...
build_ps=(root / "scripts/build-site.ps1").read_text(errors="ignore")
build_sh=(root / "scripts/build-site.sh").read_text(errors="ignore")
checks = {
    "stale compiler prevention": "SkipCompilerBuild" in build_ps and "build.ps1" in build_ps and "rm -rf" in build_sh,
    "mandatory production gate": "release-check" in build_ps and "verify-runtime" in build_ps and "release-check" in build_sh and "verify-runtime" in build_sh,
