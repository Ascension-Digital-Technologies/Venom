...
checks = {
    ...
    "opaque protected transport ids": "__venomInvokeProtectedById" in (root / "src/pipeline/js_rewriting.cpp").read_text(errors="ignore") and "__venomInvokeProtectedByName" not in (root / "src/pipeline/js_rewriting.cpp").read_text(errors="ignore"),
    "protected chess bytecode gate": "turbojs_bytecode_records" in (root / "tests/package/protected-chess-engine-smoke.py").read_text(errors="ignore"),
    "release leak scan": any("leak" in p.name for p in (root / "tests/package").glob("*leak*")),
    "native bytecode security test": (root / "tests/turbojs/native-bytecode-security.cpp").is_file(),
}
