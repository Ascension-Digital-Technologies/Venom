...
def main()->int:
    ...
    tool=root/'tools/turbojs_runtime_lifecycle.py'
    ...
    for marker in ('turbojs_runtime_lifecycle.py','New-LifecycleArguments','$env:EMCC','$resolvedEmccPath','[switch]$Force','[switch]$Status'):
    ...
    if '"--emcc", $Emcc' in ps or '--emcc $Emcc `' in ps:
        ...
        return 1
    for marker in ('turbojs_runtime_lifecycle.py','--force','--status'):
        if marker not in sh: print('missing sh marker',marker); return 1
    print('turbojs runtime lifecycle smoke: PASS'); return 0
