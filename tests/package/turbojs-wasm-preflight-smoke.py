...
def main() -> int:
    if len(sys.argv) != 3:
        print('usage: turbojs-wasm-preflight-smoke.py <repo-root> <out-dir>', file=sys.stderr)
        return 2
    root = Path(sys.argv[1]).resolve()
    out = Path(sys.argv[2]).resolve()
    out.mkdir(parents=True, exist_ok=True)
    manifest = out / 'turbojs-wasm-preflight.txt'
    report = out / 'turbojs-wasm-preflight.json'
    tool = root / 'tools' / 'turbojs_wasm_preflight.py'
    ...
    if len(info['exports']) < len(info.get('required_probe_exports', [])):
        ...
        return 1
    for name in ('venom_tjs_engine_abi', 'venom_tjs_execute_bytecode', 'venom_tjs_real_engine_candidate', 'venom_tjs_upstream_turbojs_ready'):
        if name not in info['exports']:
            print(f'missing ABI export {name}', file=sys.stderr)
            return 1

    script = root / 'tools' / 'linux-scripts' / 'build-turbojs-wasm.sh'
    ...
    if not (out / 'script-preflight' / 'turbojs-wasm-preflight.txt').exists():
        print('build-turbojs-wasm --preflight-only did not write preflight manifest', file=sys.stderr)
        return 1

    print('turbojs wasm preflight smoke: PASS')
    return 0
