...
EXPECTED = 83
CHECKS = {
    'src/turbojs/abi.hpp': r'kRuntimePackageVersion\s*=\s*(\d+)u?;',
    'src/runtime/turbojs_runtime_scaffold.c': r'#define\s+TJS_VERSION\s+(\d+)u',
    'src/runtime/turbojs_upstream_runtime.c': r'#define\s+VENOM_TJS_RUNTIME_PACKAGE_VERSION\s+(\d+)u',
    'tools/build_emscripten.py': r'PACKAGE_VERSION\s*=\s*(\d+)',
    'tools/turbojs_wasm_cutover.py': r'PACKAGE_VERSION\s*=\s*(\d+)',
    'tools/turbojs_wasm_preflight.py': r'PACKAGE_VERSION\s*=\s*(\d+)',
