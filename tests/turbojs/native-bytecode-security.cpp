...
int main() {
  ...
  const auto record = venom::turbojs::compile_native_turbojs_bytecode(
      source, "native-bytecode-security.js", false);

  if (!venom::turbojs::is_native_turbojs_bytecode(record)) fail("VTJSBC03 magic missing");
  if (contains(record, "VTJSBC01") || contains(record, "VTJSBC02")) fail("legacy record marker present");
  ...
  if (!runtime || !context) fail("could not create TurboJS runtime/context");
  ...
  const auto module_record = venom::turbojs::compile_native_turbojs_bytecode(
  ...
  std::cout << "native VTJSBC03 compile/read/execute, source stripping, and module request redaction checks: PASS\n";
  return 0;
}
