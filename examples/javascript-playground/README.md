...
A browser-based JavaScript compiler and execution playground powered directly by Venom's embedded TurboJS/WASM source-execution ABI.
...
The browser submits source through Venom's protected compiler bridge; TurboJS parses and executes it in WASM and returns engine records, console capture, evaluation data, timing, or an exception. Browser `eval` and `Function` are never used.
...
- TurboJS/WASM performs source parsing and execution.
...
Every submitted script runs in a disposable TurboJS/WASM context. The default local-development policy limits source to 256 KiB, JSON input to 64 KiB, heap to 8 MiB, stack to 256 KiB, pending jobs to 128, console capture to 128 events / 128 KiB, and returned data to 256 KiB. An interrupt budget terminates runaway execution, and the context is destroyed after every run.
