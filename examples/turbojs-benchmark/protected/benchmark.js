...
function strings(iterations, seed) {
  let checksum = seed | 0;
  const words = ["venom", "turbojs", "wasm", "runtime", "protected", "benchmark"];
  for (let i = 0; i < iterations; i++) {
...
export function runBenchmarkCase(input) {
  ...
  if (!benchmark) throw new Error(`Unknown TurboJS benchmark case: ${name}`);
  ...
  return {
    ...
    engine: "TurboJS/WASM protected runtime"
  };
}
