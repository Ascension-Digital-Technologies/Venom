# QuickJS Long-Run Benchmark

This example provides a one-click benchmark dashboard for Venom's protected QuickJS/WASM runtime. It executes a sequence of independent workloads inside protected engine code and updates the browser after every completed subsystem.
...
The displayed duration includes the protected browser bridge overhead as well as QuickJS execution. The checksum column prevents the benchmark body from becoming dead work and helps detect output drift between builds.
...
```bat
scripts\windows\build-and-launch-quickjs-benchmark.bat
```
