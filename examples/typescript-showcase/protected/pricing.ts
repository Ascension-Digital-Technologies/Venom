...
import type { PricingRequest, PricingResult, RuntimeIdentity, Tier } from "./types";
...
function tierRate(tier: Tier): number {
  if (tier === "enterprise") return 0.18;
  if (tier === "pro") return 0.10;
  return 0;
}

function couponRate(code: string | undefined): number {
  const normalized = String(code || "").trim().toUpperCase();
  if (normalized === "VENOM20") return 0.20;
  if (normalized === "TYPESCRIPT10") return 0.10;
  return 0;
}

function calculateRisk(total: number, quantity: number): number {
