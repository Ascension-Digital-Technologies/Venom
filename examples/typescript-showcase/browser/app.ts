// @venom: browser
import type { PricingRequest, PricingResult } from "../protected/types";
...
function render(result: PricingResult): void {
  if (!output) return;
  output.innerHTML = `
...
async function submitQuote(event?: Event): Promise<void> {
  ...
  const request: PricingRequest = {
    ...
    customerTier: String(data.get("customerTier") || "standard") as PricingRequest["customerTier"],
