// @venom: browser
import type { Activity, Asset, Incident, Snapshot } from "./types";
...
function seeded(index:number, salt:number):number { const value = Math.sin(index * 91.17 + salt * 17.31) * 10000; return value - Math.floor(value); }
export function createSnapshot(): Snapshot {
  ...
  const assets = [];
  for (let index=0; index<38; index++) assets.push({ id:`AST-${String(2100+index)}`, name:`${services[index%services.length]} ${index+1}`, category:["Application","Database","Gateway","Worker","Storage"][index%5], region:regions[index%regions.length], utilization:Math.round(28+seeded(index,17)*69), health:Math.round(61+seeded(index,19)*38), cost:Math.round(1400+seeded(index,23)*28600), alerts:Math.round(seeded(index,29)*8) });
  const activity = [];
  for (let index=0; index<24; index++) activity.push({ id:`ACT-${index}`, time:`${String(9+Math.floor(index/6)).padStart(2,"0")}:${String((index*11)%60).padStart(2,"0")}`, actor:owners[index%owners.length], action:["acknowledged","assigned","contained","reviewed","escalated","resolved"][index%6], target:incidents[index].id, tone:index%6===5?"success":index%4===0?"warning":"neutral" });
