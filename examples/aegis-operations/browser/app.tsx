...
function Incidents(){ const regions=["All regions",...Array.from(new Set(state.snapshot.incidents.map(item=>item.region)))]; const incidents=visibleIncidents(); return <><section className="toolbar panel"><div><strong>{incidents.length} prioritized incidents</strong><small>Scored by protected QuickJS/WASM analytics</small></div><div className="toolbar-controls"><select onChange={(event)=>{state.region=event.target.value;draw();}}>{regions.map(region=><option selected={region===state.region}>{region}</option>)}</select><button className="secondary-button" onClick={()=>setToast("Incident export prepared")}>Export table</button></div></section><IncidentTable incidents={incidents} limit={64} onSelect={id=>{state.selectedIncident=id;draw();}}/></>; }
