...
std::vector<std::string> inspect_exports(const std::vector<unsigned char>& bytes) {
  if (bytes.size() < 8u || bytes[0] != 0x00u || bytes[1] != 0x61u || bytes[2] != 0x73u || bytes[3] != 0x6du) {
    raise_error("VENOM-E5000", "embedded TurboJS runtime is not a WebAssembly module");
  }
  std::size_t pos = 8u;
...
void verify_embedded_turbojs_wasm_contract() {
  const auto exports = inspect_exports(make_turbojs_runtime_wasm_module());
  std::vector<std::string> missing;
  for (const auto required : generated::turbojs_wasm_abi::required_exports) {
    if (std::find(exports.begin(), exports.end(), required) == exports.end()) missing.emplace_back(required);
  }
  if (!missing.empty()) {
    std::ostringstream message;
    message << "embedded TurboJS/WASM ABI mismatch; missing required exports:";
    for (const auto& name : missing) message << ' ' << name;
    message << "; regenerate the runtime from contracts/turbojs-wasm-abi.json";
