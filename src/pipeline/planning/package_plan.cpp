...
Artifact Plan::write_verified(const std::filesystem::path& path,
  ...
  if (sections_.empty()) raise_error("VENOM-E5000", "cannot write an empty package plan");
  ...
  if (!input) raise_error("VENOM-E5000", "failed to reopen package artifact: " + path.string());
  ...
  if (artifact.package.sections.empty()) {
    raise_error("VENOM-E5000", "package writer produced no readable sections");
  }
  return artifact;
}
