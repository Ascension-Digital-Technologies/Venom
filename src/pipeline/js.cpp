...
std::string make_protected_client_javascript(
  ...
  out << "// Generated by Venom. Do not edit.\n"
      << "import { callProtected } from '@venom/runtime';\n";
...
std::string make_protected_client_typescript(const std::string& declarations) {
  ...
  std::ostringstream out;
  out << "// Generated by Venom. Do not edit.\n"
      << "import type { VenomCallOptions } from '@venom/runtime';\n";
  std::string line;
...
std::string make_protection_intent_ledger_json(
  ...
  for (const auto& chunk : chunks) {
    ...
    out << "    {\"intent_id\":\"" << id
        ...
        << id << "\",\"reason\":\"protected script chunk compiled to TurboJS bytecode\"}";
  }
  if (emitted != 0u) out << '\n';
  out << "  ]\n}\n";
  return out.str();
...
void enforce_protection_intent_closure(const std::vector<detail::BridgeRewriteRecord>& records,
  ...
  if (resolved != 0 && !registry_present)
    raise_error("VENOM-E5000", "VNM-PROT-1005: protected intents resolved without a TurboJS registry record");
}
...
JsBridge compile_js_bridge(const SiteGraph& graph, const RemoteVendorOptions& remote_options, bool development, bool harden_public_javascript) {
  JsBridge bridge;
  ...
  if (!rewrite_result.registry_source.empty()) {
    bridge.bridge_registry_bytecode = venom::turbojs::compile_native_turbojs_bytecode(
    ...
    const bool native_record = bridge.bridge_registry_bytecode.size() >= 8u &&
        (std::equal(bridge.bridge_registry_bytecode.begin(), bridge.bridge_registry_bytecode.begin() + 8,
                    reinterpret_cast<const unsigned char*>("VTJSBC03")) ||
         std::equal(bridge.bridge_registry_bytecode.begin(), bridge.bridge_registry_bytecode.begin() + 8,
                    reinterpret_cast<const unsigned char*>("VTJSMB04")));
    if (!native_record) {
      raise_error("VENOM-E5000", "protected bridge extraction did not produce a native TurboJS bytecode record");
    }
  }
  for (const auto& source_chunk : rewrite_result.registry_chunks) {
    ...
    compiled_chunk.bytecode = venom::turbojs::compile_native_turbojs_bytecode(
  ...
  if (!bridge.protected_exports.empty() && bridge.bridge_registry_bytecode.empty()) {
    raise_error("VENOM-E5000", "protected exports exist but the protected TurboJS bridge registry is empty");
  }
  ...
  bridge.expected_turbojs_records = bridge.protected_whole_file_intents + (!bridge.bridge_registry_bytecode.empty() ? 1u : 0u);
  ...
  bridge.bundle = encode_js_bundle(bridge.chunks, bridge.module_graph, remote_options.bridge_id_salt, development, harden_public_javascript);
...
std::vector<unsigned char> encode_protected_bridge_registry(const JsBridge& bridge, const std::string& diversification_salt) {
  ...
  synthetic.kind = 0u;
  return wrap_turbojs_record_for_bundle(bridge.bridge_registry_bytecode, diversification_salt, synthetic);
}
...
std::string make_loader_js(const std::string& runtime_asset_name, const std::string& package_asset_name, std::uint64_t expected_package_hash, const std::string& expected_package_sha256, bool hardened, const std::string& turbojs_engine_asset_name, const std::string& turbojs_runtime_wasm_asset_name, const std::string& runtime_wasm_asset_name, const std::string& style_asset_name, const std::string& package_binding_token, const std::string& worker_asset_name, const std::string& turbojs_runtime_wasm_sha256, const std::vector<std::pair<std::string, std::string>>& protected_exports,
  ...
  if (!turbojs_engine_asset_name.empty()) out << "  turboJsEngineUrl: new URL('" << turbojs_engine_asset_name << "', import.meta.url).href,\n";
  if (!turbojs_runtime_wasm_asset_name.empty()) out << "  turboJsWasmUrl: new URL('" << turbojs_runtime_wasm_asset_name << "', import.meta.url).href,\n";
  if (!turbojs_runtime_wasm_sha256.empty()) out << "  expectedTurboJsWasmSha256: '" << turbojs_runtime_wasm_sha256 << "',\n";
  ...
  if (!worker_asset_name.empty()) {
    ...
    out << "let __venomReadyResolve,__venomReadyReject; const __venomReadyPromise=new Promise((resolve,reject)=>{__venomReadyResolve=resolve;__venomReadyReject=reject;});\n"
        ...
        << "    worker.onmessage = (event) => { const m = event.data || {}; if (m.nonce !== nonce) return; clearTimeout(timer); if (m.protocol !== 1) return reject(new Error('worker protocol version mismatch')); if (m.type !== 'ready') return reject(new Error(m.message || 'worker runtime preparation failed')); if (m.packageHash !== bootOptions.expectedPackageHash) return reject(new Error('worker package hash attestation mismatch')); if (m.packageSha256 !== bootOptions.expectedPackageSha256) return reject(new Error('worker package SHA-256 attestation mismatch')); if (m.turboJsWasmSha256 !== bootOptions.expectedTurboJsWasmSha256) return reject(new Error('worker TurboJS WASM attestation mismatch')); if (m.turboJsRuntimeReady !== true) return reject(new Error('worker TurboJS WASM readiness attestation missing')); resolve(m); };\n"
        << "    worker.onerror = (event) => { clearTimeout(timer); reject(new Error(event.message || 'worker startup failed')); };\n"
        << "    worker.postMessage({ protocol: 1, type: 'prepare', nonce, packageUrl: bootOptions.packageUrl, turboJsWasmUrl: bootOptions.turboJsWasmUrl, expectedPackageHash: bootOptions.expectedPackageHash, expectedPackageSha256: bootOptions.expectedPackageSha256, expectedTurboJsWasmSha256: bootOptions.expectedTurboJsWasmSha256, maxPackageBytes: 67108864, maxWasmBytes: 33554432 }, [bridgeChannel.port2]);\n"
        << "  });\n"
        << "  bootOptions.packageBytes = prepared.packageBytes;\n"
        << "  if (prepared.turboJsModule) globalThis.__venomWorkerTurboJsModule = prepared.turboJsModule;\n"
