...
std::vector<ProtectedContractField> parse_contract_fields(const std::string& text,
  ...
  while (std::getline(stream, item, ',')) {
    ...
    if (colon == std::string::npos)
      raise_error("VENOM-E5000", "VENOM-E2401 malformed " + direction + " contract in " + source + ": " + item);
    ...
    if (!std::regex_match(name, ident) || valid.find(type) == valid.end())
      raise_error("VENOM-E5000", "VENOM-E2402 unsupported " + direction + " contract field in " + source + ": " + item);
    if (!names.insert(name).second)
      raise_error("VENOM-E5000", "VENOM-E2403 duplicate " + direction + " contract field in " + source + ": " + name);
...
ProtectedModuleSyntax parse_protected_module_syntax(const JsChunk& chunk) {
  ...
  for (const auto& reference : parsed.module_references) {
    if (reference.dynamic) {
      raise_error("VENOM-E5000", "VENOM-E2205 dynamic import is not supported in protected module: " +
  ...
  for (const auto& imported : parsed.module_imports) {
    if (imported.side_effect_only) {
      raise_error("VENOM-E5000", "VENOM-E2206 side-effect-only imports are not supported in protected module: " +
                               chunk.source + ":" + std::to_string(imported.line) + ":" +
                               std::to_string(imported.column + 1));
    }
    if (imported.has_default) {
      raise_error("VENOM-E5000", "VENOM-E2215 default imports are not supported in protected modules: " +
                               chunk.source + ":" + std::to_string(imported.line) + ":" +
                               std::to_string(imported.column + 1));
    }
...
std::string resolve_protected_module_specifier(const std::string& referrer, const std::string& specifier,
  ...
  if (specifier.empty() || specifier.front() != '.')
    raise_error("VENOM-E5000", "VENOM-E2211 protected module imports must be relative: " + referrer + " -> " + specifier);
  ...
  raise_error("VENOM-E5000", "VENOM-E2212 protected module dependency not found: " + referrer + " -> " + specifier);
}
...
std::string import_clause_to_bindings(const ProtectedModuleImport& import, const std::string& dependency_id,
  ...
  if (clause.rfind("*", 0) == 0) {
    ...
    if (!std::regex_match(clause, match, ns))
      raise_error("VENOM-E5000", "VENOM-E2213 malformed namespace import in " + source);
  ...
  if (!clause.empty() && clause.front() == '{' && clause.back() == '}') {
    ...
    while (std::getline(items, item, ',')) {
      ...
      if (!std::regex_match(item, match, named))
        raise_error("VENOM-E5000", "VENOM-E2214 malformed named import in " + source + ": " + item);
  ...
  raise_error("VENOM-E5000", "VENOM-E2215 default imports are not supported in protected modules: " + source);
}
...
ProtectedModuleRewriteResult apply_protected_module_rewrites(std::vector<JsChunk>& chunks,
  ...
  for (std::size_t i = 0; i < chunks.size(); ++i) {
    ...
    if ((chunk.flags & JsChunkBrowser) != 0u)
      raise_error("VENOM-E5000", "protected module cannot be browser-side: " + chunk.source);
  ...
  for (auto& module : modules) {
    if (!module.syntax.unsupported_exports.empty()) {
      ...
      raise_error("VENOM-E5000", "VENOM-E2202 protected modules currently export named function declarations only: " +
                               module.source + " (" + details.str() + ")");
    }
    if (module.syntax.exports.empty())
      raise_error("VENOM-E5000", "VENOM-E2203 protected module has no named function exports: " + module.source);
  ...
  std::function<void(std::size_t)> visit = [&](std::size_t index) {
    ...
    if (state[index] == 1)
      raise_error("VENOM-E5000", "VENOM-E2216 protected module import cycle detected at " + modules[index].source);
    state[index] = 1;
  ...
  for (const auto index : order) {
    ...
    for (const auto& entry : module.syntax.exports) {
      ...
      if (!public_names.insert(entry.name).second)
        raise_error("VENOM-E5000", "VENOM-E2204 duplicate public protected export name: " + entry.name +
  ...
  for (auto& importer_chunk : chunks) {
    ...
    for (const auto& imported : parsed.module_imports) {
      ...
      if (imported.side_effect_only || imported.has_default) {
        raise_error("VENOM-E5000", "VENOM-E2217 browser imports from protected modules must use named imports: " +
                                 importer_chunk.source + " -> " + imported.specifier);
      }
      const auto clause = trim_ascii(imported.clause);
      if (clause.empty() || clause.front() != '{' || clause.back() != '}') {
        raise_error("VENOM-E5000", "VENOM-E2218 browser imports from protected modules must use named imports: " +
                                 importer_chunk.source + " -> " + imported.specifier);
      }
      ...
      while (std::getline(items, item, ',')) {
        ...
        std::smatch match;
        if (!std::regex_match(item, match, named)) {
          raise_error("VENOM-E5000", "VENOM-E2219 malformed protected browser import in " +
        ...
        if (target_it == public_export_candidates.end() || candidate_it == target_it->second.end()) {
          raise_error("VENOM-E5000", "VENOM-E2220 protected module does not export " + exported + ": " + target);
        }
...
BridgeRewriteResult apply_bridge_rewrites(std::vector<JsChunk>& chunks,
  ...
  for (const auto& record : extraction_records) {
    ...
    catch (const std::regex_error& error) { raise_error("VENOM-E5000", std::string("dependency declaration regex: ") + error.what()); }
    ...
    catch (const std::regex_error& error) { raise_error("VENOM-E5000", std::string("call-site regex: ") + error.what()); }
    if (!calls_awaited) {
    ...
    if (it->code.find(extracted.declaration) != std::string::npos) {
      raise_error("VENOM-E5000", "protected function source remained in browser chunk after extraction: " + record.source + "::" + record.name);
    }
    ...
    catch (const std::regex_error& error) { raise_error("VENOM-E5000", std::string("registry declaration regex: ") + error.what()); }
