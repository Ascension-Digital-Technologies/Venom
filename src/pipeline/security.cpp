...
bool generate_key_file(const KeygenOptions& options) {
  ...
  if (std::filesystem::exists(options.output) && !options.force) {
    raise_error("VENOM-E5000", "key file already exists; pass --force to overwrite: " + options.output.string());
  }
  ...
  if (!out) {
    raise_error("VENOM-E5000", "failed to write key file " + options.output.string());
  }
  out << text;
  if (!out) {
    raise_error("VENOM-E5000", "failed while writing key file " + options.output.string());
  }
