...
std::string read_file(const std::filesystem::path& path) {
  ...
  if (!in) raise_error("VENOM-E5000", "unable to read compatibility input: " + path.string());
...
CapabilityGraph analyze_capabilities(const std::filesystem::path& input) {
  if (!std::filesystem::exists(input)) raise_error("VENOM-E5000", "compatibility input does not exist: " + input.string());
