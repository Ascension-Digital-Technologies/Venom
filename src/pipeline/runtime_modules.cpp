...
std::size_t find_function_open(const std::string& source, const std::string& name, std::size_t& start) {
  ...
  if (start == std::string::npos) raise_error("VENOM-E5000", "runtime module specialization could not find function: " + name);
  const auto params = source.find('(', start);
  if (params == std::string::npos) raise_error("VENOM-E5000", "runtime module specialization found malformed parameters: " + name);
  std::size_t depth = 0;
  char quote = 0;
  bool escape = false;
  for (std::size_t i = params; i < source.size(); ++i) {
    ...
    if (c == '(') ++depth;
      ...
      if (depth == 0) raise_error("VENOM-E5000", "runtime module specialization parameter underflow: " + name);
      --depth;
      if (depth == 0) {
        const auto open = source.find('{', i + 1);
        if (open == std::string::npos) raise_error("VENOM-E5000", "runtime module specialization found malformed function body: " + name);
        return open;
      }
    }
  }
  raise_error("VENOM-E5000", "runtime module specialization found unterminated parameters: " + name);
}
...
std::size_t find_matching_brace(const std::string& source, std::size_t open) {
  ...
  for (std::size_t i = open; i < source.size(); ++i) {
    ...
    if (c == '{') ++depth;
      ...
      if (depth == 0) raise_error("VENOM-E5000", "runtime module specialization brace underflow");
      --depth;
      if (depth == 0) return i;
    }
  }
  raise_error("VENOM-E5000", "runtime module specialization found unterminated function");
}
...
RuntimeModulePlan plan_runtime_modules(const SiteGraph& graph, const CapabilityGraph& c, const BuildOptions& options) {
  ...
  auto apply_policy = [](const char* name, const std::string& mode, bool needed) {
    if (mode == "deny" && needed) raise_error("VENOM-E5000", std::string("capability '") + name + "' is required by the project but denied in venom.toml");
