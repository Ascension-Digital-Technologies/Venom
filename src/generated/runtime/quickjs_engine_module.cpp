...
std::string make_quickjs_engine_module_js(bool protected_release) {
  std::string js = R"QJSENGINE(export function createVenomQuickJsEngineModule(config = {}) {
  ...
  function memoryView(instance) {
    ...
    if (!memory || !(memory.buffer instanceof ArrayBuffer)) throw new Error('QuickJS backend memory export is invalid');
    return new Uint8Array(memory.buffer);
  }

  function memoryRange(instance, ptr, size, label = 'QuickJS') {
    const start = ptr >>> 0;
    const length = size >>> 0;
    const memory = memoryView(instance);
    if (!start || !length || start + length < start || start + length > memory.byteLength) {
      throw new Error(label + ' memory range is invalid');
    }
    return memory.subarray(start, start + length);
  }

  function readWasmBytes(instance, ptr, size, label = 'QuickJS') {
    return memoryRange(instance, ptr, size, label).slice();
  }

  function zeroWasmRange(instance, ptr, size) {
    if (!ptr || !size) return;
    try { memoryRange(instance, ptr, size, 'QuickJS zeroize').fill(0); } catch (_) {}
  }

  function readWasmString(instance, ptr, size) {
    if (!ptr || !size) return '';
    const copy = readWasmBytes(instance, ptr, size, 'QuickJS string');
  ...
  function validateBytecodeTrustHandoff(chunk) {
    ...
    if ((handoff.byteLength >>> 0) !== (bytes.length >>> 0) || (handoff.byteHash >>> 0) !== byteHash || (handoff.bindingHash >>> 0) !== bindingHash) {
      throw new Error('QuickJS trust-domain handoff integrity mismatch');
    }
    return true;
  }

  function decodeQuickJsRecordForDeniedFallback(bytes, sourceName) {
    void bytes;
    throw new Error('native QuickJS object bytecode cannot be converted back to host source: ' + (sourceName || '<script>'));
  }

  function hasVqjsbc03Magic(bytes) {
  ...
  function decodeHostBridgeSource(chunk) {
    ...
    if ((!source || !source.trim()) && rawBytes) {
      if (hasVqjsbc03Magic(rawBytes)) {
        source = '';
        sourceKind = 'opaque-vqjsbc03-native-object';
      } else {
        try {
          source = decodeQuickJsRecordForDeniedFallback(rawBytes, chunk && chunk.source);
        } catch (_) {
          source = '';
  ...
  function replayWasmHostBridgeEffects(chunk, telemetry, bindings) {
    ...
    return Object.freeze({
      ...
      boundary: telemetry && telemetry.boundary ? telemetry.boundary : 'upstream-quickjs-wasm-host-call-bridge'
    });
  }
  ...
  function inferWasmHostBridgeTelemetry(chunk, wasmResult) {
    ...
    const wasmAccepted = !!(wasmResult && wasmResult.quickJsWasm);
    const upstreamReady = !!(wasmResult && wasmResult.upstreamQuickJsReady);
    return Object.freeze({
      version: 1,
      boundary: 'upstream-quickjs-wasm-host-call-bridge',
      sourceKind,
  ...
  function readAbiTableFromExports(instance) {
    ...
    if (!e.venom_qjs_abi_table_ptr || !e.venom_qjs_abi_table_size) return null;
    return Object.freeze({ abi: e.venom_qjs_engine_abi ? e.venom_qjs_engine_abi() >>> 0 : 0, packageVersion: e.venom_qjs_engine_version ? e.venom_qjs_engine_version() >>> 0 : 0, entryCount: e.venom_qjs_abi_entry_count ? e.venom_qjs_abi_entry_count() >>> 0 : 0, tableHash: e.venom_qjs_abi_table_hash ? String(e.venom_qjs_abi_table_hash() >>> 0) : '', table: readWasmString(instance, e.venom_qjs_abi_table_ptr() >>> 0, e.venom_qjs_abi_table_size() >>> 0) });
  }

  function readHostImportTableFromExports(instance) {
    const e = instance && instance.exports ? instance.exports : {};
    if (!e.venom_qjs_host_import_table_ptr || !e.venom_qjs_host_import_table_size) return null;
    return Object.freeze({ importCount: e.venom_qjs_host_import_count ? e.venom_qjs_host_import_count() >>> 0 : 0, tableHash: e.venom_qjs_host_import_table_hash ? String(e.venom_qjs_host_import_table_hash() >>> 0) : '', table: readWasmString(instance, e.venom_qjs_host_import_table_ptr() >>> 0, e.venom_qjs_host_import_table_size() >>> 0), design: 'quickjs-host-call-import-table-v3' });
  }

  function readBytecodeManifestFromExports(instance) {
    const e = instance && instance.exports ? instance.exports : {};
    if (!e.venom_qjs_bytecode_manifest_ptr || !e.venom_qjs_bytecode_manifest_size) return null;
    return Object.freeze({ hash: e.venom_qjs_bytecode_manifest_hash ? String(e.venom_qjs_bytecode_manifest_hash() >>> 0) : '', text: readWasmString(instance, e.venom_qjs_bytecode_manifest_ptr() >>> 0, e.venom_qjs_bytecode_manifest_size() >>> 0), loaded: true });
  }

  function readHostTrapPolicyFromExports(instance) {
    const e = instance && instance.exports ? instance.exports : {};
    if (!e.venom_qjs_host_trap_policy_ptr || !e.venom_qjs_host_trap_policy_size) return null;
    return Object.freeze({ hash: e.venom_qjs_host_trap_policy_hash ? String(e.venom_qjs_host_trap_policy_hash() >>> 0) : '', text: readWasmString(instance, e.venom_qjs_host_trap_policy_ptr() >>> 0, e.venom_qjs_host_trap_policy_size() >>> 0), loaded: true });
  }


  function readModuleGraphFromExports(instance) {
    const e = instance && instance.exports ? instance.exports : {};
    if (!e.venom_qjs_module_graph_ptr || !e.venom_qjs_module_graph_size) return null;
    return Object.freeze({ hash: e.venom_qjs_module_graph_hash ? String(e.venom_qjs_module_graph_hash() >>> 0) : '', text: readWasmString(instance, e.venom_qjs_module_graph_ptr() >>> 0, e.venom_qjs_module_graph_size() >>> 0), loaded: true });
  }
  ...
  async function loadWasm() {
    ...
    if (!providedWasmModule && !wasmUrl) { wasmError = new Error('QuickJS WASM runtime URL is missing'); return null; }
    if (!providedWasmModule && typeof fetch !== 'function') { wasmError = new Error('fetch is unavailable for QuickJS WASM loading'); return null; }
    ...
    if (!wasmPromise) {
      wasmPromise = (providedWasmModule
        ...
        : fetch(wasmUrl, { cache: 'force-cache' }).then((response) => {
            if (!response.ok) throw new Error('failed to load QuickJS WASM backend: ' + response.status);
            return response.arrayBuffer().then((bytes) => WebAssembly.compile(bytes));
          }))
        .then((module) => {
          if (protectedRelease) {
            if (!config.abiFingerprint) throw new Error('protected release lacks QuickJS ABI fingerprint');
            const canonicalNames = validateReleaseAbiExports(module);
            const actual = releaseAbiHash(canonicalNames);
            if (actual !== (config.abiFingerprint.hash >>> 0)) throw new Error('QuickJS package/runtime ABI fingerprint mismatch');
          }
        ...
        .then((result) => {
          ...
          cachedCheckpointPolicy = readTextRecordFromExports(instance, 'venom_qjs_checkpoint_policy_ptr', 'venom_qjs_checkpoint_policy_size', 'venom_qjs_checkpoint_policy_hash');
          ...
          cachedDeterminismAudit = readTextRecordFromExports(instance, 'venom_qjs_determinism_audit_ptr', 'venom_qjs_determinism_audit_size', 'venom_qjs_determinism_audit_hash');
          cachedSnapshotPolicy = readTextRecordFromExports(instance, 'venom_qjs_snapshot_policy_ptr', 'venom_qjs_snapshot_policy_size', 'venom_qjs_snapshot_policy_hash');
          cachedSnapshotRecord = readTextRecordFromExports(instance, 'venom_qjs_snapshot_record_ptr', 'venom_qjs_snapshot_record_size', 'venom_qjs_snapshot_record_hash');
          cachedReplayValidation = readTextRecordFromExports(instance, 'venom_qjs_replay_validation_ptr', 'venom_qjs_replay_validation_size', 'venom_qjs_replay_validation_hash');
          cachedDeterminismLedger = readTextRecordFromExports(instance, 'venom_qjs_determinism_ledger_ptr', 'venom_qjs_determinism_ledger_size', 'venom_qjs_determinism_ledger_hash');
          cachedAuditSeal = readTextRecordFromExports(instance, 'venom_qjs_audit_seal_ptr', 'venom_qjs_audit_seal_size', 'venom_qjs_audit_seal_hash');
    cachedExecutionCommit = readTextRecordFromExports(instance, 'venom_qjs_execution_commit_ptr', 'venom_qjs_execution_commit_size', 'venom_qjs_execution_commit_hash');
    cachedRollbackPolicy = readTextRecordFromExports(instance, 'venom_qjs_rollback_policy_ptr', 'venom_qjs_rollback_policy_size', 'venom_qjs_rollback_policy_hash');
    cachedHostReceipts = readTextRecordFromExports(instance, 'venom_qjs_host_receipts_ptr', 'venom_qjs_host_receipts_size', 'venom_qjs_host_receipts_hash');
    cachedReleaseAcceptance = readTextRecordFromExports(instance, 'venom_qjs_release_acceptance_ptr', 'venom_qjs_release_acceptance_size', 'venom_qjs_release_acceptance_hash');
    cachedCommitAudit = readTextRecordFromExports(instance, 'venom_qjs_commit_audit_ptr', 'venom_qjs_commit_audit_size', 'venom_qjs_commit_audit_hash');
          cachedCapabilityPolicy = readTextRecordFromExports(instance, 'venom_qjs_capability_policy_ptr', 'venom_qjs_capability_policy_size', 'venom_qjs_capability_policy_hash');
          cachedHostIoPolicy = readTextRecordFromExports(instance, 'venom_qjs_host_io_policy_ptr', 'venom_qjs_host_io_policy_size', 'venom_qjs_host_io_policy_hash');
          cachedPermissionSeal = readTextRecordFromExports(instance, 'venom_qjs_permission_seal_ptr', 'venom_qjs_permission_seal_size', 'venom_qjs_permission_seal_hash');
          cachedPolicyReceipts = readTextRecordFromExports(instance, 'venom_qjs_policy_receipts_ptr', 'venom_qjs_policy_receipts_size', 'venom_qjs_policy_receipts_hash');
          cachedReleaseGate = readTextRecordFromExports(instance, 'venom_qjs_release_gate_ptr', 'venom_qjs_release_gate_size', 'venom_qjs_release_gate_hash');
          cachedHostIoDecision = readTextRecordFromExports(instance, 'venom_qjs_host_io_decision_ptr', 'venom_qjs_host_io_decision_size', 'venom_qjs_host_io_decision_hash');
          cachedHostIoDenyTrace = readTextRecordFromExports(instance, 'venom_qjs_host_io_deny_trace_ptr', 'venom_qjs_host_io_deny_trace_size', 'venom_qjs_host_io_deny_trace_hash');
          cachedCapabilityLedger = readTextRecordFromExports(instance, 'venom_qjs_capability_ledger_ptr', 'venom_qjs_capability_ledger_size', 'venom_qjs_capability_ledger_hash');
          cachedPolicySealAudit = readTextRecordFromExports(instance, 'venom_qjs_policy_seal_audit_ptr', 'venom_qjs_policy_seal_audit_size', 'venom_qjs_policy_seal_audit_hash');
          cachedRuntimeDenylist = readTextRecordFromExports(instance, 'venom_qjs_runtime_denylist_ptr', 'venom_qjs_runtime_denylist_size', 'venom_qjs_runtime_denylist_hash');
          cachedRuntimeLimits = readTextRecordFromExports(instance, 'venom_qjs_runtime_limits_ptr', 'venom_qjs_runtime_limits_size', 'venom_qjs_runtime_limits_hash');
          cachedBackendContract = readTextRecordFromExports(instance, 'venom_qjs_backend_contract_ptr', 'venom_qjs_backend_contract_size', 'venom_qjs_backend_contract_hash');
          cachedInterpreterContract = readTextRecordFromExports(instance, 'venom_qjs_interpreter_contract_ptr', 'venom_qjs_interpreter_contract_size', 'venom_qjs_interpreter_contract_hash');
          cachedSemanticRecord = readTextRecordFromExports(instance, 'venom_qjs_interpreter_semantic_record_ptr', 'venom_qjs_interpreter_semantic_record_size', 'venom_qjs_interpreter_semantic_record_hash');
          cachedGlobalSlotRecord = readTextRecordFromExports(instance, 'venom_qjs_global_slot_record_ptr', 'venom_qjs_global_slot_record_size', 'venom_qjs_global_slot_record_hash');
          cachedUpstreamParityRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_parity_record_ptr', 'venom_qjs_upstream_parity_record_size', 'venom_qjs_upstream_parity_record_hash');
          cachedUpstreamBytecodeSemanticsRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_bytecode_semantics_record_ptr', 'venom_qjs_upstream_bytecode_semantics_record_size', 'venom_qjs_upstream_bytecode_semantics_record_hash');
          cachedUpstreamLexicalScopeRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_lexical_scope_record_ptr', 'venom_qjs_upstream_lexical_scope_record_size', 'venom_qjs_upstream_lexical_scope_record_hash');
          cachedUpstreamClosureRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_closure_record_ptr', 'venom_qjs_upstream_closure_record_size', 'venom_qjs_upstream_closure_record_hash');
          cachedUpstreamIteratorRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_iterator_record_ptr', 'venom_qjs_upstream_iterator_record_size', 'venom_qjs_upstream_iterator_record_hash');
          cachedUpstreamIntrinsicRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_intrinsic_record_ptr', 'venom_qjs_upstream_intrinsic_record_size', 'venom_qjs_upstream_intrinsic_record_hash');
          cachedUpstreamPropertyDescriptorRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_property_descriptor_record_ptr', 'venom_qjs_upstream_property_descriptor_record_size', 'venom_qjs_upstream_property_descriptor_record_hash');
          cachedUpstreamPrototypeChainRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_prototype_chain_record_ptr', 'venom_qjs_upstream_prototype_chain_record_size', 'venom_qjs_upstream_prototype_chain_record_hash');
          cachedUpstreamCallFrameRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_call_frame_record_ptr', 'venom_qjs_upstream_call_frame_record_size', 'venom_qjs_upstream_call_frame_record_hash');
          const wasmProbe = e.venom_qjs_parity_probe ? e.venom_qjs_parity_probe() >>> 0 : 0;
        ...
        .catch((error) => {
          ...
          if (globalThis.console && typeof globalThis.console.error === 'function') {
            globalThis.console.error('[venom] QuickJS WASM initialization failed:', wasmFailureDetail());
          }
          return null;
        });
    }
  ...
  function destroyContext(contextOrKey = '') {
    ...
    if (record && record.wasmContext && wasmInstance && wasmInstance.exports && typeof wasmInstance.exports.venom_qjs_context_free === 'function') {
      wasmInstance.exports.venom_qjs_context_free(record.wasmContext >>> 0);
    }
  ...
  function status() {
    return Object.freeze({ kind: 'venom.quickjs.engine.module', version: moduleVersion, mode, fallback, wasmUrl, wasmLoaded: !!wasmInstance, wasmError: wasmError ? String(wasmError.message || wasmError) : '', contexts: contexts.size, executions: executions.length });
  }
  ...
  async function executeWithWasmScaffold(chunk, context) {
    ...
    if (protectedRelease && diversification && !diversificationInstalled) {
      ...
      const contractPtr = e.venom_qjs_script_buffer_alloc(mutableContext.wasmContext >>> 0, contract.byteLength >>> 0) >>> 0;
      const contractCap = e.venom_qjs_script_buffer_capacity(mutableContext.wasmContext >>> 0) >>> 0;
      if (!contractPtr || contract.byteLength > contractCap) throw new Error('QuickJS diversification buffer allocation failed');
      try {
        memoryView(instance).set(contract, contractPtr);
        if ((e.venom_qjs_diversification_install(contractPtr, contract.byteLength >>> 0) >>> 0) !== 1) throw new Error('QuickJS diversification contract rejected');
        diversificationInstalled = true;
      } finally {
        e.venom_qjs_script_buffer_free(mutableContext.wasmContext >>> 0, contractPtr >>> 0);
      }
    }
    let ptr = 0;
    try {
      ptr = e.venom_qjs_script_buffer_alloc(mutableContext.wasmContext >>> 0, requested) >>> 0;
    const cap = e.venom_qjs_script_buffer_capacity(mutableContext.wasmContext >>> 0) >>> 0;
    if (!ptr || requested > cap) {
      let statusText = '';
      if (typeof e.venom_qjs_status_record_ptr === 'function' && typeof e.venom_qjs_status_record_size === 'function') {
        const statusPtr = e.venom_qjs_status_record_ptr() >>> 0;
        const statusSize = e.venom_qjs_status_record_size() >>> 0;
        if (statusPtr && statusSize) statusText = decoder.decode(memoryView(instance).slice(statusPtr, statusPtr + statusSize));
      }
      throw new Error('QuickJS script byte buffer allocation failed: requested=' + requested + ' capacity=' + cap + (statusText ? ' status=' + statusText : ''));
    }
    const memory = memoryView(instance);
    memoryRange(instance, ptr, requested, 'QuickJS script input').set(bytes);
    const expectedHash = fnv1a32(bytes);
    if (typeof e.venom_qjs_script_buffer_set_expected_hash === 'function') e.venom_qjs_script_buffer_set_expected_hash(mutableContext.wasmContext >>> 0, expectedHash >>> 0);
    if (bytecodeMode && typeof e.venom_qjs_bytecode_validate === 'function') {
      const validationStatus = e.venom_qjs_bytecode_validate(mutableContext.wasmContext >>> 0, requested) >>> 0;
      if (validationStatus !== 0) {
        const statusPtr = typeof e.venom_qjs_status_record_ptr === 'function' ? e.venom_qjs_status_record_ptr() >>> 0 : 0;
        const statusSize = typeof e.venom_qjs_status_record_size === 'function' ? e.venom_qjs_status_record_size() >>> 0 : 0;
        const statusText = statusPtr && statusSize ? decoder.decode(memory.subarray(statusPtr, statusPtr + statusSize)) : '';
        throw new Error('QuickJS bytecode validation failed: code=' + validationStatus + ' requested=' + requested + ' capacity=' + cap + (statusText ? ' status=' + statusText : ''));
      }
    }
    const moduleFlag = ((chunk.flags >>> 0) & SCRIPT_FLAG_MODULE) !== 0 || (!protectedRelease && /(^|\n)\s*(import|export)\s+/m.test(String(chunk.code || '')));
    const ok = bytecodeMode && typeof e.venom_qjs_execute_bytecode === 'function'
      ? (e.venom_qjs_execute_bytecode(mutableContext.wasmContext, requested) >>> 0)
      : ((moduleFlag && typeof e.venom_qjs_module_execute === 'function')
        ? (e.venom_qjs_module_execute(mutableContext.wasmContext, (chunk.order >>> 0) || 1, requested) >>> 0)
        : (e.venom_qjs_execute_source(mutableContext.wasmContext, requested) >>> 0));
    if (protectedRelease) {
      const compactPtr = typeof e.venom_qjs_compact_result_ptr === 'function' ? e.venom_qjs_compact_result_ptr() >>> 0 : 0;
      let abiStatus = 0;
      let exceptionCode = 0;
      let hostCallCount = 0;
      let consoleCount = 0;
      ...
      if (compactPtr) {
        ...
        if (field(0) !== 0x33524356 && field(0) !== 0x32524356) throw new Error('QuickJS compact result descriptor is invalid');
        abiStatus = field(1);
        ...
        throw new Error('QuickJS compact result descriptor is required');
      }
      let exceptionMessage = '';
      if (!executed && typeof e.venom_qjs_exception_message_ptr === 'function' && typeof e.venom_qjs_exception_message_size === 'function') {
      ...
      const hostBridgeTelemetry = inferWasmHostBridgeTelemetry(chunk, { quickJsWasm: true, report: null, hostCallCount, consoleCount, fallbackRequired, upstreamQuickJsReady: true });
      return Object.freeze({
        executed,
        quickJsWasm: true,
    ...
    if (typeof e.venom_qjs_execution_record_ptr === 'function' && typeof e.venom_qjs_execution_record_size === 'function') {
      ...
      const recSize = e.venom_qjs_execution_record_size() >>> 0;
      if (recSize > 0) {
    ...
    if (typeof e.venom_qjs_console_event_ptr === 'function' && typeof e.venom_qjs_console_event_size === 'function') {
      const eventPtr = e.venom_qjs_console_event_ptr() >>> 0;
      const eventSize = e.venom_qjs_console_event_size() >>> 0;
    ...
    const heapUsed = typeof e.venom_qjs_context_heap_used === 'function' ? e.venom_qjs_context_heap_used(mutableContext.wasmContext) >>> 0 : 0;
    const contextHeapLimit = typeof e.venom_qjs_context_heap_limit === 'function' ? e.venom_qjs_context_heap_limit(mutableContext.wasmContext) >>> 0 : 0;
    const contextStackLimit = typeof e.venom_qjs_context_stack_limit === 'function' ? e.venom_qjs_context_stack_limit(mutableContext.wasmContext) >>> 0 : 0;
    let exceptionRecord = null;
    if (typeof e.venom_qjs_exception_ptr === 'function' && typeof e.venom_qjs_exception_size === 'function') {
      const exceptionPtr = e.venom_qjs_exception_ptr() >>> 0;
      const exceptionSize = e.venom_qjs_exception_size() >>> 0;
    ...
    if (typeof e.venom_qjs_module_record_ptr === 'function' && typeof e.venom_qjs_module_record_size === 'function') {
      const modulePtr = e.venom_qjs_module_record_ptr() >>> 0;
      const moduleSize = e.venom_qjs_module_record_size() >>> 0;
    ...
    cachedReleaseGate = readTextRecordFromExports(instance, 'venom_qjs_release_gate_ptr', 'venom_qjs_release_gate_size', 'venom_qjs_release_gate_hash');
          ...
          cachedUpstreamPrototypeChainRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_prototype_chain_record_ptr', 'venom_qjs_upstream_prototype_chain_record_size', 'venom_qjs_upstream_prototype_chain_record_hash');
          cachedUpstreamCallFrameRecord = readTextRecordFromExports(instance, 'venom_qjs_upstream_call_frame_record_ptr', 'venom_qjs_upstream_call_frame_record_size', 'venom_qjs_upstream_call_frame_record_hash');
    const statusRecord = (typeof e.venom_qjs_status_record_ptr === 'function' && typeof e.venom_qjs_status_record_size === 'function') ? readWasmString(instance, e.venom_qjs_status_record_ptr() >>> 0, e.venom_qjs_status_record_size() >>> 0) : '';
    const contextReport = (typeof e.venom_qjs_context_report_ptr === 'function' && typeof e.venom_qjs_context_report_size === 'function') ? readWasmString(instance, e.venom_qjs_context_report_ptr(mutableContext.wasmContext >>> 0) >>> 0, e.venom_qjs_context_report_size() >>> 0) : '';
    const wasmHostCallCount = typeof e.venom_qjs_host_call_count === 'function' ? e.venom_qjs_host_call_count() >>> 0 : 0;
    const wasmConsoleCount = typeof e.venom_qjs_console_event_count === 'function' ? e.venom_qjs_console_event_count() >>> 0 : 0;
  ...
  function importModuleNamespace(specifier, referrer) {
    ...
    if (!normalized || normalized.startsWith('blocked:')) {
      ...
      if (config.resolverAudit && String(config.resolverAudit.unknownSpecifier || '').includes('trap')) throw new Error('QuickJS module resolver blocked ' + specifier);
      return Object.freeze({});
    }
  ...
  function transformModuleSource(source, chunk) {
    ...
    if (/\bimport\s*\(/.test(code)) throw new Error('dynamic import is trapped by QuickJS module execution prototype');
    if (/\bawait\b/.test(code) && /^\s*await\b/m.test(code)) throw new Error('top-level await is not enabled in QuickJS module execution prototype');
    const imports = [];
    const exports = [];
  ...
  async function executeChunk(input = {}) {
    ...
    if ((!chunk.code || !String(chunk.code).trim()) && !(chunk.bytecodeBytes && chunk.bytecodeBytes.length)) {
      const empty = Object.freeze({ executed: false, empty: true, engineModule: true, moduleVersion, mode, context: context.key, quickJsWasm: !!wasmUrl });
    ...
    if (wasmResult) {
      ...
      if (bindings.console && typeof bindings.console.debug === 'function') {
        bindings.console.debug('[venom] QuickJS WASM backend accepted bytecode chunk', chunk.source || 'inline');
      }
      if (fallbackDenied && wasmResult) {
        ...
        const result = Object.freeze({ executed: backendAccepted, engineModule: true, moduleVersion, mode, context: context.key, fallback: 'denied-host-js-source-eval', quickJsWasm: true, bytecodeExecuted: !!wasmResult.bytecodeExecuted, wasmReport: wasmResult.report, wasmExecutionRecord: wasmResult.executionRecord, fallbackRequired: false, hostEffectsApplied: effectsApplied, hostBridgeEffectReplay: bridgeReplay, module: false, consoleEvents: consoleEvents.length, resultRecordCount: resultRecords.length, capabilities: bindings.__venomCapabilities ? bindings.__venomCapabilities.length : 0, sourceEvalUsed: false, hostBridgeTelemetry: hostBridgeParity, hostBridgeParity });
        boundedPush(executions, result, maxExecutionRecords);
        return result;
      }
      if (wasmResult.fallbackRequired && fallbackDenied) throw new Error('QuickJS fallback policy denied backend fallback');
    }
    if (!chunk.code && chunk.bytecodeBytes && chunk.bytecodeBytes.length) {
      if (fallbackDenied) throw new Error('QuickJS WASM interpreter unavailable; source-decode fallback denied: ' + wasmFailureDetail());
      chunk = { ...chunk, code: decodeQuickJsRecordForDeniedFallback(chunk.bytecodeBytes, chunk.source) };
    }
    if (fallbackDenied) throw new Error('QuickJS/WASM backend failed; protected host source execution is denied');
...
__VENOM_QUICKJS_FALLBACK_BLOCK__
    const g = bindings.globalThis || globalThis;
    g.__venomQuickJsModuleProbe = (g.__venomQuickJsModuleProbe || 0) + 1;
