...
#define VENOM_TJS_RUNTIME_ABI_VERSION 12u
#define VENOM_TJS_RUNTIME_PACKAGE_VERSION 83u
...
static void venom_tjs_report_init(VenomTurboJsNativeReport* report) {
  if (!report) return;
  memset(report, 0, sizeof(*report));
  report->runtime_abi = VENOM_TJS_RUNTIME_ABI_VERSION;
  report->package_version = VENOM_TJS_RUNTIME_PACKAGE_VERSION;
}
...
int venom_tjs_native_run_parity_smoke(VenomTurboJsNativeReport* report) {
  venom_tjs_report_init(report);
  if (report) {
    report->available = 0u;
    report->checks_failed = 1u;
    snprintf(report->exception, sizeof(report->exception), "%s", "VENOM_ENABLE_FULL_TURBOJS is OFF; native upstream TurboJS is not linked");
  }
  return 3;
}
#else
...
static void set_exception(VenomTurboJsNativeReport* report, JSContext* ctx, const char* label) {
  if (!report) return;
  if (!ctx) {
    snprintf(report->exception, sizeof(report->exception), "%s", label ? label : "turbojs error");
    return;
  }
  JSValue ex = JS_GetException(ctx);
  const char* msg = JS_ToCString(ctx, ex);
  if (msg) {
    snprintf(report->exception, sizeof(report->exception), "%s: %s", label ? label : "turbojs exception", msg);
    JS_FreeCString(ctx, msg);
  } else {
    snprintf(report->exception, sizeof(report->exception), "%s", label ? label : "turbojs exception");
  }
  JS_FreeValue(ctx, ex);
}
...
int venom_tjs_native_run_parity_smoke(VenomTurboJsNativeReport* report) {
  ...
  snprintf(report->result_json, sizeof(report->result_json),
           "{\"engine\":\"upstream-turbojs-native\",\"available\":true,\"abi\":%u,\"packageVersion\":%u,\"checksPassed\":%u,\"checksFailed\":%u}",
