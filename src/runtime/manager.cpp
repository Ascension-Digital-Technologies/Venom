...
namespace {
fs::path wasm_path(const RuntimeOptions& o) { return (o.directory.empty() ? default_runtime_directory() : fs::absolute(o.directory)) / "turbojs-runtime.wasm"; }
bool valid_wasm(const fs::path& path) {
  std::ifstream in(path, std::ios::binary);
  unsigned char h[8]{}; in.read(reinterpret_cast<char*>(h), 8);
  return in.gcount()==8 && h[0]==0x00 && h[1]==0x61 && h[2]==0x73 && h[3]==0x6d && fs::file_size(path)==sizeof(kTurboJsRuntimeWasmBlob);
}
void install(const fs::path& path, bool force) {
  ...
  { std::ofstream out(tmp, std::ios::binary|std::ios::trunc); if(!out) raise_error("VENOM-E8000", "failed to create runtime: "+tmp); out.write(reinterpret_cast<const char*>(kTurboJsRuntimeWasmBlob), sizeof(kTurboJsRuntimeWasmBlob)); }
  ...
  manifest << "{\n  \"product\": \"Venom TurboJS/WASM\",\n  \"version\": \"" << VENOM_VERSION_STRING << "\",\n  \"bytes\": " << sizeof(kTurboJsRuntimeWasmBlob) << "\n}\n";
...
bool manage_runtime(const RuntimeOptions& options) {
  ...
  if (options.action=="verify") {
    ...
    std::cout << (ok?"[PASS] ":"[FAIL] ") << "TurboJS/WASM runtime: " << path.string() << "\n";
    return ok;
  }
