...
#if defined(__EMSCRIPTEN__) && !defined(VENOM_QJS_MINIMAL_EXPORTS)
#define VENOM_QJS_PUBLIC(name) __attribute__((export_name(name)))
#else
/* Release builds export only the symbols named by -sEXPORTED_FUNCTIONS. */
#define VENOM_QJS_PUBLIC(name)
#endif
#define VENOM_QJS_INTERNAL(name)
#if defined(__EMSCRIPTEN__)
#include <emscripten/stack.h>
#endif
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
#include <stdio.h>
#include <string.h>
#include "quickjs.h"
#endif
...
static void venom_qjs_secure_zero(void* ptr, uint32_t size) {
...
#define QJS_STATUS_OK 0u
#define QJS_STATUS_ALLOC_LIMIT 1u
#define QJS_STATUS_HASH_MISMATCH 2u
#define QJS_STATUS_INVALID_BYTECODE 3u
#define QJS_STATUS_EXECUTION_TRAP 4u
#define QJS_STATUS_HOST_DENIED 5u
#define QJS_STATUS_RELEASE_REJECTED 6u
#define QJS_STATUS_BAD_CONTEXT 7u
...
static uint32_t g_last_status_code = QJS_STATUS_OK;
static uint32_t g_last_validation_code = QJS_STATUS_OK;
...
static struct QjsContextAccount g_contexts[MAX_CONTEXTS];

struct QjsCheckpointRecord {
  uint32_t id;
  uint32_t alive;
  uint32_t ctx;
  uint32_t stage;
...
static struct QjsCheckpointRecord g_checkpoints[MAX_CHECKPOINTS];

static const unsigned char g_abi_table[] =
  "VENOM_QJS_RUNTIME_ABI_V12\n"
  "abi=12\n"
  "package_version=83\n"
  "entry_count=236\n"
  ...
  "entry\tvenom_qjs_engine_abi\texport\t() -> u32\truntime ABI major version\n"
  "entry\tvenom_qjs_engine_version\texport\t() -> u32\tVenom runtime version\n"
  "entry\tvenom_qjs_engine_state\texport\t() -> u32\tlifecycle state\n"
  "entry\tvenom_qjs_engine_trap_code\texport\t() -> u32\tlatest trap code\n"
  "entry\tvenom_qjs_context_set_limits\texport\t(ctx,heap,stack) -> bool\tcontext limits\n"
  "entry\tvenom_qjs_context_heap_used\texport\t(ctx) -> u32\theap accounting\n"
  "entry\tvenom_qjs_script_buffer_alloc\texport\t(ctx,bytes) -> ptr\tscript byte buffer allocation\n"
  "entry\tvenom_qjs_execute_bytecode\texport\t(ctx,bytes) -> bool\tprotected VQJSBC03 bytecode decode/execute boundary\n"
  "entry\tvenom_qjs_bytecode_record_executed\texport\t() -> bool\tlast execution used bytecode boundary\n"
  "entry\tvenom_qjs_script_buffer_set_expected_hash\texport\t(ctx,hash) -> bool\thash validation\n"
  "entry\tvenom_qjs_script_buffer_alloc_count\texport\t() -> u32\tallocation counter\n"
  "entry\tvenom_qjs_script_buffer_free_count\texport\t() -> u32\tfree counter\n"
  "entry\tvenom_qjs_console_callback_abi\texport\t() -> u32\tconsole callback ABI\n"
  "entry\tvenom_qjs_host_call_known\texport\t(id) -> bool\thost dispatch validation\n"
  "entry\tvenom_qjs_host_call_dispatch\texport\t(id,payload,size) -> bool\thost dispatch\n"
  "entry\tvenom_qjs_exception_message_ptr\texport\t() -> ptr\texception message\n"
  "entry\tvenom_qjs_parity_probe\texport\t() -> u32\tnative/WASM parity probe\n"
  "entry\tvenom_qjs_execution_journal_ptr\texport\t() -> ptr\texecution journal pointer\n"
  "entry\tvenom_qjs_execution_journal_size\texport\t() -> u32\texecution journal size\n"
  "entry\tvenom_qjs_checkpoint_create\texport\t(ctx,stage) -> u32\tcheckpoint capture\n"
  "entry\tvenom_qjs_checkpoint_restore\texport\t(ctx,checkpoint) -> bool\tcheckpoint restore\n"
  "entry\tvenom_qjs_replay_advance\texport\t(ctx,stage) -> bool\treplay cursor advance\n"
  "entry\tvenom_qjs_resume_state_ptr\texport\t() -> ptr\tresume state pointer\n"
  "entry\tvenom_qjs_determinism_audit_ptr\texport\t() -> ptr\tdeterminism audit pointer\n"
  "entry\tvenom_qjs_execution_commit_ptr\texport\t() -> ptr\texecution commit pointer\n"
  "entry\tvenom_qjs_execution_commit_size\texport\t() -> u32\texecution commit size\n"
  "entry\tvenom_qjs_execution_commit_hash\texport\t() -> u32\texecution commit hash\n"
  "entry\tvenom_qjs_execution_commit\texport\t(ctx,checkpoint_id) -> commit_hash\tcommit execution state\n"
  "entry\tvenom_qjs_execution_rollback\texport\t(ctx,checkpoint_id) -> bool\trollback to checkpoint\n"
  "entry\tvenom_qjs_host_receipts_ptr\texport\t() -> ptr\thost receipts pointer\n"
  "entry\tvenom_qjs_release_accept\texport\t(ctx) -> bool\trelease acceptance gate\n"
  "entry\tvenom_qjs_commit_audit_ptr\texport\t() -> ptr\tcommit audit pointer\n"
  "entry\tvenom_qjs_host_io_decision_ptr\texport\t() -> ptr\thost I/O decision records pointer\n"
  "entry\tvenom_qjs_host_io_decision_size\texport\t() -> u32\thost I/O decision records size\n"
  "entry\tvenom_qjs_host_io_decision_hash\texport\t() -> u32\thost I/O decision records hash\n"
  "entry\tvenom_qjs_host_io_request\texport\t(ctx,io_class,capability,payload_hash) -> bool\tenforced host I/O request\n"
  "entry\tvenom_qjs_host_io_deny_trace_ptr\texport\t() -> ptr\thost I/O denial trace pointer\n"
  "entry\tvenom_qjs_host_io_deny_trace_size\texport\t() -> u32\thost I/O denial trace size\n"
  "entry\tvenom_qjs_host_io_deny_trace_hash\texport\t() -> u32\thost I/O denial trace hash\n"
  "entry\tvenom_qjs_capability_ledger_ptr\texport\t() -> ptr\tcapability ledger pointer\n"
  "entry\tvenom_qjs_capability_ledger_size\texport\t() -> u32\tcapability ledger size\n"
  "entry\tvenom_qjs_capability_ledger_hash\texport\t() -> u32\tcapability ledger hash\n"
  "entry\tvenom_qjs_policy_seal_audit_ptr\texport\t() -> ptr\tpolicy seal audit pointer\n"
  "entry\tvenom_qjs_policy_seal_audit_size\texport\t() -> u32\tpolicy seal audit size\n"
  "entry\tvenom_qjs_policy_seal_audit_hash\texport\t() -> u32\tpolicy seal audit hash\n"
  "entry\tvenom_qjs_status_code\texport\t() -> u32\tlast ABI status code\n"
  "entry\tvenom_qjs_status_record_ptr\texport\t() -> ptr\tstatus record pointer\n"
  "entry\tvenom_qjs_status_record_size\texport\t() -> u32\tstatus record size\n"
  "entry\tvenom_qjs_status_record_hash\texport\t() -> u32\tstatus record hash\n"
  "entry\tvenom_qjs_runtime_limits_ptr\texport\t() -> ptr\truntime limits table pointer\n"
  "entry\tvenom_qjs_runtime_limits_size\texport\t() -> u32\truntime limits table size\n"
  "entry\tvenom_qjs_runtime_limits_hash\texport\t() -> u32\truntime limits table hash\n"
  "entry\tvenom_qjs_context_report_ptr\texport\t(ctx) -> ptr\tcontext accounting report pointer\n"
  "entry\tvenom_qjs_context_report_size\texport\t() -> u32\tcontext accounting report size\n"
  "entry\tvenom_qjs_context_report_hash\texport\t() -> u32\tcontext accounting report hash\n"
  "entry\tvenom_qjs_bytecode_validate\texport\t(ctx,bytes) -> status\tvalidate VQJSBC03 record without executing\n"
  "entry\tvenom_qjs_bytecode_record_hash32\texport\t() -> u32\tlast bytecode record hash\n"
  "entry\tvenom_qjs_bytecode_payload_size\texport\t() -> u32\tdecoded bytecode payload size\n"
  "entry\tvenom_qjs_bytecode_expected_source_hash32\texport\t() -> u32\texpected decoded source hash\n"
  "entry\tvenom_qjs_backend_contract_ptr\texport\t() -> ptr\tbackend contract record pointer\n"
  "entry\tvenom_qjs_backend_contract_size\texport\t() -> u32\tbackend contract record size\n"
  "entry\tvenom_qjs_backend_contract_hash\texport\t() -> u32\tbackend contract record hash\n"
  "entry\tvenom_qjs_release_status\texport\t() -> u32\tfail-closed release status\n"
  "entry\tvenom_qjs_interpreter_ready\texport\t() -> bool\tQuickJS WASM interpreter dispatch is available\n"
  "entry\tvenom_qjs_interpreter_contract_ptr\texport\t() -> ptr\tinterpreter contract pointer\n"
  "entry\tvenom_qjs_interpreter_contract_size\texport\t() -> u32\tinterpreter contract byte size\n"
  "entry\tvenom_qjs_interpreter_contract_hash\texport\t() -> u32\tinterpreter contract hash\n"
  "entry\tvenom_qjs_interpreter_dispatch_count\texport\t() -> u32\tinterpreter dispatch counter\n"
  "entry\tvenom_qjs_interpreter_opcode_count\texport\t() -> u32\tlast interpreter opcode estimate\n"
  "entry\tvenom_qjs_global_slot_count\texport\t() -> u32\ttracked global slot writes\n"
  "entry\tvenom_qjs_last_global_write_hash\texport\t() -> u32\tlast tracked global slot hash\n"
  "entry\tvenom_qjs_interpreter_semantic_pass_count\texport\t() -> u32\tsemantic pass counter\n"
  "entry\tvenom_qjs_interpreter_property_write_count\texport\t() -> u32\ttracked property writes\n"
  "entry\tvenom_qjs_interpreter_builtin_probe_count\texport\t() -> u32\tbuiltin probes observed by semantic interpreter\n"
  "entry\tvenom_qjs_interpreter_console_call_count\texport\t() -> u32\tconsole calls observed by semantic interpreter\n"
  "entry\tvenom_qjs_interpreter_semantic_record_ptr\texport\t() -> ptr\tsemantic runtime report pointer\n"
  "entry\tvenom_qjs_interpreter_semantic_record_size\texport\t() -> u32\tsemantic runtime report size\n"
  "entry\tvenom_qjs_interpreter_semantic_record_hash\texport\t() -> u32\tsemantic runtime report hash\n"
  "entry\tvenom_qjs_global_slot_record_ptr\texport\t() -> ptr\tglobal slot record pointer\n"
  "entry\tvenom_qjs_global_slot_record_size\texport\t() -> u32\tglobal slot record size\n"
  "entry\tvenom_qjs_global_slot_record_hash\texport\t() -> u32\tglobal slot record hash\n"
  "entry\tvenom_qjs_upstream_object_record_ptr\texport\t() -> ptr\tupstream object model record pointer\n"
  "entry\tvenom_qjs_upstream_object_record_size\texport\t() -> u32\tupstream object model record size\n"
  "entry\tvenom_qjs_upstream_object_record_hash\texport\t() -> u32\tupstream object model record hash\n"
  "entry\tvenom_qjs_upstream_exception_record_ptr\texport\t() -> ptr\tupstream exception model record pointer\n"
  "entry\tvenom_qjs_upstream_exception_record_size\texport\t() -> u32\tupstream exception model record size\n"
  "entry\tvenom_qjs_upstream_exception_record_hash\texport\t() -> u32\tupstream exception model record hash\n"
  "entry\tvenom_qjs_upstream_module_record_ptr\texport\t() -> ptr\tupstream module model record pointer\n"
  "entry\tvenom_qjs_upstream_module_record_size\texport\t() -> u32\tupstream module model record size\n"
  "entry\tvenom_qjs_upstream_module_record_hash\texport\t() -> u32\tupstream module model record hash\n"
  "entry\tvenom_qjs_upstream_runtime_bridge_score\texport\t() -> u32\tcombined upstream runtime bridge behavior score\n"
  "entry\tvenom_qjs_upstream_bytecode_semantics_record_ptr\texport\t() -> ptr\tupstream bytecode semantics record pointer\n"
  "entry\tvenom_qjs_upstream_bytecode_semantics_record_size\texport\t() -> u32\tupstream bytecode semantics record size\n"
  "entry\tvenom_qjs_upstream_bytecode_semantics_record_hash\texport\t() -> u32\tupstream bytecode semantics record hash\n"
  "entry\tvenom_qjs_upstream_bytecode_semantics_score\texport\t() -> u32\tupstream bytecode semantics score\n"
  "entry\tvenom_qjs_upstream_lexical_scope_record_ptr\texport\t() -> ptr\tupstream lexical scope record pointer\n"
  "entry\tvenom_qjs_upstream_lexical_scope_record_size\texport\t() -> u32\tupstream lexical scope record size\n"
  "entry\tvenom_qjs_upstream_lexical_scope_record_hash\texport\t() -> u32\tupstream lexical scope record hash\n"
  "entry\tvenom_qjs_upstream_closure_record_ptr\texport\t() -> ptr\tupstream closure record pointer\n"
  "entry\tvenom_qjs_upstream_closure_record_size\texport\t() -> u32\tupstream closure record size\n"
  "entry\tvenom_qjs_upstream_closure_record_hash\texport\t() -> u32\tupstream closure record hash\n"
  "entry\tvenom_qjs_upstream_iterator_record_ptr\texport\t() -> ptr\tupstream iterator record pointer\n"
  "entry\tvenom_qjs_upstream_iterator_record_size\texport\t() -> u32\tupstream iterator record size\n"
  "entry\tvenom_qjs_upstream_iterator_record_hash\texport\t() -> u32\tupstream iterator record hash\n"
  "entry\tvenom_qjs_upstream_intrinsic_record_ptr\texport\t() -> ptr\tupstream intrinsic semantics record pointer\n"
  "entry\tvenom_qjs_upstream_intrinsic_record_size\texport\t() -> u32\tupstream intrinsic semantics record size\n"
  "entry\tvenom_qjs_upstream_intrinsic_record_hash\texport\t() -> u32\tupstream intrinsic semantics record hash\n"
  "entry\tvenom_qjs_upstream_intrinsic_semantics_score\texport\t() -> u32\tupstream intrinsic semantics score\n"
  "entry\tvenom_qjs_upstream_property_descriptor_record_ptr\texport\t() -> ptr\tupstream property descriptor record pointer\n"
  "entry\tvenom_qjs_upstream_property_descriptor_record_size\texport\t() -> u32\tupstream property descriptor record size\n"
  "entry\tvenom_qjs_upstream_property_descriptor_record_hash\texport\t() -> u32\tupstream property descriptor record hash\n"
  "entry\tvenom_qjs_upstream_prototype_chain_record_ptr\texport\t() -> ptr\tupstream prototype chain record pointer\n"
  "entry\tvenom_qjs_upstream_prototype_chain_record_size\texport\t() -> u32\tupstream prototype chain record size\n"
  "entry\tvenom_qjs_upstream_prototype_chain_record_hash\texport\t() -> u32\tupstream prototype chain record hash\n"
  "entry\tvenom_qjs_upstream_call_frame_record_ptr\texport\t() -> ptr\tupstream call frame record pointer\n"
  "entry\tvenom_qjs_upstream_call_frame_record_size\texport\t() -> u32\tupstream call frame record size\n"
  "entry\tvenom_qjs_upstream_call_frame_record_hash\texport\t() -> u32\tupstream call frame record hash\n"
  "entry\tvenom_qjs_runtime_denylist_ptr\texport\t() -> ptr\truntime denylist pointer\n"
  "entry\tvenom_qjs_runtime_denylist_size\texport\t() -> u32\truntime denylist size\n"
  "entry\tvenom_qjs_runtime_denylist_hash\texport\t() -> u32\truntime denylist hash\n";

static const unsigned char g_host_import_table[] =
  "VENOM_QJS_HOST_IMPORT_TABLE_V10\n"
  "version=9\n"
  "required=false\n"
...
static const unsigned char g_bytecode_manifest[] =
  "VENOM_QJS_BYTECODE_MANIFEST_V3\n"
  "version=3\n"
  "runtime_abi=12\n"
  "package_version=83\n"
...
static const unsigned char g_module_graph[] =
  "VENOM_QJS_MODULE_GRAPH_RUNTIME_V1\n"
  "version=1\n"
  "runtime_abi=12\n"
  "package_version=83\n"
...
static const unsigned char g_checkpoint_policy_text[] =
  "VENOM_QJS_CHECKPOINT_POLICY_RUNTIME_V1\n"
  "version=1\n"
  "runtime_abi=12\n"
  "package_version=83\n"
...
static const unsigned char g_snapshot_policy_text[] =
  "VENOM_QJS_SNAPSHOT_POLICY_RUNTIME_V1\n"
  "version=1\n"
  "runtime_abi=12\n"
  "package_version=83\n"
...
static const unsigned char g_capability_policy_text[] =
  "VENOM_QJS_CAPABILITY_POLICY_RUNTIME_V1\n"
  "version=1\n"
  "runtime_abi=12\n"
  "package_version=83\n"
  "default=deny\n"
...
static const unsigned char g_host_io_policy_text[] =
  "VENOM_QJS_HOST_IO_POLICY_RUNTIME_V1\n"
  "version=1\n"
  "runtime_abi=12\n"
  "package_version=83\n"
  "console=allow\n"
...
static const unsigned char g_backend_contract_text[] =
  "VENOM_QJS_WASM_BACKEND_CONTRACT_V2\n"
  "version=2\n"
  "runtime_abi=12\n"
  "package_version=83\n"
  "backend=quickjs-wasm-abi12-upstream-global-host-api-shims\n"
  ...
  "upstream_parity_exports=venom_qjs_upstream_quickjs_ready|venom_qjs_upstream_parity_record_ptr|venom_qjs_upstream_feature_count\n"
  "upstream_runtime_exports=venom_qjs_upstream_object_record_ptr|venom_qjs_upstream_exception_record_ptr|venom_qjs_upstream_module_record_ptr|venom_qjs_upstream_runtime_bridge_score|venom_qjs_upstream_bytecode_semantics_record_ptr|venom_qjs_upstream_lexical_scope_record_ptr|venom_qjs_upstream_closure_record_ptr|venom_qjs_upstream_iterator_record_ptr\n"
  "semantic_exports=venom_qjs_interpreter_semantic_record_ptr|venom_qjs_global_slot_record_ptr|venom_qjs_interpreter_property_write_count\n"
...
static const unsigned char g_interpreter_contract_text[] =
  ...
  "runtime_abi=12\n"
  "package_version=83\n"
  "mode=quickjs-wasm-upstream-intrinsic-semantics\n"
  "upstream_quickjs_bridge=enabled\n"
...
#ifndef VENOM_ENABLE_UPSTREAM_QJS_WASM
...
#define VQJSBC03_HEADER_SIZE 48u
#define VQJSBC03_ABI_MARKER 0x01000300u

/* Validate the native QuickJS object record without materializing source text.
   The payload is the direct JS_WriteObject(..., JS_WRITE_OBJ_BYTECODE |
   JS_WRITE_OBJ_STRIP_SOURCE | JS_WRITE_OBJ_STRIP_DEBUG) result. */
static uint32_t validate_vqjsbc03_at(const unsigned char* record,
  ...
  *source_hash32_out = 0u;
  if (record_size < VQJSBC03_HEADER_SIZE || record_size > SOURCE_CAP) return 0u;
  if (!(record[0] == 'V' && record[1] == 'Q' && record[2] == 'J' && record[3] == 'S' &&
  ...
  (void)flags;
  if (version != 3u || abi_marker != VQJSBC03_ABI_MARKER || payload_size == 0u || source_size == 0u) return 0u;
  if (payload_offset != VQJSBC03_HEADER_SIZE || payload_offset > record_size || payload_size > record_size - payload_offset) return 0u;
...
static uint32_t validate_vqjsbc03_record(uint32_t record_size,
                                         uint32_t* payload_offset_out,
                                         uint32_t* payload_size_out,
                                         uint32_t* source_size_out,
                                         uint32_t* source_hash32_out) {
  return validate_vqjsbc03_at(g_source, record_size, payload_offset_out, payload_size_out, source_size_out, source_hash32_out);
}

#define VQJSMB04_HEADER_SIZE 24u
#define VQJSMB04_ENTRY_SIZE 16u

static uint32_t validate_vqjsmb04_record(uint32_t record_size, uint32_t* module_count_out, uint32_t* entry_index_out) {
  if (!module_count_out || !entry_index_out) return 0u;
  *module_count_out = 0u; *entry_index_out = 0u;
  if (record_size < VQJSMB04_HEADER_SIZE || record_size > SOURCE_CAP) return 0u;
  if (!(g_source[0] == 'V' && g_source[1] == 'Q' && g_source[2] == 'J' && g_source[3] == 'S' &&
  ...
  if (version != 1u || count == 0u || count > MAX_MODULE_RECORDS || entry_index >= count || table_offset != VQJSMB04_HEADER_SIZE) return 0u;
  if (count > (record_size - table_offset) / VQJSMB04_ENTRY_SIZE) return 0u;
  for (uint32_t i = 0u; i < count; ++i) {
    const unsigned char* item = g_source + table_offset + i * VQJSMB04_ENTRY_SIZE;
    ...
    if (!validate_vqjsbc03_at(g_source + nested_offset, nested_size, &po, &ps, &ss, &sh)) return 0u;
  }
...
static struct QjsContextAccount* find_context(uint32_t id) {
  for (uint32_t i = 0; i < MAX_CONTEXTS; ++i) if (g_contexts[i].alive && g_contexts[i].id == id) return &g_contexts[i];
  return 0;
}

static struct QjsContextAccount* make_context(uint32_t id) {
  struct QjsContextAccount* ctx = find_context(id);
  if (ctx) return ctx;
...
static void build_exception_json(uint32_t ctx, uint32_t code, const char* kind, const char* message, uint32_t bytes, uint32_t hash) {
  ...
  if (g_last_status_code == QJS_STATUS_OK) g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
  g_engine_state = STATE_TRAPPED;
  set_exception_message(message);
  out_str(g_exception, &p, EXCEPTION_CAP, "{\"ok\":false,\"abi\":"); out_u32(g_exception, &p, EXCEPTION_CAP, QJS_ABI);
  ...
  out_str(g_exception, &p, EXCEPTION_CAP, "\",\"message\":\""); out_str(g_exception, &p, EXCEPTION_CAP, message ? message : "QuickJS scaffold exception");
...
static struct QjsCheckpointRecord* find_checkpoint(uint32_t id) {
  for (uint32_t i = 0u; i < MAX_CHECKPOINTS; ++i) if (g_checkpoints[i].alive && g_checkpoints[i].id == id) return &g_checkpoints[i];
  return 0;
}

static struct QjsCheckpointRecord* alloc_checkpoint(void) {
...
static void build_execution_journal_json(uint32_t ctx, uint32_t stage, uint32_t checkpoint_id) {
  uint32_t p = 0u;
  out_str(g_execution_journal, &p, EXECUTION_JOURNAL_CAP, "{\"abi\":1,\"runtimeAbi\":"); out_u32(g_execution_journal, &p, EXECUTION_JOURNAL_CAP, QJS_ABI);
  out_str(g_execution_journal, &p, EXECUTION_JOURNAL_CAP, ",\"version\":"); out_u32(g_execution_journal, &p, EXECUTION_JOURNAL_CAP, QJS_VERSION);
...
static void build_resume_state_json(uint32_t ctx, uint32_t checkpoint_id, uint32_t ready) {
  struct QjsContextAccount* rec = find_context(ctx);
  uint32_t p = 0u;
...
static void build_permission_seal_json(uint32_t ctx) {
  ...
  g_permission_seal_hash = capability_hash ^ io_hash ^ receipt_hash ^ decision_hash ^ ledger_hash ^ QJS_ABI ^ QJS_VERSION;
...
static void build_upstream_object_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash, uint32_t object_score) {
  ...
  out_str(g_upstream_object_record, &p, UPSTREAM_OBJECT_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-object-semantics-v3\"");
...
static void build_upstream_exception_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash, uint32_t exception_score) {
  ...
  out_str(g_upstream_exception_record, &p, UPSTREAM_EXCEPTION_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-exception-semantics-v3\"");
...
static void build_upstream_module_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash, uint32_t module_score) {
  ...
  out_str(g_upstream_module_record, &p, UPSTREAM_MODULE_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-module-semantics-v3\"");
...
static void build_upstream_parity_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_parity_record, &p, UPSTREAM_PARITY_RECORD_CAP, ",\"mode\":\"quickjs-upstream-global-host-api-shims-v7\"");
  out_str(g_upstream_parity_record, &p, UPSTREAM_PARITY_RECORD_CAP, ",\"vendoredQuickJs\":true");
  ...
  out_str(g_upstream_parity_record, &p, UPSTREAM_PARITY_RECORD_CAP, ",\"cutover\":\"scaffold-semantics-to-upstream-quickjs-adapter\"");
...
static void build_upstream_lexical_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_lexical_record, &p, UPSTREAM_LEXICAL_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-lexical-scope-v3\"");
...
static void build_upstream_closure_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_closure_record, &p, UPSTREAM_CLOSURE_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-closure-semantics-v3\"");
...
static void build_upstream_iterator_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_iterator_record, &p, UPSTREAM_ITERATOR_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-iterator-semantics-v3\"");
...
static void build_upstream_property_descriptor_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_property_descriptor_record, &p, UPSTREAM_PROPERTY_DESCRIPTOR_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-property-descriptor-semantics-v4\"");
...
static void build_upstream_prototype_chain_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_prototype_chain_record, &p, UPSTREAM_PROTOTYPE_CHAIN_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-prototype-chain-semantics-v4\"");
...
static void build_upstream_call_frame_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_call_frame_record, &p, UPSTREAM_CALL_FRAME_RECORD_CAP, ",\"bridge\":\"quickjs-upstream-call-frame-semantics-v4\"");
...
static void build_upstream_intrinsic_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_intrinsic_record, &p, UPSTREAM_INTRINSIC_RECORD_CAP, ",\"mode\":\"quickjs-upstream-global-host-api-shims-v7\"");
...
static void build_upstream_bytecode_semantics_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_upstream_bytecode_semantics_record, &p, UPSTREAM_BYTECODE_SEMANTICS_RECORD_CAP, ",\"mode\":\"quickjs-upstream-global-host-api-shims-v7\"");
...
static void build_semantic_record_json(uint32_t ctx, uint32_t bytes, uint32_t source_hash) {
  ...
  out_str(g_semantic_record, &p, SEMANTIC_RECORD_CAP, ",\"mode\":\"quickjs-wasm-abi12-upstream-global-host-api-shims\"");
...
static void build_json(unsigned char* out, uint32_t* size, uint32_t cap, uint32_t ok, uint32_t ctx, uint32_t bytes, uint32_t hash, uint32_t lines, uint32_t has_console, uint32_t console_count, uint32_t heap_used, uint32_t heap_limit) {
  ...
  out_str(out, &p, cap, ",\"engine\":\"quickjs-wasm-backend-candidate\"");
  out_str(out, &p, cap, ",\"abi\":"); out_u32(out, &p, cap, QJS_ABI);
  out_str(out, &p, cap, ",\"version\":"); out_u32(out, &p, cap, QJS_VERSION);
  ...
  out_str(out, &p, cap, ",\"parityProbe\":"); out_u32(out, &p, cap, fnv32(g_abi_table, cstr_size(g_abi_table)) ^ fnv32(g_host_import_table, cstr_size(g_host_import_table)) ^ QJS_ABI ^ QJS_VERSION);
  ...
  out_str(out, &p, cap, ",\"upstreamQuickJsReady\":true");
...
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
static JSRuntime* g_upstream_qjs_runtime = 0;
static JSContext* g_upstream_qjs_context = 0;
...
static void venom_qjs_module_bundle_reset(void) {
  if (g_upstream_qjs_context) {
    for (uint32_t i = 0u; i < g_module_bundle_count; ++i) {
      if (g_module_bundle_entries[i].active) {
        JS_FreeValue(g_upstream_qjs_context, g_module_bundle_entries[i].object);
      }
...
static char* venom_qjs_module_normalize(JSContext* ctx, const char* module_base_name, const char* module_name, void* opaque) {
...
static JSModuleDef* venom_qjs_module_loader(JSContext* ctx, const char* module_name, void* opaque) {
  (void)opaque;
  if (!module_name) return 0;
  for (uint32_t i = 0u; i < g_module_bundle_count; ++i) {
...
static uint32_t g_upstream_qjs_eval_count = 0u;

static JSValue venom_qjs_console_log(JSContext* qctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  (void)qctx;
  (void)this_val;
  (void)argv;
...
static uint32_t venom_qjs_install_host_shims(JSContext* qctx) {
  static const char* prelude =
    "globalThis.window=globalThis;"
    "globalThis.globalThis=globalThis;"
    "globalThis.navigator=globalThis.navigator||{userAgent:'VenomQuickJSWasm'};"
    ...
    "globalThis.fetch=globalThis.fetch||function(){throw new Error('fetch is host-call gated by Venom QuickJS WASM')};"
  ...
  JS_SetPropertyStr(qctx, console, "log", JS_NewCFunction(qctx, venom_qjs_console_log, "log", 1));
  JS_SetPropertyStr(qctx, console, "warn", JS_NewCFunction(qctx, venom_qjs_console_log, "warn", 1));
  JS_SetPropertyStr(qctx, console, "error", JS_NewCFunction(qctx, venom_qjs_console_log, "error", 1));
...
static struct QjsContextAccount* g_interrupt_context = 0;
static int venom_qjs_interrupt_handler(JSRuntime* rt, void* opaque) {
  struct QjsContextAccount* rec = (struct QjsContextAccount*)opaque;
  (void)rt;
  if (!rec) return 1;
  rec->interrupt_ticks++;
...
static void venom_qjs_begin_execution_budget(struct QjsContextAccount* rec) {
  if (!rec || !g_upstream_qjs_runtime) return;
  rec->interrupt_ticks = 0u;
  rec->pending_jobs = 0u;
  g_interrupt_context = rec;
  JS_SetInterruptHandler(g_upstream_qjs_runtime, venom_qjs_interrupt_handler, rec);
}

static void venom_qjs_end_execution_budget(void) {
  if (g_upstream_qjs_runtime) JS_SetInterruptHandler(g_upstream_qjs_runtime, NULL, NULL);
  g_interrupt_context = 0;
}

static uint32_t venom_qjs_ensure_upstream_runtime(struct QjsContextAccount* rec) {
  if (g_upstream_qjs_context) return 1u;
  g_upstream_qjs_runtime = JS_NewRuntime();
  if (!g_upstream_qjs_runtime) return 0u;
  JS_SetMemoryLimit(g_upstream_qjs_runtime, rec && rec->heap_limit ? rec->heap_limit : DEFAULT_HEAP_LIMIT);
  JS_SetMaxStackSize(g_upstream_qjs_runtime, rec && rec->stack_limit ? rec->stack_limit : DEFAULT_STACK_LIMIT);
  g_upstream_qjs_context = JS_NewContext(g_upstream_qjs_runtime);
  if (!g_upstream_qjs_context) {
    JS_FreeRuntime(g_upstream_qjs_runtime);
    g_upstream_qjs_runtime = 0;
    return 0u;
  }
  return venom_qjs_install_host_shims(g_upstream_qjs_context);
}

static void venom_qjs_copy_exception_message(JSContext* qctx, uint32_t ctx, uint32_t bytes, uint32_t hash) {
  JSValue ex = JS_GetException(qctx);
  const char* msg = JS_ToCString(qctx, ex);
  if (msg && msg[0]) {
    build_exception_json(ctx, 91u, "quickjs-upstream", msg, bytes, hash);
  } else {
    build_exception_json(ctx, 91u, "quickjs-upstream", "upstream QuickJS execution trapped", bytes, hash);
  }
  if (msg) JS_FreeCString(qctx, msg);
  JS_FreeValue(qctx, ex);
}

static uint32_t venom_qjs_finish_upstream_jobs(uint32_t ctx, uint32_t size, uint32_t hash) {
  JSContext* job_ctx = 0;
  for (;;) {
    int job_rc = JS_ExecutePendingJob(g_upstream_qjs_runtime, &job_ctx);
    if (job_rc < 0) {
      g_engine_state = STATE_TRAPPED;
      g_engine_trap_code = QJS_STATUS_EXECUTION_TRAP;
      g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
      venom_qjs_copy_exception_message(job_ctx ? job_ctx : g_upstream_qjs_context, ctx, size, hash);
      build_status_record_json(ctx, g_last_status_code, "quickjs-upstream-job", size, hash);
      return 0u;
    }
    if (job_rc == 0) break;
    if (g_interrupt_context) {
      ...
      if (g_interrupt_context->pending_jobs > g_interrupt_context->pending_job_limit) {
        ...
        g_engine_trap_code = QJS_STATUS_EXECUTION_TRAP;
        g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
        build_exception_json(ctx, 92u, "quickjs-job-limit", "pending job limit exceeded", size, hash);
        build_status_record_json(ctx, g_last_status_code, "quickjs-job-limit", g_interrupt_context->pending_jobs, g_interrupt_context->pending_job_limit);
        return 0u;
      }
    }
  }
  ++g_upstream_qjs_eval_count;
  g_backend_kind = 2u;
  g_backend_ready = 1u;
...
static uint32_t venom_qjs_execute_source_with_upstream(uint32_t ctx, uint32_t size, uint32_t hash, struct QjsContextAccount* rec) {
  if (!venom_qjs_ensure_upstream_runtime(rec)) {
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    build_status_record_json(ctx, g_last_status_code, "quickjs-upstream-init", size, hash);
    build_exception_json(ctx, 90u, "quickjs-upstream", "failed to initialize upstream QuickJS runtime", size, hash);
    return 0u;
  }
  g_engine_state = STATE_EXECUTING;
  venom_qjs_begin_execution_budget(rec);
  JSValue value = JS_Eval(g_upstream_qjs_context, (const char*)g_source, size, "<venom-source>", JS_EVAL_TYPE_GLOBAL);
  if (JS_IsException(value)) {
    g_engine_state = STATE_TRAPPED;
    g_engine_trap_code = QJS_STATUS_EXECUTION_TRAP;
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, size, hash);
    build_status_record_json(ctx, g_last_status_code, "quickjs-upstream-source", size, hash);
    JS_FreeValue(g_upstream_qjs_context, value);
    venom_qjs_end_execution_budget();
    return 0u;
  }
  JS_FreeValue(g_upstream_qjs_context, value);
  const uint32_t jobs_ok = venom_qjs_finish_upstream_jobs(ctx, size, hash);
  venom_qjs_end_execution_budget();
  return jobs_ok;
}

static uint32_t venom_qjs_execute_bytecode_with_upstream(uint32_t ctx,
                                                         uint32_t payload_offset,
                                                         uint32_t payload_size,
                                                         uint32_t record_hash,
                                                         struct QjsContextAccount* rec) {
  if (!venom_qjs_ensure_upstream_runtime(rec)) {
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    build_status_record_json(ctx, g_last_status_code, "quickjs-bytecode-init", payload_size, record_hash);
    build_exception_json(ctx, 90u, "quickjs-bytecode", "failed to initialize upstream QuickJS runtime", payload_size, record_hash);
    return 0u;
  }
  g_engine_state = STATE_EXECUTING;
  JSValue object = JS_ReadObject(g_upstream_qjs_context,
                                 g_source + payload_offset,
                                 payload_size,
                                 JS_READ_OBJ_BYTECODE);
  if (JS_IsException(object)) {
    g_engine_state = STATE_TRAPPED;
    g_engine_trap_code = QJS_STATUS_INVALID_BYTECODE;
    g_last_status_code = QJS_STATUS_INVALID_BYTECODE;
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, payload_size, record_hash);
    build_status_record_json(ctx, g_last_status_code, "quickjs-bytecode-read", payload_size, record_hash);
    JS_FreeValue(g_upstream_qjs_context, object);
    return 0u;
  }
  JSValue value = JS_EvalFunction(g_upstream_qjs_context, object);
  if (JS_IsException(value)) {
    g_engine_state = STATE_TRAPPED;
    g_engine_trap_code = QJS_STATUS_EXECUTION_TRAP;
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, payload_size, record_hash);
    build_status_record_json(ctx, g_last_status_code, "quickjs-bytecode-execute", payload_size, record_hash);
    JS_FreeValue(g_upstream_qjs_context, value);
    return 0u;
  }
  JS_FreeValue(g_upstream_qjs_context, value);
  return venom_qjs_finish_upstream_jobs(ctx, payload_size, record_hash);
}


static uint32_t venom_qjs_execute_module_bundle_with_upstream(uint32_t ctx,
                                                              uint32_t record_size,
                                                              uint32_t record_hash,
                                                              struct QjsContextAccount* rec) {
  if (!venom_qjs_ensure_upstream_runtime(rec)) {
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    build_status_record_json(ctx, g_last_status_code, "quickjs-module-bundle-init", record_size, record_hash);
    build_exception_json(ctx, 90u, "quickjs-module-bundle", "failed to initialize upstream QuickJS runtime", record_size, record_hash);
    return 0u;
  }
  uint32_t module_count = 0u, entry_index = 0u;
  if (!validate_vqjsmb04_record(record_size, &module_count, &entry_index)) return 0u;
  venom_qjs_module_bundle_reset();
  JS_SetModuleLoaderFunc(g_upstream_qjs_runtime, venom_qjs_module_normalize, venom_qjs_module_loader, 0);
  ...
  for (uint32_t i = 0u; i < module_count; ++i) {
    ...
    if (!validate_vqjsbc03_at(g_source + nested_offset, nested_size, &payload_offset, &payload_size, &source_size, &source_hash32)) {
      venom_qjs_module_bundle_reset();
      return 0u;
    }
    JSValue object = JS_ReadObject(g_upstream_qjs_context,
                                   ...
                                   JS_READ_OBJ_BYTECODE);
    if (JS_IsException(object)) {
      venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, nested_size, record_hash);
      venom_qjs_module_bundle_reset();
      return 0u;
    }
  ...
  uint32_t payload_offset=0u, payload_size=0u, source_size=0u, source_hash32=0u;
  if (!entry_nested_size || !validate_vqjsbc03_at(g_source + entry_nested_offset, entry_nested_size, &payload_offset, &payload_size, &source_size, &source_hash32)) {
    venom_qjs_module_bundle_reset();
    return 0u;
  }
  JSValue entry_object = JS_ReadObject(g_upstream_qjs_context,
                                       g_source + entry_nested_offset + payload_offset,
                                       payload_size,
                                       JS_READ_OBJ_BYTECODE);
  if (JS_IsException(entry_object)) {
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, entry_nested_size, record_hash);
    venom_qjs_module_bundle_reset();
    return 0u;
  }
  ...
  g_engine_state = STATE_EXECUTING;
  venom_qjs_begin_execution_budget(rec);
  JSValue value = JS_EvalFunction(g_upstream_qjs_context, entry_object);
  if (JS_IsException(value)) {
    g_engine_state = STATE_TRAPPED;
    g_engine_trap_code = QJS_STATUS_EXECUTION_TRAP;
    g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, record_size, record_hash);
    JS_FreeValue(g_upstream_qjs_context, value);
    venom_qjs_end_execution_budget();
    venom_qjs_module_bundle_reset();
    return 0u;
  ...
  JS_FreeValue(g_upstream_qjs_context, value);
  const uint32_t jobs_ok = venom_qjs_finish_upstream_jobs(ctx, record_size, record_hash);
  venom_qjs_end_execution_budget();
  venom_qjs_module_bundle_reset();
  return jobs_ok;
}
...
static uint32_t venom_qjs_execute_source_with_upstream(uint32_t ctx, uint32_t size, uint32_t hash, struct QjsContextAccount* rec) {
  (void)rec;
  g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
  build_status_record_json(ctx, g_last_status_code, "quickjs-source-unavailable", size, hash);
  build_exception_json(ctx, 92u, "quickjs-source", "upstream QuickJS backend unavailable", size, hash);
  return 0u;
}
static uint32_t venom_qjs_execute_bytecode_with_upstream(uint32_t ctx, uint32_t payload_offset, uint32_t payload_size, uint32_t record_hash, struct QjsContextAccount* rec) {
  (void)payload_offset; (void)rec;
  g_last_status_code = QJS_STATUS_EXECUTION_TRAP;
  build_status_record_json(ctx, g_last_status_code, "quickjs-bytecode-unavailable", payload_size, record_hash);
  build_exception_json(ctx, 92u, "quickjs-bytecode", "upstream QuickJS bytecode backend unavailable", payload_size, record_hash);
  return 0u;
}
#endif
...
VENOM_QJS_PUBLIC("venom_qjs_wasm_native_stack_capacity") uint32_t venom_qjs_wasm_native_stack_capacity(void) {
  /*
...
VENOM_QJS_PUBLIC("venom_qjs_engine_abi") uint32_t venom_qjs_engine_abi(void) { return QJS_ABI; }
VENOM_QJS_PUBLIC("venom_qjs_engine_version") uint32_t venom_qjs_engine_version(void) { return QJS_VERSION; }
VENOM_QJS_PUBLIC("venom_qjs_engine_state") uint32_t venom_qjs_engine_state(void) { return g_engine_state; }
VENOM_QJS_PUBLIC("venom_qjs_engine_trap_code") uint32_t venom_qjs_engine_trap_code(void) { return g_engine_trap_code; }
VENOM_QJS_INTERNAL("venom_qjs_abi_table_ptr") uint32_t venom_qjs_abi_table_ptr(void) { return (uint32_t)(uintptr_t)g_abi_table; }
VENOM_QJS_INTERNAL("venom_qjs_abi_table_size") uint32_t venom_qjs_abi_table_size(void) { return cstr_size(g_abi_table); }
VENOM_QJS_INTERNAL("venom_qjs_abi_table_hash") uint32_t venom_qjs_abi_table_hash(void) { return fnv32(g_abi_table, cstr_size(g_abi_table)); }
VENOM_QJS_INTERNAL("venom_qjs_abi_entry_count") uint32_t venom_qjs_abi_entry_count(void) { return 185u; }
VENOM_QJS_INTERNAL("venom_qjs_host_import_table_ptr") uint32_t venom_qjs_host_import_table_ptr(void) { return (uint32_t)(uintptr_t)g_host_import_table; }
VENOM_QJS_INTERNAL("venom_qjs_host_import_table_size") uint32_t venom_qjs_host_import_table_size(void) { return cstr_size(g_host_import_table); }
VENOM_QJS_INTERNAL("venom_qjs_host_import_table_hash") uint32_t venom_qjs_host_import_table_hash(void) { return fnv32(g_host_import_table, cstr_size(g_host_import_table)); }
VENOM_QJS_INTERNAL("venom_qjs_host_import_count") uint32_t venom_qjs_host_import_count(void) { return 38u; }
VENOM_QJS_PUBLIC("venom_qjs_diversification_install") uint32_t venom_qjs_diversification_install(uint32_t ptr, uint32_t size) {
...
VENOM_QJS_PUBLIC("venom_qjs_host_call_known") uint32_t venom_qjs_host_call_known(uint32_t id) { return host_call_known(host_call_logical_from_wire(id)); }
VENOM_QJS_PUBLIC("venom_qjs_host_call_count") uint32_t venom_qjs_host_call_count(void) { return g_host_call_count; }
VENOM_QJS_PUBLIC("venom_qjs_host_call_dispatch") uint32_t venom_qjs_host_call_dispatch(uint32_t id, uint32_t payload_ptr, uint32_t payload_size) {
...
VENOM_QJS_PUBLIC("venom_qjs_parity_probe") uint32_t venom_qjs_parity_probe(void) { return fnv32(g_abi_table, cstr_size(g_abi_table)) ^ fnv32(g_host_import_table, cstr_size(g_host_import_table)) ^ QJS_ABI ^ QJS_VERSION; }
VENOM_QJS_INTERNAL("venom_qjs_bytecode_manifest_ptr") uint32_t venom_qjs_bytecode_manifest_ptr(void) { return (uint32_t)(uintptr_t)g_bytecode_manifest; }
VENOM_QJS_INTERNAL("venom_qjs_bytecode_manifest_size") uint32_t venom_qjs_bytecode_manifest_size(void) { return cstr_size(g_bytecode_manifest); }
VENOM_QJS_INTERNAL("venom_qjs_bytecode_manifest_hash") uint32_t venom_qjs_bytecode_manifest_hash(void) { return fnv32(g_bytecode_manifest, cstr_size(g_bytecode_manifest)); }
VENOM_QJS_INTERNAL("venom_qjs_module_resolver_abi") uint32_t venom_qjs_module_resolver_abi(void) { return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_module_record_ptr") uint32_t venom_qjs_module_record_ptr(void) { return (uint32_t)(uintptr_t)g_module_record; }
VENOM_QJS_PUBLIC("venom_qjs_module_record_size") uint32_t venom_qjs_module_record_size(void) { return g_module_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_module_resolve") uint32_t venom_qjs_module_resolve(uint32_t specifier_ptr, uint32_t specifier_size, uint32_t referrer_ptr, uint32_t referrer_size) { (void)specifier_ptr; (void)referrer_ptr; if (g_module_record_count >= MAX_MODULE_RECORDS) { build_exception_json(referrer_size, 50u, "module", "module record limit exceeded", specifier_size, 0u); return 0u; } build_module_record_json(referrer_size, specifier_size <= SOURCE_CAP, specifier_size); venom_qjs_host_call_dispatch(4u, specifier_ptr, specifier_size); return specifier_size <= SOURCE_CAP ? 1u : 0u; }
VENOM_QJS_INTERNAL("venom_qjs_module_graph_ptr") uint32_t venom_qjs_module_graph_ptr(void) { return (uint32_t)(uintptr_t)g_module_graph; }
VENOM_QJS_INTERNAL("venom_qjs_module_graph_size") uint32_t venom_qjs_module_graph_size(void) { return cstr_size(g_module_graph); }
VENOM_QJS_INTERNAL("venom_qjs_module_graph_hash") uint32_t venom_qjs_module_graph_hash(void) { return fnv32(g_module_graph, cstr_size(g_module_graph)); }
VENOM_QJS_PUBLIC("venom_qjs_module_execution_count") uint32_t venom_qjs_module_execution_count(void) { return g_module_execution_count; }
VENOM_QJS_INTERNAL("venom_qjs_module_cache_state_ptr") uint32_t venom_qjs_module_cache_state_ptr(void) { return (uint32_t)(uintptr_t)g_module_cache_state; }
VENOM_QJS_INTERNAL("venom_qjs_module_cache_state_size") uint32_t venom_qjs_module_cache_state_size(void) { return g_module_cache_state_size; }
VENOM_QJS_INTERNAL("venom_qjs_resolver_audit_ptr") uint32_t venom_qjs_resolver_audit_ptr(void) { return (uint32_t)(uintptr_t)g_resolver_audit; }
VENOM_QJS_INTERNAL("venom_qjs_resolver_audit_size") uint32_t venom_qjs_resolver_audit_size(void) { return g_resolver_audit_size; }
VENOM_QJS_PUBLIC("venom_qjs_interop_fallback_required") uint32_t venom_qjs_interop_fallback_required(void) { return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_script_record_ptr") uint32_t venom_qjs_script_record_ptr(void) { return (uint32_t)(uintptr_t)g_script_record; }
VENOM_QJS_INTERNAL("venom_qjs_script_record_size") uint32_t venom_qjs_script_record_size(void) { return g_script_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_script_record_hash") uint32_t venom_qjs_script_record_hash(void) { return fnv32(g_script_record, g_script_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_eval_result_ptr") uint32_t venom_qjs_eval_result_ptr(void) { return (uint32_t)(uintptr_t)g_eval_result; }
VENOM_QJS_INTERNAL("venom_qjs_eval_result_size") uint32_t venom_qjs_eval_result_size(void) { return g_eval_result_size; }
VENOM_QJS_INTERNAL("venom_qjs_eval_result_hash") uint32_t venom_qjs_eval_result_hash(void) { return fnv32(g_eval_result, g_eval_result_size); }
VENOM_QJS_INTERNAL("venom_qjs_console_capture_ptr") uint32_t venom_qjs_console_capture_ptr(void) { return (uint32_t)(uintptr_t)g_console_capture; }
VENOM_QJS_INTERNAL("venom_qjs_console_capture_size") uint32_t venom_qjs_console_capture_size(void) { return g_console_capture_size; }
VENOM_QJS_INTERNAL("venom_qjs_failure_report_ptr") uint32_t venom_qjs_failure_report_ptr(void) { return (uint32_t)(uintptr_t)g_failure_report; }
VENOM_QJS_INTERNAL("venom_qjs_failure_report_size") uint32_t venom_qjs_failure_report_size(void) { return g_failure_report_size; }
VENOM_QJS_INTERNAL("venom_qjs_execution_journal_ptr") uint32_t venom_qjs_execution_journal_ptr(void) { return (uint32_t)(uintptr_t)g_execution_journal; }
VENOM_QJS_INTERNAL("venom_qjs_execution_journal_size") uint32_t venom_qjs_execution_journal_size(void) { return g_execution_journal_size; }
VENOM_QJS_INTERNAL("venom_qjs_execution_journal_hash") uint32_t venom_qjs_execution_journal_hash(void) { return fnv32(g_execution_journal, g_execution_journal_size); }
VENOM_QJS_INTERNAL("venom_qjs_checkpoint_policy_ptr") uint32_t venom_qjs_checkpoint_policy_ptr(void) { return (uint32_t)(uintptr_t)g_checkpoint_policy_text; }
VENOM_QJS_INTERNAL("venom_qjs_checkpoint_policy_size") uint32_t venom_qjs_checkpoint_policy_size(void) { return cstr_size(g_checkpoint_policy_text); }
VENOM_QJS_INTERNAL("venom_qjs_checkpoint_policy_hash") uint32_t venom_qjs_checkpoint_policy_hash(void) { return fnv32(g_checkpoint_policy_text, cstr_size(g_checkpoint_policy_text)); }
VENOM_QJS_INTERNAL("venom_qjs_checkpoint_create") uint32_t venom_qjs_checkpoint_create(uint32_t ctx, uint32_t stage) { struct QjsContextAccount* rec = make_context(ctx); struct QjsCheckpointRecord* cp = alloc_checkpoint(); if (!rec || !cp) { build_exception_json(ctx, 70u, "checkpoint", "checkpoint capacity exceeded", stage, g_last_source_hash); return 0u; } cp->id = g_next_checkpoint_id++; cp->alive = 1u; cp->ctx = ctx; cp->stage = stage; cp->engine_state = g_engine_state; cp->source_hash = g_last_source_hash; cp->heap_used = rec->heap_used; cp->host_calls = g_host_call_count; cp->console_events = g_console_count; cp->module_records = g_module_record_count; g_last_checkpoint_id = cp->id; ++g_replay_sequence; build_execution_journal_json(ctx, stage, cp->id); build_replay_cursor_json(ctx, stage, cp->id); build_resume_state_json(ctx, cp->id, 1u); build_determinism_audit_json(ctx, 1u); venom_qjs_host_call_dispatch(15u, (uint32_t)(uintptr_t)g_execution_journal, g_execution_journal_size); return cp->id; }
VENOM_QJS_INTERNAL("venom_qjs_checkpoint_restore") uint32_t venom_qjs_checkpoint_restore(uint32_t ctx, uint32_t checkpoint_id) { struct QjsContextAccount* rec = make_context(ctx); struct QjsCheckpointRecord* cp = find_checkpoint(checkpoint_id); if (!rec || !cp || cp->ctx != ctx) { build_exception_json(ctx, 71u, "checkpoint", "checkpoint restore failed", checkpoint_id, g_last_source_hash); build_resume_state_json(ctx, checkpoint_id, 0u); return 0u; } rec->heap_used = cp->heap_used; g_engine_state = cp->engine_state; g_last_source_hash = cp->source_hash; g_host_call_count = cp->host_calls; g_console_count = cp->console_events; g_module_record_count = cp->module_records; g_last_checkpoint_id = checkpoint_id; build_resume_state_json(ctx, checkpoint_id, 1u); venom_qjs_host_call_dispatch(17u, (uint32_t)(uintptr_t)g_resume_state, g_resume_state_size); return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_replay_cursor_ptr") uint32_t venom_qjs_replay_cursor_ptr(void) { return (uint32_t)(uintptr_t)g_replay_cursor; }
VENOM_QJS_INTERNAL("venom_qjs_replay_cursor_size") uint32_t venom_qjs_replay_cursor_size(void) { return g_replay_cursor_size; }
VENOM_QJS_INTERNAL("venom_qjs_replay_cursor_hash") uint32_t venom_qjs_replay_cursor_hash(void) { return fnv32(g_replay_cursor, g_replay_cursor_size); }
VENOM_QJS_INTERNAL("venom_qjs_replay_advance") uint32_t venom_qjs_replay_advance(uint32_t ctx, uint32_t stage) { ++g_replay_sequence; build_replay_cursor_json(ctx, stage, g_last_checkpoint_id); venom_qjs_host_call_dispatch(16u, (uint32_t)(uintptr_t)g_replay_cursor, g_replay_cursor_size); return 1u; }

VENOM_QJS_INTERNAL("venom_qjs_snapshot_capture") uint32_t venom_qjs_snapshot_capture(uint32_t ctx, uint32_t stage) {
  build_snapshot_record_json(ctx, stage, g_last_checkpoint_id);
  build_determinism_ledger_json(ctx);
  build_audit_seal_json(ctx, 1u);
  venom_qjs_host_call_dispatch(19u, (uint32_t)(uintptr_t)g_snapshot_record, g_snapshot_record_size);
  venom_qjs_host_call_dispatch(21u, (uint32_t)(uintptr_t)g_determinism_ledger, g_determinism_ledger_size);
  venom_qjs_host_call_dispatch(22u, (uint32_t)(uintptr_t)g_audit_seal, g_audit_seal_size);
  return g_last_snapshot_hash;
}

VENOM_QJS_INTERNAL("venom_qjs_snapshot_validate") uint32_t venom_qjs_snapshot_validate(uint32_t ctx, uint32_t expected_hash) {
  ...
  venom_qjs_host_call_dispatch(20u, (uint32_t)(uintptr_t)g_replay_validation, g_replay_validation_size);
...
VENOM_QJS_INTERNAL("venom_qjs_resume_state_ptr") uint32_t venom_qjs_resume_state_ptr(void) { return (uint32_t)(uintptr_t)g_resume_state; }
VENOM_QJS_INTERNAL("venom_qjs_resume_state_size") uint32_t venom_qjs_resume_state_size(void) { return g_resume_state_size; }
VENOM_QJS_INTERNAL("venom_qjs_resume_state_hash") uint32_t venom_qjs_resume_state_hash(void) { return fnv32(g_resume_state, g_resume_state_size); }
VENOM_QJS_INTERNAL("venom_qjs_determinism_audit_ptr") uint32_t venom_qjs_determinism_audit_ptr(void) { return (uint32_t)(uintptr_t)g_determinism_audit; }
VENOM_QJS_INTERNAL("venom_qjs_determinism_audit_size") uint32_t venom_qjs_determinism_audit_size(void) { return g_determinism_audit_size; }
VENOM_QJS_INTERNAL("venom_qjs_determinism_audit_hash") uint32_t venom_qjs_determinism_audit_hash(void) { return fnv32(g_determinism_audit, g_determinism_audit_size); }

VENOM_QJS_INTERNAL("venom_qjs_snapshot_policy_ptr") uint32_t venom_qjs_snapshot_policy_ptr(void) { (void)memcpy(g_snapshot_policy, g_snapshot_policy_text, cstr_size(g_snapshot_policy_text)); return (uint32_t)(uintptr_t)g_snapshot_policy; }
VENOM_QJS_INTERNAL("venom_qjs_snapshot_policy_size") uint32_t venom_qjs_snapshot_policy_size(void) { return cstr_size(g_snapshot_policy_text); }
VENOM_QJS_INTERNAL("venom_qjs_snapshot_policy_hash") uint32_t venom_qjs_snapshot_policy_hash(void) { return fnv32(g_snapshot_policy_text, cstr_size(g_snapshot_policy_text)); }
VENOM_QJS_INTERNAL("venom_qjs_snapshot_record_ptr") uint32_t venom_qjs_snapshot_record_ptr(void) { return (uint32_t)(uintptr_t)g_snapshot_record; }
VENOM_QJS_INTERNAL("venom_qjs_snapshot_record_size") uint32_t venom_qjs_snapshot_record_size(void) { return g_snapshot_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_snapshot_record_hash") uint32_t venom_qjs_snapshot_record_hash(void) { return fnv32(g_snapshot_record, g_snapshot_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_replay_validation_ptr") uint32_t venom_qjs_replay_validation_ptr(void) { return (uint32_t)(uintptr_t)g_replay_validation; }
VENOM_QJS_INTERNAL("venom_qjs_replay_validation_size") uint32_t venom_qjs_replay_validation_size(void) { return g_replay_validation_size; }
VENOM_QJS_INTERNAL("venom_qjs_replay_validation_hash") uint32_t venom_qjs_replay_validation_hash(void) { return fnv32(g_replay_validation, g_replay_validation_size); }
VENOM_QJS_INTERNAL("venom_qjs_determinism_ledger_ptr") uint32_t venom_qjs_determinism_ledger_ptr(void) { return (uint32_t)(uintptr_t)g_determinism_ledger; }
VENOM_QJS_INTERNAL("venom_qjs_determinism_ledger_size") uint32_t venom_qjs_determinism_ledger_size(void) { return g_determinism_ledger_size; }
VENOM_QJS_INTERNAL("venom_qjs_determinism_ledger_hash") uint32_t venom_qjs_determinism_ledger_hash(void) { return fnv32(g_determinism_ledger, g_determinism_ledger_size); }
VENOM_QJS_INTERNAL("venom_qjs_audit_seal_ptr") uint32_t venom_qjs_audit_seal_ptr(void) { return (uint32_t)(uintptr_t)g_audit_seal; }
VENOM_QJS_INTERNAL("venom_qjs_audit_seal_size") uint32_t venom_qjs_audit_seal_size(void) { return g_audit_seal_size; }
VENOM_QJS_INTERNAL("venom_qjs_audit_seal_hash") uint32_t venom_qjs_audit_seal_hash(void) { return fnv32(g_audit_seal, g_audit_seal_size); }

VENOM_QJS_INTERNAL("venom_qjs_execution_commit_ptr") uint32_t venom_qjs_execution_commit_ptr(void) { return (uint32_t)(uintptr_t)g_execution_commit; }
VENOM_QJS_INTERNAL("venom_qjs_execution_commit_size") uint32_t venom_qjs_execution_commit_size(void) { return g_execution_commit_size; }
VENOM_QJS_INTERNAL("venom_qjs_execution_commit_hash") uint32_t venom_qjs_execution_commit_hash(void) { return fnv32(g_execution_commit, g_execution_commit_size); }
VENOM_QJS_INTERNAL("venom_qjs_execution_commit") uint32_t venom_qjs_execution_commit(uint32_t ctx, uint32_t checkpoint_id) { uint32_t cp = checkpoint_id ? checkpoint_id : g_last_checkpoint_id; if (!cp || !g_last_snapshot_valid) { build_exception_json(ctx, 90u, "commit", "execution commit rejected before valid snapshot", checkpoint_id, g_last_snapshot_hash); build_release_acceptance_json(ctx, 0u); build_commit_audit_json(ctx); return 0u; } build_execution_commit_json(ctx, cp); venom_qjs_host_call_dispatch(23u, (uint32_t)(uintptr_t)g_execution_commit, g_execution_commit_size); build_release_acceptance_json(ctx, 1u); venom_qjs_host_call_dispatch(25u, (uint32_t)(uintptr_t)g_host_receipts, g_host_receipts_size); venom_qjs_host_call_dispatch(26u, (uint32_t)(uintptr_t)g_release_acceptance, g_release_acceptance_size); build_commit_audit_json(ctx); return g_last_commit_hash; }
VENOM_QJS_INTERNAL("venom_qjs_rollback_policy_ptr") uint32_t venom_qjs_rollback_policy_ptr(void) { (void)memcpy(g_rollback_policy, g_rollback_policy_text, cstr_size(g_rollback_policy_text)); g_rollback_policy_size = cstr_size(g_rollback_policy_text); return (uint32_t)(uintptr_t)g_rollback_policy; }
VENOM_QJS_INTERNAL("venom_qjs_rollback_policy_size") uint32_t venom_qjs_rollback_policy_size(void) { return cstr_size(g_rollback_policy_text); }
VENOM_QJS_INTERNAL("venom_qjs_rollback_policy_hash") uint32_t venom_qjs_rollback_policy_hash(void) { return fnv32(g_rollback_policy_text, cstr_size(g_rollback_policy_text)); }
VENOM_QJS_INTERNAL("venom_qjs_execution_rollback") uint32_t venom_qjs_execution_rollback(uint32_t ctx, uint32_t checkpoint_id) { uint32_t ok = venom_qjs_checkpoint_restore(ctx, checkpoint_id); if (ok) venom_qjs_host_call_dispatch(24u, (uint32_t)(uintptr_t)g_resume_state, g_resume_state_size); return ok; }
VENOM_QJS_INTERNAL("venom_qjs_host_receipts_ptr") uint32_t venom_qjs_host_receipts_ptr(void) { return (uint32_t)(uintptr_t)g_host_receipts; }
VENOM_QJS_INTERNAL("venom_qjs_host_receipts_size") uint32_t venom_qjs_host_receipts_size(void) { return g_host_receipts_size; }
VENOM_QJS_INTERNAL("venom_qjs_host_receipts_hash") uint32_t venom_qjs_host_receipts_hash(void) { return fnv32(g_host_receipts, g_host_receipts_size); }
VENOM_QJS_INTERNAL("venom_qjs_release_acceptance_ptr") uint32_t venom_qjs_release_acceptance_ptr(void) { return (uint32_t)(uintptr_t)g_release_acceptance; }
VENOM_QJS_INTERNAL("venom_qjs_release_acceptance_size") uint32_t venom_qjs_release_acceptance_size(void) { return g_release_acceptance_size; }
VENOM_QJS_INTERNAL("venom_qjs_release_acceptance_hash") uint32_t venom_qjs_release_acceptance_hash(void) { return fnv32(g_release_acceptance, g_release_acceptance_size); }
VENOM_QJS_INTERNAL("venom_qjs_release_accept") uint32_t venom_qjs_release_accept(uint32_t ctx) { uint32_t ok = g_last_commit_hash != 0u && g_last_snapshot_valid != 0u && g_engine_trap_code == 0u; build_release_acceptance_json(ctx, ok); if (!ok) { build_exception_json(ctx, 91u, "release-acceptance", "release acceptance gate rejected execution", g_last_commit_hash, g_last_snapshot_hash); return 0u; } venom_qjs_host_call_dispatch(26u, (uint32_t)(uintptr_t)g_release_acceptance, g_release_acceptance_size); build_commit_audit_json(ctx); return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_commit_audit_ptr") uint32_t venom_qjs_commit_audit_ptr(void) { return (uint32_t)(uintptr_t)g_commit_audit; }
VENOM_QJS_INTERNAL("venom_qjs_commit_audit_size") uint32_t venom_qjs_commit_audit_size(void) { return g_commit_audit_size; }
VENOM_QJS_INTERNAL("venom_qjs_commit_audit_hash") uint32_t venom_qjs_commit_audit_hash(void) { return fnv32(g_commit_audit, g_commit_audit_size); }

VENOM_QJS_INTERNAL("venom_qjs_capability_policy_ptr") uint32_t venom_qjs_capability_policy_ptr(void) { return (uint32_t)(uintptr_t)g_capability_policy_text; }
VENOM_QJS_INTERNAL("venom_qjs_capability_policy_size") uint32_t venom_qjs_capability_policy_size(void) { return cstr_size(g_capability_policy_text); }
VENOM_QJS_INTERNAL("venom_qjs_capability_policy_hash") uint32_t venom_qjs_capability_policy_hash(void) { return fnv32(g_capability_policy_text, cstr_size(g_capability_policy_text)); }
VENOM_QJS_INTERNAL("venom_qjs_host_io_policy_ptr") uint32_t venom_qjs_host_io_policy_ptr(void) { return (uint32_t)(uintptr_t)g_host_io_policy_text; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_policy_size") uint32_t venom_qjs_host_io_policy_size(void) { return cstr_size(g_host_io_policy_text); }
VENOM_QJS_INTERNAL("venom_qjs_host_io_policy_hash") uint32_t venom_qjs_host_io_policy_hash(void) { return fnv32(g_host_io_policy_text, cstr_size(g_host_io_policy_text)); }
VENOM_QJS_INTERNAL("venom_qjs_policy_receipts_ptr") uint32_t venom_qjs_policy_receipts_ptr(void) { return (uint32_t)(uintptr_t)g_policy_receipts; }
VENOM_QJS_INTERNAL("venom_qjs_policy_receipts_size") uint32_t venom_qjs_policy_receipts_size(void) { return g_policy_receipts_size; }
VENOM_QJS_INTERNAL("venom_qjs_policy_receipts_hash") uint32_t venom_qjs_policy_receipts_hash(void) { return fnv32(g_policy_receipts, g_policy_receipts_size); }
VENOM_QJS_INTERNAL("venom_qjs_permission_seal_ptr") uint32_t venom_qjs_permission_seal_ptr(void) { return (uint32_t)(uintptr_t)g_permission_seal; }
VENOM_QJS_INTERNAL("venom_qjs_permission_seal_size") uint32_t venom_qjs_permission_seal_size(void) { return g_permission_seal_size; }
VENOM_QJS_INTERNAL("venom_qjs_permission_seal_hash") uint32_t venom_qjs_permission_seal_hash(void) { return g_permission_seal_size ? fnv32(g_permission_seal, g_permission_seal_size) : g_permission_seal_hash; }
VENOM_QJS_INTERNAL("venom_qjs_capability_check") uint32_t venom_qjs_capability_check(uint32_t ctx, uint32_t capability_id) { (void)ctx; uint32_t ok = capability_allowed(capability_id); build_policy_receipts_json(capability_id, ok, g_last_source_hash); venom_qjs_host_call_dispatch(27u, (uint32_t)(uintptr_t)g_policy_receipts, g_policy_receipts_size); return ok; }
VENOM_QJS_INTERNAL("venom_qjs_permission_seal") uint32_t venom_qjs_permission_seal(uint32_t ctx) { build_capability_ledger_json(ctx); build_permission_seal_json(ctx); build_policy_seal_audit_json(ctx); venom_qjs_host_call_dispatch(29u, (uint32_t)(uintptr_t)g_permission_seal, g_permission_seal_size); venom_qjs_host_call_dispatch(34u, (uint32_t)(uintptr_t)g_policy_seal_audit, g_policy_seal_audit_size); return g_permission_seal_hash; }
VENOM_QJS_INTERNAL("venom_qjs_release_gate_ptr") uint32_t venom_qjs_release_gate_ptr(void) { return (uint32_t)(uintptr_t)g_release_gate; }
VENOM_QJS_INTERNAL("venom_qjs_release_gate_size") uint32_t venom_qjs_release_gate_size(void) { return g_release_gate_size; }
VENOM_QJS_INTERNAL("venom_qjs_release_gate_hash") uint32_t venom_qjs_release_gate_hash(void) { return fnv32(g_release_gate, g_release_gate_size); }
VENOM_QJS_INTERNAL("venom_qjs_release_gate") uint32_t venom_qjs_release_gate(uint32_t ctx) { uint32_t ok = g_release_accepted && g_permission_seal_hash != 0u && g_policy_receipt_count != 0u && g_host_io_denial_count == 0u && g_engine_trap_code == 0u; build_release_gate_json(ctx, ok); if (!ok) { build_exception_json(ctx, 101u, "release-gate", "release gate rejected unsealed or denied QuickJS host I/O policy", g_permission_seal_hash, g_policy_receipt_count); return 0u; } venom_qjs_host_call_dispatch(30u, (uint32_t)(uintptr_t)g_release_gate, g_release_gate_size); return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_decision_ptr") uint32_t venom_qjs_host_io_decision_ptr(void) { return (uint32_t)(uintptr_t)g_host_io_decision; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_decision_size") uint32_t venom_qjs_host_io_decision_size(void) { return g_host_io_decision_size; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_decision_hash") uint32_t venom_qjs_host_io_decision_hash(void) { return fnv32(g_host_io_decision, g_host_io_decision_size); }
VENOM_QJS_INTERNAL("venom_qjs_host_io_request") uint32_t venom_qjs_host_io_request(uint32_t ctx, uint32_t io_class, uint32_t capability_id, uint32_t payload_hash) { uint32_t policy_ok = (io_class == 1u || io_class == 2u) && capability_allowed(capability_id); build_policy_receipts_json(capability_id, policy_ok, payload_hash); build_host_io_decision_json(ctx, io_class, capability_id, policy_ok, payload_hash); if (!policy_ok) { build_host_io_deny_trace_json(ctx, io_class, capability_id, payload_hash, "host I/O rejected by runtime denylist"); build_capability_ledger_json(ctx); venom_qjs_host_call_dispatch(32u, (uint32_t)(uintptr_t)g_host_io_deny_trace, g_host_io_deny_trace_size); return 0u; } build_capability_ledger_json(ctx); venom_qjs_host_call_dispatch(31u, (uint32_t)(uintptr_t)g_host_io_decision, g_host_io_decision_size); venom_qjs_host_call_dispatch(33u, (uint32_t)(uintptr_t)g_capability_ledger, g_capability_ledger_size); return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_deny_trace_ptr") uint32_t venom_qjs_host_io_deny_trace_ptr(void) { return (uint32_t)(uintptr_t)g_host_io_deny_trace; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_deny_trace_size") uint32_t venom_qjs_host_io_deny_trace_size(void) { return g_host_io_deny_trace_size; }
VENOM_QJS_INTERNAL("venom_qjs_host_io_deny_trace_hash") uint32_t venom_qjs_host_io_deny_trace_hash(void) { return fnv32(g_host_io_deny_trace, g_host_io_deny_trace_size); }
VENOM_QJS_INTERNAL("venom_qjs_capability_ledger_ptr") uint32_t venom_qjs_capability_ledger_ptr(void) { return (uint32_t)(uintptr_t)g_capability_ledger; }
VENOM_QJS_INTERNAL("venom_qjs_capability_ledger_size") uint32_t venom_qjs_capability_ledger_size(void) { return g_capability_ledger_size; }
VENOM_QJS_INTERNAL("venom_qjs_capability_ledger_hash") uint32_t venom_qjs_capability_ledger_hash(void) { return g_capability_ledger_hash ? g_capability_ledger_hash : fnv32(g_capability_ledger, g_capability_ledger_size); }
VENOM_QJS_INTERNAL("venom_qjs_policy_seal_audit_ptr") uint32_t venom_qjs_policy_seal_audit_ptr(void) { return (uint32_t)(uintptr_t)g_policy_seal_audit; }
VENOM_QJS_INTERNAL("venom_qjs_policy_seal_audit_size") uint32_t venom_qjs_policy_seal_audit_size(void) { return g_policy_seal_audit_size; }
VENOM_QJS_INTERNAL("venom_qjs_policy_seal_audit_hash") uint32_t venom_qjs_policy_seal_audit_hash(void) { return g_policy_seal_audit_hash ? g_policy_seal_audit_hash : fnv32(g_policy_seal_audit, g_policy_seal_audit_size); }
VENOM_QJS_INTERNAL("venom_qjs_runtime_denylist_ptr") uint32_t venom_qjs_runtime_denylist_ptr(void) { return (uint32_t)(uintptr_t)g_runtime_denylist_text; }
VENOM_QJS_INTERNAL("venom_qjs_runtime_denylist_size") uint32_t venom_qjs_runtime_denylist_size(void) { return cstr_size(g_runtime_denylist_text); }
VENOM_QJS_INTERNAL("venom_qjs_runtime_denylist_hash") uint32_t venom_qjs_runtime_denylist_hash(void) { return fnv32(g_runtime_denylist_text, cstr_size(g_runtime_denylist_text)); }
VENOM_QJS_INTERNAL("venom_qjs_exception_abi") uint32_t venom_qjs_exception_abi(void) { return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_exception_ptr") uint32_t venom_qjs_exception_ptr(void) { return (uint32_t)(uintptr_t)g_exception; }
VENOM_QJS_PUBLIC("venom_qjs_exception_size") uint32_t venom_qjs_exception_size(void) { return g_exception_size; }
VENOM_QJS_PUBLIC("venom_qjs_exception_code") uint32_t venom_qjs_exception_code(void) { return g_exception_code; }
VENOM_QJS_PUBLIC("venom_qjs_exception_message_ptr") uint32_t venom_qjs_exception_message_ptr(void) { return (uint32_t)(uintptr_t)g_exception_message; }
VENOM_QJS_PUBLIC("venom_qjs_exception_message_size") uint32_t venom_qjs_exception_message_size(void) { return g_exception_message_size; }
VENOM_QJS_PUBLIC("venom_qjs_exception_clear") uint32_t venom_qjs_exception_clear(void) { uint32_t code = g_exception_code; clear_exception(); if (g_engine_state == STATE_TRAPPED) g_engine_state = STATE_LOADED; return code; }
VENOM_QJS_INTERNAL("venom_qjs_host_trap_policy_ptr") uint32_t venom_qjs_host_trap_policy_ptr(void) { return (uint32_t)(uintptr_t)g_host_trap_policy; }
VENOM_QJS_INTERNAL("venom_qjs_host_trap_policy_size") uint32_t venom_qjs_host_trap_policy_size(void) { return cstr_size(g_host_trap_policy); }
VENOM_QJS_INTERNAL("venom_qjs_host_trap_policy_hash") uint32_t venom_qjs_host_trap_policy_hash(void) { return fnv32(g_host_trap_policy, cstr_size(g_host_trap_policy)); }

VENOM_QJS_INTERNAL("venom_qjs_source_ptr") uint32_t venom_qjs_source_ptr(void) { return (uint32_t)(uintptr_t)g_source; }
VENOM_QJS_INTERNAL("venom_qjs_source_capacity") uint32_t venom_qjs_source_capacity(void) { return SOURCE_CAP; }
VENOM_QJS_PUBLIC("venom_qjs_compact_result_ptr") uint32_t venom_qjs_compact_result_ptr(void) {
  uint32_t logical[8];
  uint32_t i;
  ...
  logical[7] = g_last_status_code == QJS_STATUS_OK ? 1u : 0u;
  for (i = 0u; i < 8u; ++i) g_compact_result[g_result_field_wire[i]] = logical[i];
  return (uint32_t)(uintptr_t)g_compact_result;
}
VENOM_QJS_PUBLIC("venom_qjs_result_ptr") uint32_t venom_qjs_result_ptr(void) { return (uint32_t)(uintptr_t)g_result; }
VENOM_QJS_PUBLIC("venom_qjs_result_size") uint32_t venom_qjs_result_size(void) { return g_result_size; }
VENOM_QJS_PUBLIC("venom_qjs_execution_record_ptr") uint32_t venom_qjs_execution_record_ptr(void) { return (uint32_t)(uintptr_t)g_record; }
VENOM_QJS_PUBLIC("venom_qjs_execution_record_size") uint32_t venom_qjs_execution_record_size(void) { return g_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_last_context") uint32_t venom_qjs_last_context(void) { return g_last_context; }
VENOM_QJS_INTERNAL("venom_qjs_last_source_size") uint32_t venom_qjs_last_source_size(void) { return g_last_source_size; }
VENOM_QJS_INTERNAL("venom_qjs_last_source_hash") uint32_t venom_qjs_last_source_hash(void) { return g_last_source_hash; }
VENOM_QJS_INTERNAL("venom_qjs_console_count") uint32_t venom_qjs_console_count(void) { return g_console_count; }
VENOM_QJS_INTERNAL("venom_qjs_console_callback_abi") uint32_t venom_qjs_console_callback_abi(void) { return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_console_event_ptr") uint32_t venom_qjs_console_event_ptr(void) { return (uint32_t)(uintptr_t)g_console; }
VENOM_QJS_PUBLIC("venom_qjs_console_event_size") uint32_t venom_qjs_console_event_size(void) { return g_console_size; }
VENOM_QJS_PUBLIC("venom_qjs_console_event_count") uint32_t venom_qjs_console_event_count(void) { return g_console_count; }
VENOM_QJS_PUBLIC("venom_qjs_console_clear") uint32_t venom_qjs_console_clear(void) { uint32_t n = g_console_count; g_console_count = 0u; g_console_size = 0u; return n; }
VENOM_QJS_PUBLIC("venom_qjs_fallback_required") uint32_t venom_qjs_fallback_required(void) { return g_fallback_required; }
VENOM_QJS_PUBLIC("venom_qjs_backend_kind") uint32_t venom_qjs_backend_kind(void) { return g_backend_kind; }
VENOM_QJS_PUBLIC("venom_qjs_backend_ready") uint32_t venom_qjs_backend_ready(void) { return g_backend_ready; }
VENOM_QJS_PUBLIC("venom_qjs_real_engine_candidate") uint32_t venom_qjs_real_engine_candidate(void) { return g_real_engine_candidate; }


VENOM_QJS_PUBLIC("venom_qjs_status_code") uint32_t venom_qjs_status_code(void) { return g_last_status_code; }
VENOM_QJS_PUBLIC("venom_qjs_status_record_ptr") uint32_t venom_qjs_status_record_ptr(void) { return (uint32_t)(uintptr_t)g_status_record; }
VENOM_QJS_PUBLIC("venom_qjs_status_record_size") uint32_t venom_qjs_status_record_size(void) { return g_status_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_status_record_hash") uint32_t venom_qjs_status_record_hash(void) { return fnv32(g_status_record, g_status_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_runtime_limits_ptr") uint32_t venom_qjs_runtime_limits_ptr(void) { return (uint32_t)(uintptr_t)g_runtime_limits_text; }
VENOM_QJS_INTERNAL("venom_qjs_runtime_limits_size") uint32_t venom_qjs_runtime_limits_size(void) { return cstr_size(g_runtime_limits_text); }
VENOM_QJS_INTERNAL("venom_qjs_runtime_limits_hash") uint32_t venom_qjs_runtime_limits_hash(void) { return fnv32(g_runtime_limits_text, cstr_size(g_runtime_limits_text)); }
VENOM_QJS_INTERNAL("venom_qjs_context_report_ptr") uint32_t venom_qjs_context_report_ptr(uint32_t ctx) { build_context_report_json(ctx); return (uint32_t)(uintptr_t)g_context_report; }
VENOM_QJS_INTERNAL("venom_qjs_context_report_size") uint32_t venom_qjs_context_report_size(void) { return g_context_report_size; }
VENOM_QJS_INTERNAL("venom_qjs_context_report_hash") uint32_t venom_qjs_context_report_hash(void) { return fnv32(g_context_report, g_context_report_size); }
VENOM_QJS_PUBLIC("venom_qjs_bytecode_record_hash32") uint32_t venom_qjs_bytecode_record_hash32(void) { return g_bytecode_record_hash32; }
VENOM_QJS_PUBLIC("venom_qjs_bytecode_payload_size") uint32_t venom_qjs_bytecode_payload_size(void) { return g_bytecode_payload_size; }
VENOM_QJS_PUBLIC("venom_qjs_bytecode_expected_source_hash32") uint32_t venom_qjs_bytecode_expected_source_hash32(void) { return g_bytecode_expected_source_hash32; }
VENOM_QJS_INTERNAL("venom_qjs_backend_contract_ptr") uint32_t venom_qjs_backend_contract_ptr(void) { return (uint32_t)(uintptr_t)g_backend_contract_text; }
VENOM_QJS_INTERNAL("venom_qjs_backend_contract_size") uint32_t venom_qjs_backend_contract_size(void) { return cstr_size(g_backend_contract_text); }
VENOM_QJS_INTERNAL("venom_qjs_backend_contract_hash") uint32_t venom_qjs_backend_contract_hash(void) { return fnv32(g_backend_contract_text, cstr_size(g_backend_contract_text)); }
VENOM_QJS_PUBLIC("venom_qjs_release_status") uint32_t venom_qjs_release_status(void) { return g_last_status_code == QJS_STATUS_OK ? 1u : 0u; }
VENOM_QJS_PUBLIC("venom_qjs_interpreter_ready") uint32_t venom_qjs_interpreter_ready(void) { return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_contract_ptr") uint32_t venom_qjs_interpreter_contract_ptr(void) { return (uint32_t)(uintptr_t)g_interpreter_contract_text; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_contract_size") uint32_t venom_qjs_interpreter_contract_size(void) { return cstr_size(g_interpreter_contract_text); }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_contract_hash") uint32_t venom_qjs_interpreter_contract_hash(void) { return fnv32(g_interpreter_contract_text, cstr_size(g_interpreter_contract_text)); }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_dispatch_count") uint32_t venom_qjs_interpreter_dispatch_count(void) { return g_interpreter_dispatch_count; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_opcode_count") uint32_t venom_qjs_interpreter_opcode_count(void) { return g_interpreter_opcode_count; }
VENOM_QJS_INTERNAL("venom_qjs_global_slot_count") uint32_t venom_qjs_global_slot_count(void) { return g_global_slot_count; }
VENOM_QJS_INTERNAL("venom_qjs_last_global_write_hash") uint32_t venom_qjs_last_global_write_hash(void) { return g_last_global_write_hash; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_semantic_pass_count") uint32_t venom_qjs_interpreter_semantic_pass_count(void) { return g_interpreter_semantic_pass_count; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_property_write_count") uint32_t venom_qjs_interpreter_property_write_count(void) { return g_interpreter_property_write_count; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_builtin_probe_count") uint32_t venom_qjs_interpreter_builtin_probe_count(void) { return g_interpreter_builtin_probe_count; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_console_call_count") uint32_t venom_qjs_interpreter_console_call_count(void) { return g_interpreter_console_call_count; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_semantic_record_ptr") uint32_t venom_qjs_interpreter_semantic_record_ptr(void) { return (uint32_t)(uintptr_t)g_semantic_record; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_semantic_record_size") uint32_t venom_qjs_interpreter_semantic_record_size(void) { return g_semantic_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_interpreter_semantic_record_hash") uint32_t venom_qjs_interpreter_semantic_record_hash(void) { return fnv32(g_semantic_record, g_semantic_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_global_slot_record_ptr") uint32_t venom_qjs_global_slot_record_ptr(void) { return (uint32_t)(uintptr_t)g_global_slot_record; }
VENOM_QJS_INTERNAL("venom_qjs_global_slot_record_size") uint32_t venom_qjs_global_slot_record_size(void) { return g_global_slot_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_global_slot_record_hash") uint32_t venom_qjs_global_slot_record_hash(void) { return fnv32(g_global_slot_record, g_global_slot_record_size); }
VENOM_QJS_PUBLIC("venom_qjs_upstream_quickjs_ready") uint32_t venom_qjs_upstream_quickjs_ready(void) { return 1u; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_parity_record_ptr") uint32_t venom_qjs_upstream_parity_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_parity_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_parity_record_size") uint32_t venom_qjs_upstream_parity_record_size(void) { return g_upstream_parity_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_parity_record_hash") uint32_t venom_qjs_upstream_parity_record_hash(void) { return fnv32(g_upstream_parity_record, g_upstream_parity_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_feature_count") uint32_t venom_qjs_upstream_feature_count(void) { return g_upstream_feature_count; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_builtin_count") uint32_t venom_qjs_upstream_builtin_count(void) { return g_upstream_builtin_count; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_object_model_score") uint32_t venom_qjs_upstream_object_model_score(void) { return g_upstream_object_model_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_exception_model_score") uint32_t venom_qjs_upstream_exception_model_score(void) { return g_upstream_exception_model_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_module_model_score") uint32_t venom_qjs_upstream_module_model_score(void) { return g_upstream_module_model_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_object_record_ptr") uint32_t venom_qjs_upstream_object_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_object_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_object_record_size") uint32_t venom_qjs_upstream_object_record_size(void) { return g_upstream_object_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_object_record_hash") uint32_t venom_qjs_upstream_object_record_hash(void) { return g_upstream_object_record_hash ? g_upstream_object_record_hash : fnv32(g_upstream_object_record, g_upstream_object_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_exception_record_ptr") uint32_t venom_qjs_upstream_exception_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_exception_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_exception_record_size") uint32_t venom_qjs_upstream_exception_record_size(void) { return g_upstream_exception_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_exception_record_hash") uint32_t venom_qjs_upstream_exception_record_hash(void) { return g_upstream_exception_record_hash ? g_upstream_exception_record_hash : fnv32(g_upstream_exception_record, g_upstream_exception_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_module_record_ptr") uint32_t venom_qjs_upstream_module_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_module_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_module_record_size") uint32_t venom_qjs_upstream_module_record_size(void) { return g_upstream_module_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_module_record_hash") uint32_t venom_qjs_upstream_module_record_hash(void) { return g_upstream_module_record_hash ? g_upstream_module_record_hash : fnv32(g_upstream_module_record, g_upstream_module_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_runtime_bridge_score") uint32_t venom_qjs_upstream_runtime_bridge_score(void) { return g_upstream_runtime_bridge_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_bytecode_semantics_record_ptr") uint32_t venom_qjs_upstream_bytecode_semantics_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_bytecode_semantics_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_bytecode_semantics_record_size") uint32_t venom_qjs_upstream_bytecode_semantics_record_size(void) { return g_upstream_bytecode_semantics_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_bytecode_semantics_record_hash") uint32_t venom_qjs_upstream_bytecode_semantics_record_hash(void) { return g_upstream_bytecode_semantics_hash ? g_upstream_bytecode_semantics_hash : fnv32(g_upstream_bytecode_semantics_record, g_upstream_bytecode_semantics_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_bytecode_semantics_score") uint32_t venom_qjs_upstream_bytecode_semantics_score(void) { return g_upstream_bytecode_semantics_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_lexical_scope_record_ptr") uint32_t venom_qjs_upstream_lexical_scope_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_lexical_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_lexical_scope_record_size") uint32_t venom_qjs_upstream_lexical_scope_record_size(void) { return g_upstream_lexical_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_lexical_scope_record_hash") uint32_t venom_qjs_upstream_lexical_scope_record_hash(void) { return g_upstream_lexical_record_hash ? g_upstream_lexical_record_hash : fnv32(g_upstream_lexical_record, g_upstream_lexical_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_closure_record_ptr") uint32_t venom_qjs_upstream_closure_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_closure_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_closure_record_size") uint32_t venom_qjs_upstream_closure_record_size(void) { return g_upstream_closure_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_closure_record_hash") uint32_t venom_qjs_upstream_closure_record_hash(void) { return g_upstream_closure_record_hash ? g_upstream_closure_record_hash : fnv32(g_upstream_closure_record, g_upstream_closure_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_iterator_record_ptr") uint32_t venom_qjs_upstream_iterator_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_iterator_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_iterator_record_size") uint32_t venom_qjs_upstream_iterator_record_size(void) { return g_upstream_iterator_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_iterator_record_hash") uint32_t venom_qjs_upstream_iterator_record_hash(void) { return g_upstream_iterator_record_hash ? g_upstream_iterator_record_hash : fnv32(g_upstream_iterator_record, g_upstream_iterator_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_intrinsic_record_ptr") uint32_t venom_qjs_upstream_intrinsic_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_intrinsic_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_intrinsic_record_size") uint32_t venom_qjs_upstream_intrinsic_record_size(void) { return g_upstream_intrinsic_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_intrinsic_record_hash") uint32_t venom_qjs_upstream_intrinsic_record_hash(void) { return g_upstream_intrinsic_record_hash ? g_upstream_intrinsic_record_hash : fnv32(g_upstream_intrinsic_record, g_upstream_intrinsic_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_intrinsic_semantics_score") uint32_t venom_qjs_upstream_intrinsic_semantics_score(void) { return g_upstream_intrinsic_semantics_score; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_property_descriptor_record_ptr") uint32_t venom_qjs_upstream_property_descriptor_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_property_descriptor_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_property_descriptor_record_size") uint32_t venom_qjs_upstream_property_descriptor_record_size(void) { return g_upstream_property_descriptor_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_property_descriptor_record_hash") uint32_t venom_qjs_upstream_property_descriptor_record_hash(void) { return g_upstream_property_descriptor_record_hash ? g_upstream_property_descriptor_record_hash : fnv32(g_upstream_property_descriptor_record, g_upstream_property_descriptor_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_prototype_chain_record_ptr") uint32_t venom_qjs_upstream_prototype_chain_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_prototype_chain_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_prototype_chain_record_size") uint32_t venom_qjs_upstream_prototype_chain_record_size(void) { return g_upstream_prototype_chain_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_prototype_chain_record_hash") uint32_t venom_qjs_upstream_prototype_chain_record_hash(void) { return g_upstream_prototype_chain_record_hash ? g_upstream_prototype_chain_record_hash : fnv32(g_upstream_prototype_chain_record, g_upstream_prototype_chain_record_size); }
VENOM_QJS_INTERNAL("venom_qjs_upstream_call_frame_record_ptr") uint32_t venom_qjs_upstream_call_frame_record_ptr(void) { return (uint32_t)(uintptr_t)g_upstream_call_frame_record; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_call_frame_record_size") uint32_t venom_qjs_upstream_call_frame_record_size(void) { return g_upstream_call_frame_record_size; }
VENOM_QJS_INTERNAL("venom_qjs_upstream_call_frame_record_hash") uint32_t venom_qjs_upstream_call_frame_record_hash(void) { return g_upstream_call_frame_record_hash ? g_upstream_call_frame_record_hash : fnv32(g_upstream_call_frame_record, g_upstream_call_frame_record_size); }

VENOM_QJS_PUBLIC("venom_qjs_context_alloc") uint32_t venom_qjs_context_alloc(void) { uint32_t id = g_context_counter++; g_engine_state = STATE_LOADED; return make_context(id) ? id : 0u; }
VENOM_QJS_PUBLIC("venom_qjs_context_free") uint32_t venom_qjs_context_free(uint32_t ctx) { struct QjsContextAccount* rec = find_context(ctx); if (!rec) return 0u; rec->alive = 0u; rec->heap_used = 0u; rec->script_bytes = 0u; rec->interrupt_ticks = 0u; rec->pending_jobs = 0u; if (g_active_script_ctx == ctx) { if (g_script_buffer_size) venom_qjs_secure_zero(g_source, g_script_buffer_size); g_active_script_ctx = 0u; g_script_buffer_size = 0u; g_script_buffer_expected_hash = 0u; } if (g_bridge_active_ctx == ctx) { if (g_bridge_input_size) venom_qjs_secure_zero(g_bridge_input, g_bridge_input_size); if (g_bridge_result_size) venom_qjs_secure_zero(g_bridge_result, g_bridge_result_size); g_bridge_input_size = 0u; g_bridge_result_size = 0u; g_bridge_active_ctx = 0u; }
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
  if (g_upstream_qjs_context) { JS_FreeContext(g_upstream_qjs_context); g_upstream_qjs_context = 0; }
  if (g_upstream_qjs_runtime) { JS_FreeRuntime(g_upstream_qjs_runtime); g_upstream_qjs_runtime = 0; }
#endif
  return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_context_set_limits") uint32_t venom_qjs_context_set_limits(uint32_t ctx, uint32_t heap_limit, uint32_t stack_limit) { struct QjsContextAccount* rec = make_context(ctx); if (!rec) { g_last_status_code = QJS_STATUS_BAD_CONTEXT; build_status_record_json(ctx, g_last_status_code, "context", 0u, 0u); return 0u; } rec->heap_limit = heap_limit ? heap_limit : DEFAULT_HEAP_LIMIT; rec->stack_limit = stack_limit ? stack_limit : DEFAULT_STACK_LIMIT; if (rec->heap_limit > DEFAULT_HEAP_LIMIT) rec->heap_limit = DEFAULT_HEAP_LIMIT; if (rec->stack_limit > DEFAULT_STACK_LIMIT) rec->stack_limit = DEFAULT_STACK_LIMIT; if (rec->heap_used > rec->heap_limit) { g_last_status_code = QJS_STATUS_ALLOC_LIMIT; build_status_record_json(ctx, g_last_status_code, "context", rec->heap_used, rec->heap_limit); return 0u; } g_last_status_code = QJS_STATUS_OK; build_status_record_json(ctx, g_last_status_code, "context", rec->heap_limit, rec->stack_limit); g_engine_state = STATE_CONFIGURED; return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_context_set_execution_limits") uint32_t venom_qjs_context_set_execution_limits(uint32_t ctx, uint32_t interrupt_budget, uint32_t pending_job_limit) {
  struct QjsContextAccount* rec = make_context(ctx);
  if (!rec) return 0u;
...
VENOM_QJS_PUBLIC("venom_qjs_context_heap_limit") uint32_t venom_qjs_context_heap_limit(uint32_t ctx) { struct QjsContextAccount* rec = find_context(ctx); return rec ? rec->heap_limit : 0u; }
VENOM_QJS_PUBLIC("venom_qjs_context_heap_used") uint32_t venom_qjs_context_heap_used(uint32_t ctx) { struct QjsContextAccount* rec = find_context(ctx); return rec ? rec->heap_used : 0u; }
VENOM_QJS_PUBLIC("venom_qjs_context_stack_limit") uint32_t venom_qjs_context_stack_limit(uint32_t ctx) { struct QjsContextAccount* rec = find_context(ctx); return rec ? rec->stack_limit : 0u; }

VENOM_QJS_PUBLIC("venom_qjs_script_buffer_alloc") uint32_t venom_qjs_script_buffer_alloc(uint32_t ctx, uint32_t size) {
  struct QjsContextAccount* rec = make_context(ctx);
  if (!rec || size == 0u || size > SOURCE_CAP || size > rec->heap_limit) { g_last_status_code = !rec ? QJS_STATUS_BAD_CONTEXT : QJS_STATUS_ALLOC_LIMIT; build_status_record_json(ctx, g_last_status_code, "script-buffer", size, 0u); build_exception_json(ctx, 40u, "script-buffer", size == 0u ? "zero-size script buffer rejected" : "script buffer allocation limit exceeded", size, 0u); return 0u; }
  if (g_script_buffer_size) venom_qjs_secure_zero(g_source, g_script_buffer_size);
  ...
  g_last_status_code = QJS_STATUS_OK;
  build_status_record_json(ctx, g_last_status_code, "script-buffer", size, fnv32(g_source, 0u));
  g_engine_state = STATE_LOADED;
  return (uint32_t)(uintptr_t)g_source;
}
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_ptr") uint32_t venom_qjs_script_buffer_ptr(void) { return (uint32_t)(uintptr_t)g_source; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_size") uint32_t venom_qjs_script_buffer_size(void) { return g_script_buffer_size; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_capacity") uint32_t venom_qjs_script_buffer_capacity(void) { return SOURCE_CAP; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_set_expected_hash") uint32_t venom_qjs_script_buffer_set_expected_hash(uint32_t ctx, uint32_t hash) { (void)ctx; g_script_buffer_expected_hash = hash; return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_expected_hash") uint32_t venom_qjs_script_buffer_expected_hash(void) { return g_script_buffer_expected_hash; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_alloc_count") uint32_t venom_qjs_script_buffer_alloc_count(void) { return g_script_buffer_alloc_count; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_free_count") uint32_t venom_qjs_script_buffer_free_count(void) { return g_script_buffer_free_count; }
VENOM_QJS_PUBLIC("venom_qjs_script_buffer_free") uint32_t venom_qjs_script_buffer_free(uint32_t ctx, uint32_t ptr) { (void)ptr; struct QjsContextAccount* rec = find_context(ctx); if (!rec) return 0u; if (g_active_script_ctx == ctx && g_script_buffer_size) venom_qjs_secure_zero(g_source, g_script_buffer_size); rec->heap_used = 0u; rec->script_bytes = 0u; rec->interrupt_ticks = 0u; rec->pending_jobs = 0u; if (g_active_script_ctx == ctx) { g_active_script_ctx = 0u; g_script_buffer_size = 0u; g_script_buffer_expected_hash = 0u; } ++g_script_buffer_free_count; return 1u; }


VENOM_QJS_PUBLIC("venom_qjs_bridge_abi") uint32_t venom_qjs_bridge_abi(void) { return 1u; }
VENOM_QJS_PUBLIC("venom_qjs_bridge_input_alloc") uint32_t venom_qjs_bridge_input_alloc(uint32_t ctx, uint32_t size) {
  struct QjsContextAccount* rec = find_context(ctx);
  if (!rec || size == 0u || size > BRIDGE_INPUT_CAP) return 0u;
  if (g_bridge_input_size) venom_qjs_secure_zero(g_bridge_input, g_bridge_input_size);
  if (g_bridge_result_size) venom_qjs_secure_zero(g_bridge_result, g_bridge_result_size);
  g_bridge_input_size = size;
  g_bridge_result_size = 0u;
  g_bridge_active_ctx = ctx;
  return (uint32_t)(uintptr_t)g_bridge_input;
}
VENOM_QJS_PUBLIC("venom_qjs_bridge_input_capacity") uint32_t venom_qjs_bridge_input_capacity(void) { return BRIDGE_INPUT_CAP; }
VENOM_QJS_PUBLIC("venom_qjs_bridge_result_ptr") uint32_t venom_qjs_bridge_result_ptr(void) { return (uint32_t)(uintptr_t)g_bridge_result; }
VENOM_QJS_PUBLIC("venom_qjs_bridge_result_size") uint32_t venom_qjs_bridge_result_size(void) { return g_bridge_result_size; }
VENOM_QJS_PUBLIC("venom_qjs_bridge_release") uint32_t venom_qjs_bridge_release(uint32_t ctx) {
...
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
static uint32_t venom_qjs_bridge_copy_json_result(uint32_t ctx, JSValue value) {
  JSValue json = JS_JSONStringify(g_upstream_qjs_context, value, JS_UNDEFINED, JS_UNDEFINED);
  if (JS_IsException(json)) {
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, g_bridge_input_size, 0u);
    JS_FreeValue(g_upstream_qjs_context, json);
    return 0u;
  }
  size_t size = 0u;
  const char* text = JS_ToCStringLen(g_upstream_qjs_context, &size, json);
  ...
  JS_FreeCString(g_upstream_qjs_context, text);
  JS_FreeValue(g_upstream_qjs_context, json);
  return 1u;
}
#endif

VENOM_QJS_PUBLIC("venom_qjs_bridge_invoke") uint32_t venom_qjs_bridge_invoke(uint32_t ctx, uint32_t size) {
  struct QjsContextAccount* rec = find_context(ctx);
...
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
  if (!venom_qjs_ensure_upstream_runtime(rec)) {
    build_exception_json(ctx, 90u, "bridge", "failed to initialize upstream QuickJS runtime", size, 0u);
    return 0u;
  }
  JSValue envelope = JS_ParseJSON(g_upstream_qjs_context, (const char*)g_bridge_input, size, "<venom-bridge-request>");
  if (JS_IsException(envelope)) {
    venom_qjs_copy_exception_message(g_upstream_qjs_context, ctx, size, 0u);
    JS_FreeValue(g_upstream_qjs_context, envelope);
    return 0u;
  }
  JSValue candidate = JS_GetPropertyStr(g_upstream_qjs_context, envelope, "candidate");
  JSValue args = JS_GetPropertyStr(g_upstream_qjs_context, envelope, "args");
  const char* candidate_name = JS_ToCString(g_upstream_qjs_context, candidate);
  if (!candidate_name || !JS_IsArray(args)) {
    if (candidate_name) JS_FreeCString(g_upstream_qjs_context, candidate_name);
    JS_FreeValue(g_upstream_qjs_context, candidate);
    JS_FreeValue(g_upstream_qjs_context, args);
    JS_FreeValue(g_upstream_qjs_context, envelope);
    build_exception_json(ctx, 102u, "bridge", "invalid bridge request envelope", size, 0u);
    return 0u;
  }
  JSValue global = JS_GetGlobalObject(g_upstream_qjs_context);
  JSValue registry = JS_GetPropertyStr(g_upstream_qjs_context, global, "__venomProtectedBridge");
  JSValue function = JS_IsObject(registry) ? JS_GetPropertyStr(g_upstream_qjs_context, registry, candidate_name) : JS_UNDEFINED;
  JS_FreeCString(g_upstream_qjs_context, candidate_name);
  if (!JS_IsFunction(g_upstream_qjs_context, function)) {
    JS_FreeValue(g_upstream_qjs_context, function);
    JS_FreeValue(g_upstream_qjs_context, registry);
    JS_FreeValue(g_upstream_qjs_context, global);
    JS_FreeValue(g_upstream_qjs_context, candidate);
    JS_FreeValue(g_upstream_qjs_context, args);
    JS_FreeValue(g_upstream_qjs_context, envelope);
  ...
  uint32_t argc = 0u;
  JSValue length = JS_GetPropertyStr(g_upstream_qjs_context, args, "length");
  JS_ToUint32(g_upstream_qjs_context, &argc, length);
  JS_FreeValue(g_upstream_qjs_context, length);
  if (argc > 64u) argc = 64u;
  JSValue argv[64];
  for (uint32_t i = 0u; i < argc; ++i) argv[i] = JS_GetPropertyUint32(g_upstream_qjs_context, args, i);
  venom_qjs_begin_execution_budget(rec);
  JSValue result = JS_Call(g_upstream_qjs_context, function, JS_UNDEFINED, (int)argc, argv);
  for (uint32_t i = 0u; i < argc; ++i) JS_FreeValue(g_upstream_qjs_context, argv[i]);
  JS_FreeValue(g_upstream_qjs_context, function);
  JS_FreeValue(g_upstream_qjs_context, registry);
  JS_FreeValue(g_upstream_qjs_context, global);
  JS_FreeValue(g_upstream_qjs_context, candidate);
  ...
  return copied;
#else
  build_exception_json(ctx, 104u, "bridge", "upstream QuickJS bridge executor unavailable", size, 0u);
  return 0u;
#endif
}
#if !defined(__EMSCRIPTEN__)
uint32_t venom_qjs_test_load_bytes(uint32_t ctx, const unsigned char* bytes, uint32_t size) {
  struct QjsContextAccount* rec = find_context(ctx);
...
const char* venom_qjs_test_exception_message(void) { return (const char*)g_exception_message; }
uint32_t venom_qjs_test_source_byte(uint32_t index) { return index < SOURCE_CAP ? (uint32_t)g_source[index] : 0u; }
uint32_t venom_qjs_test_bridge_load_bytes(uint32_t ctx, const unsigned char* bytes, uint32_t size) {
...
uint32_t venom_qjs_test_bridge_result_byte(uint32_t index) { return index < g_bridge_result_size ? (uint32_t)g_bridge_result[index] : 0u; }
#endif
...
static uint32_t execute_native_bytecode_with_interpreter(uint32_t ctx,
  ...
  if (!rec || record_size == 0u || record_size > SOURCE_CAP || payload_size == 0u || payload_size > rec->heap_limit) {
    g_result_size = 0u;
    g_last_status_code = !rec ? QJS_STATUS_BAD_CONTEXT : QJS_STATUS_EXECUTION_TRAP;
    build_status_record_json(ctx, g_last_status_code, "bytecode-dispatch", record_size, record_hash);
    build_exception_json(ctx, !rec ? 46u : 47u, "bytecode", !rec ? "QuickJS context not found" : "native QuickJS bytecode limit exceeded", record_size, record_hash);
  ...
  build_status_record_json(ctx, g_last_status_code, "bytecode-dispatch", record_size, record_hash);
  build_script_record_json(ctx, source_size, source_hash32, 0u);
  venom_qjs_checkpoint_create(ctx, 1u);
  venom_qjs_host_call_dispatch(11u, (uint32_t)(uintptr_t)g_script_record, g_script_record_size);
  g_engine_state = STATE_EXECUTING;
  ...
  if (!venom_qjs_execute_bytecode_with_upstream(ctx, payload_offset, payload_size, record_hash, rec)) {
  ...
  venom_qjs_checkpoint_create(ctx, 4u);
  venom_qjs_snapshot_capture(ctx, 4u);
  venom_qjs_snapshot_validate(ctx, g_last_snapshot_hash);
  venom_qjs_replay_advance(ctx, 4u);
  venom_qjs_execution_commit(ctx, g_last_checkpoint_id);
  venom_qjs_release_accept(ctx);
  venom_qjs_capability_check(ctx, 1u);
  venom_qjs_permission_seal(ctx);
  venom_qjs_release_gate(ctx);
  venom_qjs_host_call_dispatch(12u, (uint32_t)(uintptr_t)g_script_record, g_script_record_size);
  venom_qjs_host_call_dispatch(13u, (uint32_t)(uintptr_t)g_eval_result, g_eval_result_size);
...
VENOM_QJS_PUBLIC("venom_qjs_execute_source") uint32_t venom_qjs_execute_source(uint32_t ctx, uint32_t size) {
  struct QjsContextAccount* rec = make_context(ctx);
  ...
  if (!venom_qjs_execute_source_with_upstream(ctx, size, hash, rec)) {
  ...
  if (contains_word(g_source, size, "console", 7u)) { venom_qjs_host_io_request(ctx, 1u, 1u, hash); if (g_console_count < MAX_CONSOLE_EVENTS) ++g_console_count; build_console_json(ctx, g_console_count); venom_qjs_host_call_dispatch(1u, (uint32_t)(uintptr_t)g_console, g_console_size); build_console_capture_json(ctx); venom_qjs_host_call_dispatch(14u, (uint32_t)(uintptr_t)g_console_capture, g_console_capture_size); }
  if (contains_word(g_source, size, "fetch", 5u)) { venom_qjs_host_io_request(ctx, 3u, 7u, hash); }
  g_fallback_required = 0u;
...
VENOM_QJS_PUBLIC("venom_qjs_bytecode_validate") uint32_t venom_qjs_bytecode_validate(uint32_t ctx, uint32_t size) {
  ...
  if (g_script_buffer_expected_hash && record_hash != g_script_buffer_expected_hash) {
    g_last_validation_code = QJS_STATUS_HASH_MISMATCH;
    g_last_status_code = QJS_STATUS_HASH_MISMATCH;
    build_status_record_json(ctx, g_last_status_code, "bytecode-validate", size, record_hash);
    return g_last_validation_code;
  }
  uint32_t payload_offset = 0u, payload_size = 0u, source_size = 0u, source_hash32 = 0u;
  uint32_t module_count = 0u, entry_index = 0u;
  if (validate_vqjsbc03_record(size, &payload_offset, &payload_size, &source_size, &source_hash32)) {
    g_bytecode_payload_size = payload_size;
    g_bytecode_expected_source_hash32 = source_hash32;
  } else if (validate_vqjsmb04_record(size, &module_count, &entry_index)) {
    g_bytecode_payload_size = size;
    g_bytecode_expected_source_hash32 = module_count ^ (entry_index << 16u);
  } else {
    g_last_validation_code = QJS_STATUS_INVALID_BYTECODE;
    g_last_status_code = QJS_STATUS_INVALID_BYTECODE;
    build_status_record_json(ctx, g_last_status_code, "bytecode-validate", size, record_hash);
    return g_last_validation_code;
  }
  g_last_validation_code = QJS_STATUS_OK;
  g_last_status_code = QJS_STATUS_OK;
  build_status_record_json(ctx, g_last_status_code, "bytecode-validate", size, record_hash);
  return QJS_STATUS_OK;
}

VENOM_QJS_PUBLIC("venom_qjs_execute_bytecode") uint32_t venom_qjs_execute_bytecode(uint32_t ctx, uint32_t size) {
  const uint32_t record_hash = size <= SOURCE_CAP ? fnv32(g_source, size) : 0u;
  const uint32_t validation = venom_qjs_bytecode_validate(ctx, size);
  if (validation != QJS_STATUS_OK) {
    build_exception_json(ctx,
                         validation == QJS_STATUS_HASH_MISMATCH ? 44u : 45u,
                         "bytecode",
                         validation == QJS_STATUS_HASH_MISMATCH ? "protected bytecode record hash mismatch" : "invalid protected QuickJS bytecode record",
                         size,
                         record_hash);
    return 0u;
  }
  ...
  uint32_t module_count = 0u, entry_index = 0u;
  if (validate_vqjsmb04_record(size, &module_count, &entry_index)) {
    struct QjsContextAccount* rec = make_context(ctx);
    if (!rec) return 0u;
#ifdef VENOM_ENABLE_UPSTREAM_QJS_WASM
    const uint32_t ok = venom_qjs_execute_module_bundle_with_upstream(ctx, size, record_hash, rec);
#else
    const uint32_t ok = 0u;
    build_exception_json(ctx, 90u, "quickjs-module-bundle", "upstream QuickJS unavailable", size, record_hash);
#endif
    ...
    g_last_status_code = ok ? QJS_STATUS_OK : QJS_STATUS_EXECUTION_TRAP;
  ...
  if (!validate_vqjsbc03_record(size, &payload_offset, &payload_size, &source_size, &source_hash32)) {
    ...
    build_exception_json(ctx, 45u, "bytecode", "invalid native QuickJS bytecode record", size, record_hash);
    return 0u;
  }
...
VENOM_QJS_PUBLIC("venom_qjs_bytecode_record_executed") uint32_t venom_qjs_bytecode_record_executed(void) { return g_bytecode_record_executed; }
VENOM_QJS_PUBLIC("venom_qjs_bytecode_record_size") uint32_t venom_qjs_bytecode_record_size(void) { return g_bytecode_record_size; }
VENOM_QJS_PUBLIC("venom_qjs_bytecode_record_source_hash32") uint32_t venom_qjs_bytecode_record_source_hash32(void) { return g_bytecode_record_source_hash32; }

VENOM_QJS_PUBLIC("venom_qjs_module_execute") uint32_t venom_qjs_module_execute(uint32_t ctx, uint32_t module_id, uint32_t size) {
  uint32_t ok = venom_qjs_execute_source(ctx, size);
  if (ok) {
    ++g_module_execution_count;
    venom_qjs_host_call_dispatch(7u, (uint32_t)(uintptr_t)g_module_record, g_module_record_size);
    venom_qjs_host_call_dispatch(8u, (uint32_t)(uintptr_t)g_module_record, g_module_record_size);
    venom_qjs_host_call_dispatch(10u, (uint32_t)(uintptr_t)g_module_cache_state, g_module_cache_state_size);
  }
