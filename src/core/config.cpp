...
std::vector<std::string> parse_string_list(std::string value, const std::string& key) {
  ...
  if (value.size() < 2 || value.front() != '[' || value.back() != ']')
    raise_error("VENOM-E1100", "venom.toml: " + key + " must be an array of strings");
  ...
  current = trim(current);
  if (!current.empty()) out.push_back(unquote(current));
  if (quoted) raise_error("VENOM-E1100", "venom.toml: unterminated string in " + key);
  return out;
}
bool parse_bool(const std::string& value, const std::string& key) {
  ...
  raise_error("VENOM-E1100", "venom.toml: " + key + " must be true or false");
}
}
...
void apply_project_config(const std::filesystem::path& path, BuildOptions& options) {
  ...
  if (!in) raise_error("VENOM-E1100", "failed to open config: " + path.string());
  ...
  while (std::getline(in,line)) {
    ...
    const auto eq=line.find('='); if (eq==std::string::npos) raise_error("VENOM-E1100", "venom.toml:"+std::to_string(line_no)+": expected key = value");
    ...
    if (full=="project.entry" || full=="project.input") options.input=unquote(value);
    ...
    else if (full=="project.target") {
      options.project_target=unquote(value);
      if (options.project_target != "web" && options.project_target != "chrome-extension")
        raise_error("VENOM-E1100", "venom.toml: project.target must be web or chrome-extension");
    }
      ...
      if (mode != "auto" && mode != "allow" && mode != "deny" && mode != "read-only")
        raise_error("VENOM-E1100", "venom.toml: " + full + " must be auto, allow, deny, or read-only");
      if (mode == "read-only" && key != "storage")
        raise_error("VENOM-E1100", "venom.toml: read-only is supported only for capabilities.storage");
    ...
    } else raise_error("VENOM-E1100", "venom.toml:"+std::to_string(line_no)+": unknown section or key: "+full);
  }
...
bool validate_project_config(const std::filesystem::path& path, std::string* error) {
  try {
    ...
    if (options.input.empty()) raise_error("VENOM-E1100", "venom.toml: project.entry or project.input is required");
    if (options.profile != "prod") raise_error("VENOM-E1100", "venom.toml: build.profile must be prod; the dev profile was removed");
    return true;
