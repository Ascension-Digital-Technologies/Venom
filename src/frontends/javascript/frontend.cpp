...
void eval_checked(JSContext* ctx, const char* source, std::size_t size, const char* filename) {
  ...
  if (JS_IsException(value)) {
    ...
    raise_error("VENOM-E2000", std::string("JavaScript frontend failed while loading ") + filename + ": " + error);
  }
  JS_FreeValue(ctx, value);
}
...
std::uint32_t get_u32(JSContext* ctx, JSValueConst object, const char* name) {
  ...
  if (JS_ToUint32(ctx, &out, value) < 0) {
    JS_FreeValue(ctx, value);
    raise_error("VENOM-E2000", std::string("JavaScript frontend returned an invalid ") + name);
  }
  JS_FreeValue(ctx, value);
  return out;
}
...
ParseSummary parse(const std::string& source, const std::string& filename, bool module) {
  ...
  try {
    ...
    eval_checked(context, embedded_js_assets::terser_bundle().data(), embedded_js_assets::terser_bundle().size(), "<embedded-terser-frontend>");
    ...
    if (!JS_IsObject(result)) {
      ...
      JS_FreeValue(context, global);
      raise_error("VENOM-E2000", "JavaScript frontend produced no parse result for " + filename);
    }
...
ProtectedUnit lower_protected_unit(const std::string& source, const std::string& filename,
  ...
  try {
    ...
    if (!JS_IsObject(result)) {
      JS_FreeValue(context, result);
      JS_FreeValue(context, global);
      raise_error("VENOM-E2000", "JavaScript frontend produced no protected-unit result for " + filename);
    }
    ProtectedUnit unit;
...
FunctionScopeAnalysis analyze_function_scope(const std::string& source, const std::string& filename,
  ...
  try {
    ...
    if (!JS_IsObject(result)) {
      JS_FreeValue(context, result);
      JS_FreeValue(context, global);
      raise_error("VENOM-E2000", "JavaScript frontend produced no scope result for " + filename);
    }
