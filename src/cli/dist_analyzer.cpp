...
std::vector<unsigned char> read_bytes(const std::filesystem::path& path) {
  ...
  if (!in) raise_error("VENOM-E1000", "failed to read distribution file: " + path.string());
...
bool analyze_distribution(const DistAnalyzeOptions& options) {
  ...
  if (!std::filesystem::is_directory(root)) raise_error("VENOM-E1000", "analyze requires a distribution directory");
