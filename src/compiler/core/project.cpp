...
void write_text(const fs::path& path, const std::string& text, bool force) {
  if (fs::exists(path) && !force) throw std::runtime_error("refusing to overwrite existing file: " + path.string() + " (use --force)");
...
bool initialize_project(const InitProjectOptions& options) {
  ...
  if (!fs::is_directory(root)) throw std::runtime_error("init target is not a directory: " + root.string());
...
bool create_project(const NewProjectOptions& options) {
  if (options.directory.empty()) throw std::runtime_error("new requires a project name or path");
  const auto root = fs::absolute(options.directory);
  if (fs::exists(root) && !fs::is_empty(root) && !options.force) throw std::runtime_error("target directory is not empty: " + root.string() + " (use --force)");
