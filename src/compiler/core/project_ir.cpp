...
void write_project_ir_atomic(const ProjectIr& ir, const std::filesystem::path& path) {
  ...
  const auto temporary = path.string() + ".tmp";
  {
    ...
    if (!out) throw std::runtime_error("failed to write project IR: " + temporary);
    out << serialize_project_ir(ir);
    if (!out) throw std::runtime_error("failed to flush project IR: " + temporary);
  }
  std::error_code ec;
  ...
  if (ec) {
    ...
    ec.clear();
    std::filesystem::rename(temporary, path, ec);
    if (ec) throw std::runtime_error("failed to commit project IR: " + path.string());
  }
}
