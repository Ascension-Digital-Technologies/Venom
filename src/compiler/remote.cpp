...
std::vector<unsigned char> read_bytes(const std::filesystem::path& path) {
  ...
  if (!input) {
    throw std::runtime_error("failed to read remote vendor file: " + path.string());
  }
...
void write_bytes_atomic(const std::filesystem::path& path, const std::vector<unsigned char>& bytes) {
  ...
  const std::filesystem::path temporary = temporary_name.str();
  {
    ...
    if (!output) {
      throw std::runtime_error("failed to write remote vendor file: " + temporary.string());
    }
    output.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
    if (!output) {
      throw std::runtime_error("failed to write remote vendor bytes: " + temporary.string());
    }
  }
  std::error_code ec;
...
std::size_t parse_size_value(const std::string& value, const std::string& context) {
  if (value.empty() || !std::all_of(value.begin(), value.end(), [](unsigned char c) { return std::isdigit(c) != 0; })) {
    throw std::runtime_error("invalid numeric value in " + context);
  }
  try {
    return static_cast<std::size_t>(std::stoull(value));
  } catch (const std::exception&) {
    throw std::runtime_error("numeric value out of range in " + context);
  }
}
...
void validate_script_bytes(const std::vector<unsigned char>& bytes,
  ...
  if (bytes.empty()) {
    throw std::runtime_error("remote script is empty: " + url);
  }
  if (bytes.size() > max_script_bytes) {
    throw std::runtime_error("remote script exceeds the production vendor size limit: " + url);
  }
  if (std::find(bytes.begin(), bytes.end(), static_cast<unsigned char>(0)) != bytes.end()) {
    throw std::runtime_error("remote script contains NUL bytes: " + url);
  }
  if (!valid_utf8(bytes)) {
    throw std::runtime_error("remote script is not valid UTF-8: " + url);
  }
  ...
  if (first != std::string::npos) {
    ...
    if (trimmed.rfind("<!doctype html", 0) == 0 || trimmed.rfind("<html", 0) == 0 ||
      ...
      throw std::runtime_error("remote script request returned HTML instead of JavaScript: " + url);
    }
  }
}
...
std::string url_host(const std::string& normalized_url) {
  ...
  if (authority.find('@') != std::string::npos) {
    throw std::runtime_error("remote script URL credentials are denied");
  }
  if (authority.front() == '[') {
    throw std::runtime_error("remote script IPv6 literals are denied");
  }
...
void validate_public_https_url(const std::string& normalized_url) {
  ...
  if (normalized_url.rfind("https://", 0) != 0) {
    throw std::runtime_error("production remote script vendoring requires HTTPS: " + normalized_url);
  }
  const auto host = url_host(normalized_url);
  if (host.empty()) {
    throw std::runtime_error("remote script URL is missing a host: " + normalized_url);
  }
  if (host == "localhost" ||
    ...
    throw std::runtime_error("remote script URL targets a private or local host: " + host);
  }
}
...
std::string integrity_status_for(const std::string& declared_integrity,
  ...
  if (saw_supported) {
    throw std::runtime_error("remote script SRI integrity mismatch: " + url);
  }
  throw std::runtime_error("remote script declares no supported SRI algorithm (sha256/sha384/sha512): " + url);
}
  ...
  for (const auto& record : records) {
    ...
    if (!inserted.second) {
      ...
      if (existing.content_sha256 != entry.content_sha256 || existing.byte_size != entry.byte_size ||
        ...
        throw std::runtime_error("conflicting remote dependency records for: " + entry.normalized_url);
      }
    }
  }
...
RemoteVendorLock load_remote_vendor_lock(const std::filesystem::path& path) {
  ...
  if (!std::filesystem::is_regular_file(path)) {
    throw std::runtime_error("vendor lock path is not a regular file: " + path.string());
  }
  const auto text = read_text(path);
  std::stringstream input(text);
  std::string line;
  if (!std::getline(input, line)) {
    throw std::runtime_error("vendor lock has an invalid header: " + path.string());
  }
  ...
  if (!std::getline(input, version_line) || !std::getline(input, count_line) || !std::getline(input, columns_line)) {
    throw std::runtime_error("vendor lock is truncated: " + path.string());
  }
  ...
  if (version_line != "version=1" || columns_line != "url\tsha256\tbytes\tintegrity") {
    throw std::runtime_error("vendor lock schema is unsupported: " + path.string());
  }
  if (count_line.rfind("entry_count=", 0) != 0) {
    throw std::runtime_error("vendor lock is missing entry_count: " + path.string());
  }
  ...
  while (std::getline(input, line)) {
    ...
    if (fields.size() != 4u) {
      throw std::runtime_error("vendor lock entry has an invalid field count: " + path.string());
    }
    RemoteVendorLockEntry entry;
    entry.normalized_url = normalize_remote_script_url(fields[0]);
    if (entry.normalized_url != fields[0]) {
      throw std::runtime_error("vendor lock URL is not normalized: " + fields[0]);
    }
    entry.content_sha256 = fields[1];
    if (!is_hex_digest(entry.content_sha256, 64u)) {
      throw std::runtime_error("vendor lock has an invalid SHA-256 digest: " + entry.normalized_url);
    }
    entry.byte_size = parse_size_value(fields[2], "vendor lock byte size");
    if (entry.byte_size == 0u) {
      throw std::runtime_error("vendor lock byte size must be non-zero: " + entry.normalized_url);
    }
    ...
    if (entry.integrity_status != "not-declared" &&
      ...
      throw std::runtime_error("vendor lock has an invalid integrity status: " + entry.normalized_url);
    }
    if (!seen.insert(entry.normalized_url).second) {
      throw std::runtime_error("vendor lock contains a duplicate URL: " + entry.normalized_url);
    }
    lock.entries.push_back(std::move(entry));
  }
  if (lock.entries.size() != expected_count) {
    throw std::runtime_error("vendor lock entry_count does not match its records: " + path.string());
  }
...
void validate_remote_vendor_lock_coverage(const RemoteVendorLock& lock,
  ...
  if (actual.size() != lock.entries.size()) {
    throw std::runtime_error("venom.lock dependency count mismatch; run with --refresh-vendors to update the lock");
  }
  for (std::size_t i = 0; i < actual.size(); ++i) {
    const auto& expected = lock.entries[i];
    const auto& observed = actual[i];
    if (expected.normalized_url != observed.normalized_url) {
      throw std::runtime_error("venom.lock dependency set mismatch; run with --refresh-vendors to update the lock");
    }
    if (expected.content_sha256 != observed.content_sha256 || expected.byte_size != observed.byte_size ||
      ...
      throw std::runtime_error("venom.lock record mismatch for: " + observed.normalized_url);
    }
  }
}
...
RemoteVendorResult vendor_remote_script(const std::string& source_url,
  ...
  if (!options.enabled) {
    throw std::runtime_error("production remote script vendoring is disabled");
  }
  ...
  if (options.lock_present && !options.refresh && locked == nullptr) {
    throw std::runtime_error("remote dependency is not present in venom.lock: " + normalized_url + "; run with --refresh-vendors to update the lock");
  }
  ...
  if (!options.refresh && try_read_cache(body_path, metadata_path, normalized_url, options.max_script_bytes, result.bytes)) {
    ...
    if (options.offline) {
      throw std::runtime_error("remote vendor cache miss in offline mode: " + normalized_url);
    }
    ...
    // used to transport an HTTPS request. Redirect following is enabled with a
    // zero redirect budget so any redirect fails instead of changing origin.
    ...
    if (size_error) {
      cleanup_temporary();
      throw std::runtime_error("failed to inspect downloaded remote vendor file: " + normalized_url);
    }
    if (downloaded_size > options.max_script_bytes) {
      cleanup_temporary();
      throw std::runtime_error("remote script exceeds the production vendor size limit: " + normalized_url);
    }
    try {
  ...
  if (locked != nullptr && !options.refresh) {
    if (locked->content_sha256 != result.record.content_sha256 ||
      ...
      throw std::runtime_error("venom.lock rejected changed remote dependency: " + normalized_url + "; run with --refresh-vendors only after reviewing the dependency update");
    }
