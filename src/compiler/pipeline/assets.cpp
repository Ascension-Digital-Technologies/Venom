...
bool is_public_asset(const SiteFile& file, bool production) {
...
void write_bytes(const std::filesystem::path& path, const std::vector<unsigned char>& bytes) {
  ...
  if (!out) {
    throw std::runtime_error("failed to write asset " + path.string());
  }
...
AssetPipeline build_asset_pipeline(const SiteGraph& graph, bool hashed, bool production) {
  AssetPipeline pipeline;
  pipeline.hashed = hashed;
  std::unordered_set<std::string> used_names;

  for (const auto& file : graph.files) {
    if (!is_public_asset(file, production)) {
      continue;
    }
...
void emit_public_assets(const AssetPipeline& pipeline, const std::filesystem::path& output_assets_dir) {
  ...
  for (const auto& record : pipeline.records) {
    if (record.file == nullptr) {
      throw std::runtime_error("asset pipeline record is missing source file: " + record.source);
    }
