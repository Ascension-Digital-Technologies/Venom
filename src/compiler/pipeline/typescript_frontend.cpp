...
std::string read_file_text(const std::filesystem::path& path) {
  ...
  if (!input) throw std::runtime_error("unable to read structural TypeScript frontend output: " + path.string());
  return std::string(std::istreambuf_iterator<char>(input), std::istreambuf_iterator<char>());
}

void write_file_text(const std::filesystem::path& path, const std::string& text) {
  std::ofstream output(path, std::ios::binary | std::ios::trunc);
  if (!output) throw std::runtime_error("unable to create structural TypeScript frontend input: " + path.string());
...
std::size_t json_array_count(const std::string& json, const std::string& field) {
  const auto marker = json.find("\"" + field + "\"");
  if (marker == std::string::npos) return 0;
  const auto open = json.find('[', marker);
  ...
  return static_cast<std::size_t>(std::count(json.begin() + static_cast<std::ptrdiff_t>(open),
                                             json.begin() + static_cast<std::ptrdiff_t>(close), '{'));
}

std::string structural_cache_key(const std::string& source, const std::string& filename) {
  const char* node_env = std::getenv("VENOM_NODE");
  const std::string node = node_env && *node_env ? node_env : "node";
  const std::string material = "venom-typescript-structural-cache-v2\nfrontend=typescript-compiler-api\nversion=5.9\njsx=venom-classic-v1\nnode=" + node + "\nfilename=" + filename + "\nsource=" + source;
  return venom::package::sha256_hex(reinterpret_cast<const unsigned char*>(material.data()), material.size());
}

bool read_structural_cache(const std::filesystem::path& base, TranspileResult& result) {
  ...
  if (metadata.find("schema=2\n") != 0) return false;
  result.javascript = read_file_text(js);
  result.source_map = std::filesystem::is_regular_file(base.string() + ".map") ? read_file_text(base.string() + ".map") : std::string{};
  result.frontend = "typescript-compiler-api";
  result.frontend_version = "5.9";
  result.typescript = true;
  result.tsx = metadata.find("tsx=1\n") != std::string::npos;
  const auto pos = metadata.find("diagnostics=");
  if (pos != std::string::npos) result.diagnostic_count = static_cast<std::size_t>(std::stoull(metadata.substr(pos + 12)));
...
void write_structural_cache(const std::filesystem::path& base, const TranspileResult& result) {
  ...
  meta << "schema=2\nfrontend=typescript-compiler-api\nversion=5.9\ntsx=" << (result.tsx ? 1 : 0)
       << "\ndiagnostics=" << result.diagnostic_count << "\njsx=" << result.lowered_jsx_elements << "\n";
  write_file_text(base.string() + ".meta", meta.str());
}

TranspileResult transpile_structural(const std::string& source, const std::string& filename) {
#ifdef VENOM_SOURCE_ROOT
  const std::filesystem::path root(VENOM_SOURCE_ROOT);
#else
  const std::filesystem::path root = std::filesystem::current_path();
#endif
  const auto driver = root / "tools" / "typescript_structural_frontend.js";
  const auto compiler = root / "third_party" / "typescript" / "lib" / "typescript.js";
  if (!std::filesystem::is_regular_file(driver) || !std::filesystem::is_regular_file(compiler)) {
    throw std::runtime_error("structural TypeScript frontend assets are unavailable");
  }
  ...
  bool cache_verbose = false;
  {
    ...
    if (cache_enabled) {
      const auto digest = structural_cache_key(source, filename);
      cache_base = g_cache_directory / digest.substr(0, 2) / digest;
    }
  }
  if (cache_enabled) {
    TranspileResult cached;
    if (read_structural_cache(cache_base, cached)) {
      ++g_cache_hits;
      if (cache_verbose) std::cerr << "[CACHE]    TypeScript frontend hit: " << filename << "\n";
      return cached;
    }
    ++g_cache_misses;
    if (cache_verbose) std::cerr << "[CACHE]    TypeScript frontend miss: " << filename << "\n";
  }

  const auto nonce = std::to_string(std::chrono::high_resolution_clock::now().time_since_epoch().count());
  const auto temp = std::filesystem::temp_directory_path() / ("venom-ts-" + nonce);
  std::filesystem::create_directories(temp);
  const auto extension = std::filesystem::path(filename).extension().string();
  const auto input = temp / ("input" + (extension.empty() ? ".ts" : extension) + ".venom-input");
  const auto output = temp / "output.js";
  const auto metadata = temp / "metadata.json";
  write_file_text(input, source);

  const char* node_env = std::getenv("VENOM_NODE");
  const std::string node = node_env && *node_env ? node_env : "node";
  const int status = run_process(node, {driver.string(), input.string(), output.string(), metadata.string()});
  if (status != 0 || !std::filesystem::is_regular_file(output) || !std::filesystem::is_regular_file(metadata)) {
    std::filesystem::remove_all(temp);
    SourceLocation location;
    location.file = filename;
    location.line = 1;
    location.column = 0;
    raise_diagnostic("VENOM-E2504", "Structural TypeScript lowering failed", std::move(location),
                     "The vendored TypeScript compiler frontend exited with status " + std::to_string(status) + ".",
                     "Install Node.js 18 or newer, or set VENOM_TYPESCRIPT_FRONTEND=native for the compatibility frontend.",
                     "docs/reference/diagnostics.md#venom-e2504");
  }

  TranspileResult result;
  result.typescript = true;
  { const auto lowered_name = lower(filename); result.tsx = lowered_name.size() >= 4 && lowered_name.compare(lowered_name.size() - 4, 4, ".tsx") == 0; }
  result.javascript = read_file_text(output);
  const auto meta = read_file_text(metadata);
  result.frontend = json_string_field(meta, "frontend");
  result.frontend_version = json_string_field(meta, "version");
  result.diagnostic_count = json_array_count(meta, "diagnostics");
  const auto source_map_path = std::filesystem::path(output.string() + ".map");
  if (std::filesystem::is_regular_file(source_map_path)) result.source_map = read_file_text(source_map_path);
  ...
  if (cache_enabled) {
    std::lock_guard<std::mutex> lock(g_cache_mutex);
    write_structural_cache(cache_base, result);
    ++g_cache_writes;
  }
  std::filesystem::remove_all(temp);
  return result;
...
TranspileResult transpile(const std::string& source, const std::string& filename) {
  ...
  if (selected && std::string(selected) == "native") {
    auto result = transpile_native(source, filename);
    result.frontend = "venom-native-typescript-eraser";
    result.frontend_version = "4";
    return result;
  }
  return transpile_structural(source, filename);
}

const char* frontend_name() { return "typescript-compiler-api"; }
const char* frontend_version() { return "5.9"; }
