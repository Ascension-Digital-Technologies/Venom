...
bool build_site(const BuildOptions& requested_options) {
  ...
  if (external_lazy_registry) {
    for (std::size_t i = 0; i < js_bridge.bridge_registry_chunks.size(); ++i) {
      ...
      if (!lazy_out) throw std::runtime_error("failed to write lazy protected registry chunk: " + compiled_chunk.id);
      ...
      lazy_out.flush();
      if (!lazy_out) throw std::runtime_error("failed to finalize lazy protected registry chunk: " + compiled_chunk.id);
    }
  }
  {
