...
void strip_protected_route_vm_js(std::string& source) {
  ...
  const std::string opcode_line = "  const opcodeMap = parsePolymorphicMap(findSection(pkg, SECTION.INTEGRITY, 'vm-polymorph.vpol'));\n";
  const auto opcode_pos = source.find(opcode_line);
  if (opcode_pos == std::string::npos) throw std::runtime_error("failed to remove protected JavaScript opcode-map parsing");
  source.erase(opcode_pos, opcode_line.size());
...
function installVenomWasmPackageParser(instance) {
  ...
  venomPackageParser = (bytes, expectedPackageHash) => {
    ...
    if (bytes.length > packageCap) throw new Error('package exceeds WASM parser capacity');
    wasmMemoryView(instance).set(bytes, packagePtr);
    const rc = e.v12_x(1, bytes.length >>> 0, 0, 0, 0);
    venomWasmLoadedPackageSize = bytes.length >>> 0;
    ...
    const pkg = {
      bytes, view: null, version: readU32(mv, 12), runtimeAbi: readU32(mv, 16), flags: readU32(mv, 20),
    ...
    if (readU32(mv, 52) !== bytes.length) throw new Error('WASM package metadata file-size mismatch');
...
function installVenomWasmPackageDecoder(instance) {
  ...
  venomPackageSectionDecoder = (pkg, sectionIndex, rawSize, name, type) => {
    if (!pkg || !(pkg.bytes instanceof Uint8Array)) throw new Error('WASM section decoder received an invalid package');
    if (loadedPackage !== pkg) {
      const packagePtr = e.v12_p(0);
      const packageCap = e.v12_n(0);
      if (pkg.bytes.length > packageCap) throw new Error('package exceeds WASM section decoder capacity');
      wasmMemoryView(instance).set(pkg.bytes, packagePtr);
      loadedPackage = pkg;
    }
    const rc = e.v12_x(3, pkg.bytes.length >>> 0, sectionIndex >>> 0, type >>> 0, rawSize >>> 0);
