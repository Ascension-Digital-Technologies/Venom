...
std::uint32_t read_u32(const std::vector<unsigned char>& bytes, std::size_t& pos) {
  ...
  while (pos < bytes.size()) {
    ...
    shift += 7u;
    if (shift > 35u) throw std::runtime_error("invalid WebAssembly LEB128 value");
  }
  throw std::runtime_error("truncated WebAssembly LEB128 value");
}

std::string read_name(const std::vector<unsigned char>& bytes, std::size_t& pos) {
  const auto size = read_u32(bytes, pos);
  if (pos + size > bytes.size()) throw std::runtime_error("truncated WebAssembly export name");
...
std::vector<std::string> inspect_exports(const std::vector<unsigned char>& bytes) {
  if (bytes.size() < 8u || bytes[0] != 0x00u || bytes[1] != 0x61u || bytes[2] != 0x73u || bytes[3] != 0x6du) {
    throw std::runtime_error("embedded QuickJS runtime is not a WebAssembly module");
  }
  std::size_t pos = 8u;
  ...
  while (pos < bytes.size()) {
    ...
    if (end > bytes.size()) throw std::runtime_error("truncated WebAssembly section");
    if (id == 7u) {
      const auto count = read_u32(bytes, pos);
      exports.reserve(count);
      for (std::uint32_t i = 0; i < count; ++i) {
        exports.push_back(read_name(bytes, pos));
        if (pos >= end) throw std::runtime_error("truncated WebAssembly export entry");
        ++pos;
