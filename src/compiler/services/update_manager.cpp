...
fs::path find_tool(const fs::path& executable) {
  ...
  throw std::runtime_error("update tool not found; expected tools/update_release.py beside the Venom release or source tree");
}
std::string find_python() {
  ...
  if (run_process("python", {"--version"}) == 0) return "python";
#endif
  throw std::runtime_error("Python 3 is required for secure release updates");
}
}
bool manage_update(const UpdateOptions& o, const fs::path& executable) {
  ...
  if (code < 0) throw std::runtime_error("failed to start update helper");
  return code == 0;
}
}
