...
void install(const fs::path& path, bool force) {
  ...
  { std::ofstream out(tmp, std::ios::binary|std::ios::trunc); if(!out) throw std::runtime_error("failed to create runtime: "+tmp); out.write(reinterpret_cast<const char*>(kQuickJsRuntimeWasmBlob), sizeof(kQuickJsRuntimeWasmBlob)); }
  if (!valid_wasm(tmp)) { fs::remove(tmp); throw std::runtime_error("embedded runtime verification failed"); }
...
bool manage_runtime(const RuntimeOptions& options) {
  ...
  throw std::runtime_error("runtime action must be install, verify, update, or path");
}
}
