...
bool compile_snippet(std::istream& input, std::ostream& output) {
  ...
  if (source.empty()) throw std::runtime_error("compile-snippet received empty source");
  if (source.size() > 1024u * 1024u) throw std::runtime_error("compile-snippet source exceeds 1 MiB");

  const auto bytecode = venom::quickjs::compile_native_quickjs_bytecode(
      source, "playground/user-script.js", false, true, nullptr);
  if (bytecode.empty()) throw std::runtime_error("QuickJS produced an empty bytecode record");
